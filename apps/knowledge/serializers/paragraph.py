...
from knowledge.serializers.problem import (
...
class ParagraphSerializers(serializers.Serializer):
    ...
    content = serializers.CharField(
        required=True, max_length=102400, label=_("section title")
    )
    ...
    class Problem(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                ...
                .filter(id=self.data.get("paragraph_id"))
                .exists()
            ):
        ...
        def list(self, with_valid=False):
            ...
            return [
                ...
                for row in QuerySet(Problem).filter(
                    id__in=[row.problem_id for row in problem_paragraph_mapping]
                )
            ]
        ...
        def save(
            ...
            if (
                ...
                raise AppApiException(
                    500, _("Already associated, please do not associate again")
                )
    ...
    class Operate(serializers.Serializer):
        ...
        def post_embedding(paragraph, instance, knowledge_id):
            if "is_active" in instance and instance.get("is_active") is not None:
                (
                    ...
                    else disable_embedding_by_paragraph
                )(paragraph.get("id"))
        ...
        def edit(self, instance: Dict):
            ...
            if "problem_list" in instance:
                update_problem_list = list(
                    filter(
                        lambda row: "id" in row and row.get("id") is not None,
                        instance.get("problem_list"),
                    )
                )

                create_problem_list = list(
                    filter(
                        lambda row: row.get("id") is None, instance.get("problem_list")
                    )
                )
                ...
                for update_problem in update_problem_list:
                    if not set([str(row.id) for row in problem_list]).__contains__(
                        update_problem.get("id")
                    ):
                ...
                delete_problem_list = (
                    list(
                        filter(
                            lambda row: not [
                                str(update_row.get("id"))
                                for update_row in update_problem_list
                            ].__contains__(str(row.id)),
                            problem_list,
                        )
                    )
                ...
                (
                    QuerySet(Problem)
                    .filter(id__in=[row.id for row in delete_problem_list])
                    .delete()
                ...
                (
                    QuerySet(Problem).bulk_update(
                        [
                            Problem(id=row.get("id"), content=row.get("content"))
                            for row in update_problem_list
                        ],
                        ["content"],
                    )
                    if len(update_problem_list) > 0
        ...
        def one(self, with_valid=False):
            ...
            return {
                **ParagraphSerializer(
                    QuerySet(model=Paragraph).get(id=self.data.get("paragraph_id"))
                ).data,
    ...
    class Create(serializers.Serializer):
        ...
        def save(self, instance: Dict, with_valid=True, with_embedding=True):
            ...
            Paragraph.objects.filter(document_id=document_id).update(
                position=F("position") + 1
            )

            paragraph_problem_model = self.get_paragraph_problem_model(
                knowledge_id, document_id, instance
            )
            paragraph = paragraph_problem_model.get("paragraph")
            problem_paragraph_object_list = paragraph_problem_model.get(
        ...
        def or_get(exists_problem_list, content, knowledge_id):
            ...
            if len(exists) > 0:
                ...
                return Problem(
                    id=uuid.uuid7(), content=content, knowledge_id=knowledge_id
                )
    ...
    class Query(serializers.Serializer):
        ...
        def list(self):
            return list(
                map(lambda row: ParagraphSerializer(row).data, self.get_query_set())
            )
        ...
        def page(self, current_page, page_size):
            ...
            return page_search(
                ...
                page_size,
                query_set,
                lambda row: ParagraphSerializer(row).data,
            )
    ...
    class Association(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=True):
            ...
            if (
                not QuerySet(Paragraph)
                .filter(knowledge_id=knowledge_id, id=paragraph_id)
                .exists()
            ):
                raise AppApiException(500, _("Paragraph does not exist"))
            if (
                not QuerySet(Problem)
                .filter(knowledge_id=knowledge_id, id=problem_id)
                .exists()
            ):
    ...
    class Batch(serializers.Serializer):
        ...
        def batch_delete(self, instance: Dict, with_valid=True):
            if with_valid:
                BatchSerializer(data=instance).is_valid(
                    model=Paragraph, raise_exception=True
                )
                self.is_valid(raise_exception=True)
        ...
        def batch_generate_related(self, instance: Dict, with_valid=True):
            ...
            try:
                ...
                raise AppApiException(
                    500, _("The task is being executed, please do not send it again.")
                )
    ...
    class Migrate(serializers.Serializer):
        ...
        target_document_id = serializers.UUIDField(
            required=True, label=_("target document id")
        )
        paragraph_id_list = serializers.ListField(
            required=True,
            label=_("paragraph id list"),
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            document_list = QuerySet(Document).filter(
                id__in=[
                    ...
                    self.data.get("target_document_id"),
                ]
            )
            document_id = self.data.get("document_id")
            ...
            if document_id == target_document_id:
                raise AppApiException(
                    5000,
                    _(
                        "The document to be migrated is consistent with the target document"
                    ),
                )
            if (
                len(
                    [
                        ...
                        if str(document.id) == self.data.get("document_id")
                    ]
                )
                < 1
            ):
            ...
            if (
                len(
                    [
                        document
                        for document in document_list
                        if str(document.id) == self.data.get("target_document_id")
                    ]
                )
                < 1
            ):
        ...
        def migrate(self, with_valid=True):
            ...
            if target_knowledge_id == knowledge_id:
                if len(problem_paragraph_mapping_list):
                    problem_paragraph_mapping_list = [
                        self.update_problem_paragraph_mapping(
                            target_document_id, problem_paragraph_mapping
                        )
                        for problem_paragraph_mapping in problem_paragraph_mapping_list
                    ]
                    # 修改mapping
                    QuerySet(ProblemParagraphMapping).bulk_update(
                        problem_paragraph_mapping_list, ["document_id"]
                    )
                update_embedding_document_id(
            ...
            # 不同数据集迁移
                ...
                # 修改段落信息
                paragraph_list.update(
                    knowledge_id=target_knowledge_id, document_id=target_document_id
                )
        ...
        @staticmethod
        def update_problem_paragraph_mapping(
            target_document_id: str, problem_paragraph_mapping
        ):
