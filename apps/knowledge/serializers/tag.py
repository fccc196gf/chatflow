...
class TagSerializers(serializers.Serializer):
    class Create(serializers.Serializer):
        ...
        tags = serializers.ListField(
            required=True, label=_("Tags"), child=TagCreateSerializer()
        )
    ...
    class Operate(serializers.Serializer):
        ...
        def edit(self, instance: Dict):
            ...
            if instance.get("key") and instance.get("key") != tag.key:
                ...
                existing_key_exists = (
                    QuerySet(Tag)
                    .filter(knowledge_id=tag.knowledge_id, key=new_key)
                    .exists()
                )
                ...
                QuerySet(Tag).filter(knowledge_id=tag.knowledge_id, key=old_key).update(
                    key=new_key
                )
            ...
            if instance.get("value") and instance.get("value") != tag.value:
                ...
                QuerySet(Tag).filter(id=tag.id).update(value=instance.get("value"))
        ...
        def delete(self, delete_type: str):
            ...
            if delete_type == "key":
                ...
                if tag is None:
                    raise AppApiException(500, _("Tag id does not exist"))
                QuerySet(Tag).filter(
                    knowledge_id=tag.knowledge_id, key=tag.key
                ).delete()
    ...
    class BatchDelete(serializers.Serializer):
        ...
        tag_ids = serializers.ListField(
            required=True, label=_("Tag IDs"), child=serializers.UUIDField()
        )
        ...
        def batch_delete(self):
            ...
            # 删除具有相同key的所有标签
            QuerySet(Tag).filter(
                knowledge_id=self.data.get("knowledge_id"), key__in=keys_to_delete
            ).delete()
