        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if response.status == 500:
                raise AppApiException(
                    500,
                    _("URL error, cannot parse [{source_url}]").format(
    ...
    def is_valid(self, *, model=None, raise_exception=False):
        ...
        if model is not None:
            ...
            if len(model_list) != len(id_list):
                ...
                raise AppApiException(
                    500,
                    _("The following id does not exist: {error_id_list}").format(
...
class GenerateRelatedSerializer(serializers.Serializer):
    ...
    state_list = serializers.ListField(
        required=False,
...
class ProblemParagraphManage:
    def __init__(
        self, problem_paragraph_object_list: List[ProblemParagraphObject], knowledge_id
    ):
        self.knowledge_id = knowledge_id
    ...
    def to_problem_model_list(self):
        problem_list = [
            item.problem_content for item in self.problem_paragraph_object_list
        ]
        exists_problem_list = []
        if len(self.problem_paragraph_object_list) > 0:
            ...
            exists_problem_list = (
                QuerySet(Problem)
                .filter(knowledge_id=self.knowledge_id, content__in=problem_list)
                .all()
            )
        ...
        problem_model_list = [
            or_get(
                ...
                problemParagraphObject.paragraph_id,
                problem_content_dict,
            )
        ...
        result = [
            problem_model
            for problem_model, is_create in problem_content_dict.values()
            if is_create
        ], problem_paragraph_mapping_list
        return result
...
def get_embedding_model_by_knowledge_id_list(knowledge_id_list: List):
    ...
    if len(knowledge_list) == 0:
        raise Exception(
            _("Knowledge base setting error, please reset the knowledge base")
        )
...
def list_paragraph(paragraph_list: List[str]):
    ...
    return native_search(
        QuerySet(Paragraph).filter(id__in=paragraph_list),
        get_file_content(
...
def create_knowledge_index(knowledge_id=None, document_id=None):
    ...
    if not index:
        ...
        if dims < 2000:
            ...
            update_execute(sql, [])
            chatflow_logger.info(f"Created index for knowledge ID: {k_id}")
...
def drop_knowledge_index(knowledge_id=None, document_id=None):
    ...
    if index:
        ...
        update_execute(sql, [])
        chatflow_logger.info(f"Dropped index for knowledge ID: {k_id}")
...
def update_resource_mapping_by_knowledge(knowledge_id: str):
    ...
    if knowledge.type == KnowledgeType.WORKFLOW:
        ...
        save_workflow_mapping(
            {}, ResourceType.KNOWLEDGE, str(knowledge_id), instance_mapping
        )
