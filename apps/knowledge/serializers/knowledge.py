...
class KnowledgeBaseCreateRequest(serializers.Serializer):
    ...
    embedding_model_id = serializers.CharField(
        required=True, label=_("knowledge embedding")
    )
...
class KnowledgeWebCreateRequest(serializers.Serializer):
    ...
    selector = serializers.CharField(
        required=False, label=_("knowledge selector"), allow_null=True, allow_blank=True
    )


class KnowledgeEditRequest(serializers.Serializer):
    name = serializers.CharField(
        required=False, max_length=64, min_length=1, label=_("knowledge name")
    )
    desc = serializers.CharField(
        required=False, max_length=256, min_length=1, label=_("knowledge description")
    )
    ...
    file_size_limit = serializers.IntegerField(
        required=False, label=_("file size limit")
    )
    file_count_limit = serializers.IntegerField(
        required=False, label=_("file count limit")
    )
...
class HitTestSerializer(serializers.Serializer):
    ...
    top_number = serializers.IntegerField(
        required=True, max_value=10000, min_value=1, label=_("top number")
    )
    similarity = serializers.FloatField(
        required=True, max_value=2, min_value=0, label=_("similarity")
    )
...
class KnowledgeSerializer(serializers.Serializer):
    class Query(serializers.Serializer):
        ...
        folder_id = serializers.CharField(
            required=False, label=_("folder id"), allow_null=True
        )
        name = serializers.CharField(
            required=False,
        ...
        user_id = serializers.UUIDField(
            required=False, label=_("user id"), allow_null=True
        )
        scope = serializers.CharField(
            required=False, label=_("knowledge scope"), allow_null=True
        )
        ...
        def get_query_set(self, workspace_manage, is_x_pack_ee):
            ...
            if "desc" in self.data and self.data.get("desc") is not None:
                query_set = query_set.filter(
                    **{"temp.desc__icontains": self.data.get("desc")}
                )
                folder_query_set = folder_query_set.filter(
                    **{"desc__icontains": self.data.get("desc")}
                )
            if "name" in self.data and self.data.get("name") is not None:
                query_set = query_set.filter(
                    **{"temp.name__icontains": self.data.get("name")}
                )
                folder_query_set = folder_query_set.filter(
                    **{"name__icontains": self.data.get("name")}
                )
            if (
                "workspace_id" in self.data
                and self.data.get("workspace_id") is not None
            ):
                query_set = query_set.filter(
                    **{"temp.workspace_id": self.data.get("workspace_id")}
                )
                folder_query_set = folder_query_set.filter(
                    **{"workspace_id": self.data.get("workspace_id")}
                )
            if (
                ...
                query_set = query_set.filter(
                    **{"temp.folder_id": self.data.get("folder_id")}
                )
                folder_query_set = folder_query_set.filter(
                    **{"parent_id": self.data.get("folder_id")}
                )
            ...
            if "create_user" in self.data and self.data.get("create_user") is not None:
                query_set = query_set.filter(
                    **{"temp.user_id": self.data.get("create_user")}
                )
    ...
    class Operate(serializers.Serializer):
        ...
        def embedding(self, with_valid=True):
            ...
            ListenerManagement.get_aggregation_document_status_by_knowledge_id(
            ...
            try:
                ...
                raise AppApiException(
                    500,
                    _("Failed to send the vectorization task, please try again later!"),
                )
        ...
        def generate_related(self, instance: Dict, with_valid=True):
            ...
            try:
                generate_related_by_knowledge_id.delay(
                    knowledge_id, model_id, model_params_setting, prompt, state_list
                )
            except AlreadyQueued as e:
                raise AppApiException(
                    500,
        ...
        def one(self):
            ...
            all_application_list = [
                str(adm.get("id")) for adm in self.list_application(with_valid=False)
            ]
            ...
            if knowledge_dict.get("type") == 4:
                ...
                k = (
                    ...
                    .filter(knowledge_id=knowledge_dict.get("id"))
                    .first()
                )
                if k:
            ...
            return {
                ...
                **workflow,
                ...
                "application_id_list": list(
                    filter(
                        lambda application_id: all_application_list.__contains__(
                            application_id
                        ),
                        [
                            str(application_knowledge_mapping.source_id)
                            for application_knowledge_mapping in QuerySet(
                                ResourceMapping
                            ).filter(
        ...
        def delete(self):
            ...
            QuerySet(ResourceMapping).filter(
                Q(target_id=self.data.get("knowledge_id"))
        ...
        def merge_problem(paragraph_list: List[Dict], problem_mapping_list: List[Dict]):
            ...
            for paragraph in paragraph_list:
                ...
                if d is None:
                    document_dict[paragraph.get("document_name")] = {
    ...
    class Create(serializers.Serializer):
        ...
        def save_base(self, instance, with_valid=True):
            ...
            # 插入文档
            for document in (
                instance.get("documents") if "documents" in instance else []
            ):
                ...
                document_model_list.append(
                    document_paragraph_dict_model.get("document")
                )
            ...
            return {
                **KnowledgeModelSerializer(knowledge).data,
                ...
                "char_length": reduce(
                    lambda x, y: x + y, [d.char_length for d in document_model_list], 0
                ),
            }, knowledge_id
        ...
        def save_web(self, instance: Dict, with_valid=True):
            ...
            sync_web_knowledge.delay(
                str(knowledge_id), instance.get("source_url"), instance.get("selector")
            )
            update_resource_mapping_by_knowledge(str(knowledge_id))
    ...
    class SyncWeb(serializers.Serializer):
        ...
        sync_type = serializers.CharField(
            ...
            validators=[
                validators.RegexValidator(
                    ...
                    message=_(
                        "The synchronization type only supports:replace|complete"
                    ),
                    code=500,
                )
            ],
            ...
            def handler(child_link: ChildLink, response: Fork.Response):
                if response.status == 200:
                    try:
                        ...
                        chatflow_logger.info(child_link.url.strip())
                        first = (
                            QuerySet(Document)
                            .filter(
        ...
        def replace_sync(self, knowledge):
            ...
            selector = (
                knowledge.meta.get("selector") if "selector" in knowledge.meta else None
            )
        ...
        def hit_test(self):
            ...
            exclude_document_id_list = [
                ...
                for document in QuerySet(Document).filter(
                    knowledge_id=self.data.get("knowledge_id"), is_active=False
                )
            ]
            ...
            hit_dict = reduce(
                lambda x, y: {**x, **y},
                [{hit.get("paragraph_id"): hit} for hit in hit_list],
                {},
            )
            ...
            return [
                {
                    **p,
                    ...
                    "comprehensive_score": hit_dict.get(p.get("id")).get(
                        "comprehensive_score"
                    ),
                }
                for p in p_list
