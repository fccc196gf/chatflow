...
class KnowledgeWorkflowVersionSerializer(serializers.Serializer):
    ...
    class Query(serializers.Serializer):
        ...
        def get_query_set(self, query):
            query_set = QuerySet(KnowledgeWorkflowVersion).filter(
        ...
        def list(self, query, with_valid=True):
            if with_valid:
                ...
                KnowledgeWorkflowVersionQuerySerializer(data=query).is_valid(
    ...
    class Operate(serializers.Serializer):
        ...
        knowledge_id = serializers.UUIDField(required=True, label=_("Knowledge ID"))
        knowledge_version_id = serializers.UUIDField(
            required=True, label=_("Knowledge version ID")
        )
        ...
        def edit(self, instance: Dict, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                KnowledgeWorkflowVersionEditSerializer(data=instance).is_valid(
                    raise_exception=True
                )
            knowledge_version = (
                QuerySet(KnowledgeWorkflowVersion)
