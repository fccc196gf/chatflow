...
class ProblemSerializers(serializers.Serializer):
    class BatchOperate(serializers.Serializer):
        ...
        def delete(self, problem_id_list: List, with_valid=True):
            ...
            problem_paragraph_mapping_list = QuerySet(ProblemParagraphMapping).filter(
                knowledge_id=knowledge_id, problem_id__in=problem_id_list
            )
    ...
    class Operate(serializers.Serializer):
        ...
        def list_paragraph(self, with_valid=True):
            ...
            return native_search(
                ...
                select_string=get_file_content(
                    os.path.join(
                        PROJECT_DIR, "apps", "knowledge", "sql", "list_paragraph.sql"
                    )
                ),
            )
        ...
        def edit(self, instance: Dict, with_valid=True):
            ...
            problem = (
                QuerySet(Problem)
                .filter(id=problem_id, knowledge_id=knowledge_id)
                .first()
            )
    ...
    class Create(serializers.Serializer):
        ...
        def batch(self, problem_list, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                ProblemBatchSerializer(data={"problem_list": problem_list}).is_valid(
            ...
            exists_problem_content_list = [
                ...
                for problem in QuerySet(Problem).filter(
                    knowledge_id=knowledge_id, content__in=problem_list
                )
            ]
            ...
            (
                QuerySet(Problem).bulk_create(problem_instance_list)
                if len(problem_instance_list) > 0
                else None
            )
            return [
    ...
    class Query(serializers.Serializer):
        ...
        def list(self):
            ...
            return native_search(
                ...
                select_string=get_file_content(
                    os.path.join(
                        PROJECT_DIR, "apps", "knowledge", "sql", "list_problem.sql"
                    )
                ),
            )
...
def is_exits(exits_problem_paragraph_mapping_list, new_paragraph_mapping):
    filter_list = [
        ...
        and str(exits_problem_paragraph_mapping.knowledge_id)
        == new_paragraph_mapping.knowledge_id
    ]
    return len(filter_list) > 0


def to_problem_paragraph_mapping(
    problem, document_id: str, paragraph_id: str, knowledge_id: str
):
    return (
        ProblemParagraphMapping(
            id=uuid.uuid7(),
