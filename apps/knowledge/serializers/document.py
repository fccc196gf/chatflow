...
from chatflow.const import PROJECT_DIR
...
class BatchCancelInstanceSerializer(serializers.Serializer):
    id_list = serializers.ListField(
        required=True, child=serializers.UUIDField(required=True), label=_("id list")
    )
...
class DocumentInstanceSerializer(serializers.Serializer):
    ...
    source_file_id = serializers.UUIDField(
        required=False, allow_null=True, label=_("source file id")
    )
...
class DocumentEditInstanceSerializer(serializers.Serializer):
    ...
    hit_handling_method = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                regex=re.compile("^optimization|directly_return$"),
    ...
    def is_valid(self, *, document: Document = None):
        ...
        if (
            "meta" in self.data
            and self.data.get("meta") is not None
            and self.data.get("meta") != {}
        ):
...
class DocumentWebInstanceSerializer(serializers.Serializer):
    ...
    selector = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("selector")
    )
...
class BatchEditHitHandlingSerializer(serializers.Serializer):
    ...
    hit_handling_method = serializers.CharField(
        required=True, label=_("hit handling method")
    )
    directly_return_similarity = serializers.FloatField(
        required=False, max_value=2, min_value=0, label=_("directly return similarity")
    )
    ...
    def is_valid(self, *, raise_exception=False):
        super().is_valid(raise_exception=True)
        if self.data.get("hit_handling_method") not in [
            "optimization",
            "directly_return",
        ]:
            raise AppApiException(
                500, _("The type only supports optimization|directly_return")
            )
...
class DocumentSerializers(serializers.Serializer):
            ...
            if self.data.get("type") == "csv":
                ...
                file = open(
                    os.path.join(
                        ...
                        "knowledge",
                        "template",
                        f"excel_template_{to_locale(language)}.xlsx",
                    ),
                    "rb",
                )
                content = file.read()
    ...
    class Migrate(serializers.Serializer):
        ...
        target_knowledge_id = serializers.UUIDField(
            required=True, label=_("target knowledge id")
        )
        document_id_list = serializers.ListField(
            required=True,
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            query_set = QuerySet(Knowledge).filter(
                id=self.data.get("target_knowledge_id")
            )
            if workspace_id:
                query_set = query_set.filter(workspace_id=workspace_id)
            if not query_set.exists():
        ...
        def migrate(self, with_valid=True):
            ...
            target_problem_list = list(
                QuerySet(Problem).filter(
                    content__in=[problem.content for problem in problem_list],
            ...
            create_problem_list = [
                problem
                for problem, is_create in target_handle_problem_list
                if is_create is not None and is_create
            ]
            # 插入问题
            ...
            # 修改文档
            if (
                knowledge.type == KnowledgeType.BASE.value
                and target_knowledge.type == KnowledgeType.WEB.value
            ):
                ...
                target_knowledge.type == KnowledgeType.BASE.value
                and knowledge.type == KnowledgeType.WEB.value
            ):
                document_list.update(
                    knowledge_id=target_knowledge_id, type=KnowledgeType.BASE, meta={}
                )
            else:
        ...
        def get_target_knowledge_problem(
            ...
            if len(source_problem_list) > 0:
                ...
                problem_list = [
                    ...
                    if problem.content == problem_content
                ]
    ...
    class Query(serializers.Serializer):
        ...
        is_active = serializers.BooleanField(
            required=False, label=_("document is active"), allow_null=True
        )
        task_type = serializers.IntegerField(required=False, label=_("task type"))
        status = serializers.CharField(
            required=False, label=_("status"), allow_null=True, allow_blank=True
        )
        order_by = serializers.CharField(
            required=False, label=_("order by"), allow_null=True, allow_blank=True
        )
        tag = serializers.CharField(
            required=False, label=_("tag"), allow_null=True, allow_blank=True
        )
        ...
        def get_query_set(self):
            ...
            if "name" in self.data and self.data.get("name") is not None:
                query_set = query_set.filter(
                    **{"name__icontains": self.data.get("name")}
                )
            if "hit_handling_method" in self.data and self.data.get(
                "hit_handling_method"
            ) not in [None, ""]:
                query_set = query_set.filter(
                ...
                if task_type is not None:
                    query_set = (
                        query_set.annotate(
                            ...
                            task_type_status=Substr(
                                "reversed_status", TaskType(task_type).value, 1
                            ),
                        )
                        .filter(task_type_status=State(status).value)
                        .values("id")
                    )
                else:
            ...
            if "tag" in self.data and self.data.get("tag") not in [None, ""]:
                ...
                document_id_list = (
                    ...
                    .filter(
                        Q(tag__key__icontains=tag_name)
                        | Q(tag__value__icontains=tag_name)
                    )
        ...
        def list(self):
            ...
            return native_search(
                ...
                select_string=get_file_content(
                    os.path.join(
                        PROJECT_DIR, "apps", "knowledge", "sql", "list_document.sql"
                    )
                ),
            )
    ...
    class Sync(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if first.type != KnowledgeType.WEB:
                raise AppApiException(
                    500, _("Synchronization is only supported for web site types")
                )
        ...
        def sync(self, with_valid=True, with_embedding=True):
            ...
            try:
                ...
                selector_list = (
                    ...
                    if "selector" in document.meta
                    and document.meta.get("selector") is not None
                    else []
                )
                ...
                if result.status == 200:
                    ...
                    # 删除问题
                    QuerySet(model=ProblemParagraphMapping).filter(
                    ...
                    document_paragraph_model = (
                        DocumentSerializers.Create.get_paragraph_model(
                            document, paragraphs
                        )
                    )

                    paragraph_model_list = document_paragraph_model.get(
                        "paragraph_model_list"
                    )
                    problem_paragraph_object_list = document_paragraph_model.get(
                    ...
                    if len(paragraph_model_list) > 0:
                        max_position = (
                            Paragraph.objects.filter(document_id=document_id).aggregate(
                    ...
                    # 向量化
                    if with_embedding:
                        embedding_model_id = get_embedding_model_id_by_knowledge_id(
                ...
                chatflow_logger.error(f"{str(e)}:{traceback.format_exc()}")
                state = State.FAILURE
            ListenerManagement.update_status(
                QuerySet(Document).filter(id=document_id), TaskType.SYNC, state
            )
            ListenerManagement.update_status(
                QuerySet(Paragraph).filter(document_id=document_id),
                TaskType.SYNC,
    ...
    class Operate(serializers.Serializer):
        workspace_id = serializers.CharField(
            required=False, label=_("workspace id"), allow_blank=True
        )
        document_id = serializers.UUIDField(required=True, label=_("document id"))
        knowledge_id = serializers.UUIDField(required=True, label=_("knowledge id"))
        ...
        def export(self, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            document = (
                QuerySet(Document).filter(id=self.data.get("document_id")).first()
            )
            paragraph_list = native_search(
                QuerySet(Paragraph).filter(document_id=self.data.get("document_id")),
                get_file_content(
                    os.path.join(
                        PROJECT_DIR,
                        "apps",
            ...
            data_dict, document_dict = self.merge_problem(
        ...
        def export_zip(self, with_valid=True):
            ...
            res = [
                parse_image(paragraph.get("content")) for paragraph in paragraph_list
            ]

            workbook = DocumentSerializers.Operate.get_workbook(
                data_dict, document_dict
            )
            response = HttpResponse(content_type="application/zip")
            response["Content-Disposition"] = (
                f'attachment; filename="{document.name.strip()}.zip"'
            )
        ...
        def download_source_file(self):
            ...
            if not file:
                raise AppApiException(
                    500,
                    _("File not exist. Only manually uploaded documents are supported"),
                )
        ...
        def edit(self, instance: Dict, with_valid=False):
            ...
            if with_valid:
                DocumentEditInstanceSerializer(data=instance).is_valid(
                    document=_document
                )
            update_keys = [
                "name",
        ...
        def cancel(self, instance, with_valid=True):
            ...
            ListenerManagement.update_status(
                ...
                .annotate(
                    reversed_status=Reverse("status"),
                    task_type_status=Substr(
                        "reversed_status", TaskType(instance.get("type")).value, 1
                    ),
                )
        ...
        def delete(self):
            ...
            QuerySet(File).filter(
                source_id=document_id, source_type=FileSourceType.DOCUMENT
            ).delete()
            paragraph_ids = (
                QuerySet(model=Paragraph)
                .filter(document_id=document_id)
        ...
        def refresh(self, state_list=None, with_valid=True):
            ...
            if with_valid:
                self.is_valid(raise_exception=True)
            knowledge = (
                QuerySet(Knowledge).filter(id=self.data.get("knowledge_id")).first()
            )
            ...
            ListenerManagement.update_status(
                ...
                .annotate(
                    reversed_status=Reverse("status"),
                    task_type_status=Substr(
                        "reversed_status", TaskType.EMBEDDING.value, 1
                    ),
                )
            ...
            try:
                ...
                raise AppApiException(
                    500,
                    _("The task is being executed, please do not send it repeatedly."),
                )
        ...
        @staticmethod
        def merge_problem(
            paragraph_list: List[Dict], problem_mapping_list: List[Dict], document_list
        ):
            result = {}
            document_dict = {}
            ...
            for paragraph in paragraph_list:
                ...
                document_name = DocumentSerializers.Operate.reset_document_name(
                    paragraph.get("document_name")
                )
            ...
            for d_name in document_dict:
                for index, d_id in enumerate(document_dict.get(d_name)):
                    result_document_dict[d_id] = (
                        d_name if index == 0 else d_name + str(index)
                    )
    ...
    class Create(serializers.Serializer):
        workspace_id = serializers.CharField(
            required=False, label=_("workspace id"), allow_null=True
        )
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if (
                ...
                .filter(id=self.data.get("knowledge_id"))
                .exists()
            ):
        ...
        def save(self, instance: Dict, with_valid=True, **kwargs):
            if with_valid:
                ...
                self.is_valid(raise_exception=True)
            knowledge_id = self.data.get("knowledge_id")
            document_paragraph_model = self.get_document_paragraph_model(
                knowledge_id, instance
            )
            ...
            if len(paragraph_model_list) > 0:
                max_position = (
                    Paragraph.objects.filter(document_id=document_model.id).aggregate(
                        max_position=Max("position")
                    )["max_position"]
                    or 0
                )
        ...
        def get_paragraph_model(document_model, paragraph_list: List):
            ...
            paragraph_model_dict_list = [
                ParagraphSerializers.Create(
                    ...
                    knowledge_id, document_model.id, paragraph
                )
                for paragraph in paragraph_list
            ]
        ...
        def get_document_paragraph_model(knowledge_id, instance: Dict):
            source_meta = (
                {"source_file_id": instance.get("source_file_id")}
                if instance.get("source_file_id")
                else {}
            )
            meta = (
                {**instance.get("meta"), **source_meta}
                if instance.get("meta") is not None
                else source_meta
            )
            ...
            document_model = Document(
                **{
                    ...
                    "type": (
                        instance.get("type")
                        if instance.get("type") is not None
                        else KnowledgeType.BASE
                    ),
                }
            )
        ...
        def save_web(self, instance: Dict, with_valid=True):
            if with_valid:
                DocumentWebInstanceSerializer(data=instance).is_valid(
        ...
        def save_qa(self, instance: Dict, with_valid=True):
            if with_valid:
                DocumentInstanceQASerializer(data=instance).is_valid(
                    raise_exception=True
                )
                self.is_valid(raise_exception=True)
        ...
        def save_table(self, instance: Dict, with_valid=True):
            if with_valid:
                DocumentInstanceTableSerializer(data=instance).is_valid(
                    raise_exception=True
                )
                self.is_valid(raise_exception=True)
            file_list = instance.get("file_list")
            document_list = flat_map(
                [self.parse_table_file(file) for file in file_list]
            )
            return DocumentSerializers.Batch(
                data={
                    "knowledge_id": self.data.get("knowledge_id"),
        ...
        def parse_qa_file(self, file):
            ...
            for parse_qa_handle in parse_qa_handle_list:
                if parse_qa_handle.support(file, get_buffer):
                    documents = parse_qa_handle.handle(
                        file, get_buffer, self.save_image
                    )
        ...
        def parse_table_file(self, file):
            ...
            for parse_table_handle in parse_table_handle_list:
                if parse_table_handle.support(file, get_buffer):
                    documents = parse_table_handle.handle(
                        file, get_buffer, self.save_image
                    )
                    for doc in documents:
                        doc["source_file_id"] = source_file_id
        ...
        def save_image(self, image_list):
            if image_list is not None and len(image_list) > 0:
                exist_image_list = [
                    str(i.get("id"))
                    for i in QuerySet(File)
                    .filter(id__in=[i.id for i in image_list])
                    .values("id")
                ]
                save_image_list = [
                    image
                    for image in image_list
                    if not exist_image_list.__contains__(str(image.id))
                ]
                save_image_list = list(
                    {img.id: img for img in save_image_list}.values()
                )
                # save image
    ...
    class Split(serializers.Serializer):
        ...
        def file_to_paragraph(
            self, file, pattern_list: List, with_filter: bool, limit: int
        ):
            # 保存源文件
            file_id = uuid.uuid7()
            raw_file = File(
    ...
    class Batch(serializers.Serializer):
        ...
        def batch_save(self, instance_list: List[Dict], with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
            DocumentInstanceSerializer(many=True, data=instance_list).is_valid(
                raise_exception=True
            )
            workspace_id = self.data.get("workspace_id")
            knowledge_id = self.data.get("knowledge_id")
            ...
            for document in instance_list:
                document_paragraph_dict_model = (
                    DocumentSerializers.Create.get_document_paragraph_model(
                ...
                for paragraph in document_paragraph_dict_model.get(
                    "paragraph_model_list"
                ):
                    paragraph_model_list.append(paragraph)
                for problem_paragraph_object in document_paragraph_dict_model.get(
                    "problem_paragraph_object_list"
                ):
            ...
            if len(paragraph_model_list) > 0:
                for document in document_model_list:
                    max_position = (
                        Paragraph.objects.filter(document_id=document.id).aggregate(
                            max_position=Max("position")
                        )["max_position"]
                        or 0
                    )
                    sub_list = [
                        p for p in paragraph_model_list if p.document_id == document.id
                    ]
                    for i, paragraph in enumerate(sub_list):
                        paragraph.position = max_position + i + 1
                    QuerySet(Paragraph).bulk_create(
                        sub_list if len(sub_list) > 0 else []
                    )
            # 批量插入问题
            bulk_create_in_batches(Problem, problem_model_list, batch_size=1000)
            # 批量插入关联问题
            bulk_create_in_batches(
                ProblemParagraphMapping, problem_paragraph_mapping_list, batch_size=1000
            )
            # 查询文档
            query_set = QuerySet(model=Document)
            if len(document_model_list) == 0:
            ...
            query_set = query_set.filter(
                **{"id__in": [d.id for d in document_model_list]}
            )
            return (
                native_search(
                    {
                        "document_custom_sql": query_set,
                        "order_by_query": QuerySet(Document).order_by(
                            "-create_time", "id"
                        ),
                    },
                    select_string=get_file_content(
        ...
        def batch_sync(self, instance: Dict, with_valid=True):
            if with_valid:
                BatchSerializer(data=instance).is_valid(
                    model=Document, raise_exception=True
                )
                self.is_valid(raise_exception=True)
            # 异步同步
        ...
        def batch_delete(self, instance: Dict, with_valid=True):
            ...
            source_file_ids = [
                doc["meta"].get("source_file_id")
                for doc in Document.objects.filter(id__in=document_id_list).values(
                    "meta"
                )
            ]
            QuerySet(File).filter(id__in=source_file_ids).delete()
        ...
        def batch_cancel(self, instance: Dict, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                BatchCancelInstanceSerializer(data=instance).is_valid(
                    raise_exception=True
                )
            document_id_list = instance.get("id_list")
            ListenerManagement.update_status(
        ...
        def batch_edit_hit_handling(self, instance: Dict, with_valid=True):
            if with_valid:
                ...
                if (
                    ...
                    and hit_handling_method != "directly_return"
                ):
                    raise AppApiException(
                        500,
                        _(
                            "The hit processing method must be directly_return|optimization"
                        ),
                    )
                self.is_valid(raise_exception=True)
            document_id_list = instance.get("id_list")
        ...
        def batch_add_tag(self, instance: Dict, with_valid=True):
            ...
            existing_relations = {
                ...
                .filter(document_id__in=document_id_list, tag_id__in=tag_id_list)
        ...
        def batch_generate_related(self, instance: Dict, with_valid=True):
            ...
            ListenerManagement.update_status(
                ...
                .annotate(
                    reversed_status=Reverse("status"),
                    task_type_status=Substr(
                        "reversed_status", TaskType.GENERATE_PROBLEM.value, 1
                    ),
                )
                .filter(
    ...
    class Tags(serializers.Serializer):
        ...
        document_id = serializers.UUIDField(required=True, label=_("document id"))
        name = serializers.CharField(
            required=False, allow_null=True, allow_blank=True, label=_("search value")
        )
        ...
        def list(self):
            ...
            tag_ids = (
                QuerySet(DocumentTag)
                .filter(document_id=self.data.get("document_id"))
            ...
            if self.data.get("name"):
                tag_ids = (
                    ...
                    .filter(
                        Q(key__icontains=self.data.get("name"))
                        | Q(value__icontains=self.data.get("name"))
                    )
                    .values_list("id", flat=True)
                )
    ...
    class AddTags(serializers.Serializer):
        ...
        tag_ids = serializers.ListField(
            required=True,
            label=_("tag ids"),
            child=serializers.UUIDField(required=True, label=_("tag id")),
        )
        ...
        def add_tags(self):
            ...
            existing_tag_ids = set(
                str(tag_id)
                for tag_id in QuerySet(DocumentTag)
                .filter(document_id=document_id, tag_id__in=tag_ids)
                .values_list("tag_id", flat=True)
            )
            new_tags = [
                DocumentTag(id=uuid.uuid7(), document_id=document_id, tag_id=tag_id)
    ...
    class ReplaceSourceFile(serializers.Serializer):
        ...
        def replace(self):
            ...
            source_file = (
                QuerySet(File).filter(source_id=self.data.get("document_id")).first()
            )
