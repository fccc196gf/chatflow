...
class KnowledgeWorkflowActionRequestSerializer(serializers.Serializer):
    ...
    knowledge_base = serializers.DictField(
        required=True, label=_("knowledge base data")
    )
...
class KnowledgeWorkflowActionListQuerySerializer(serializers.Serializer):
    user_name = serializers.CharField(
        required=False, label=_("Name"), allow_blank=True, allow_null=True
    )
    state = serializers.CharField(
        required=False, label=_("State"), allow_blank=True, allow_null=True
    )
...
class KnowledgeWorkflowActionSerializer(serializers.Serializer):
    ...
    def get_query_set(self, instance: Dict):
        ...
        if instance.get("user_name"):
            query_set = query_set.filter(
                meta__user_name__icontains=instance.get("user_name")
            )
    ...
    def list(self, instance: Dict, is_valid=True):
        if is_valid:
            ...
            KnowledgeWorkflowActionListQuerySerializer(data=instance).is_valid(
    ...
    def action(self, instance: Dict, user, with_valid=True):
        ...
        work_flow_manage = KnowledgeWorkflowManage(
            ...
            is_the_task_interrupted=lambda: cache.get(
                Cache_Version.KNOWLEDGE_WORKFLOW_INTERRUPTED.get_key(
                    action_id=knowledge_action_id
                ),
    ...
    def upload_document(self, instance: Dict, user, with_valid=True):
        ...
        if not knowledge_workflow.is_publish:
            raise AppApiException(
                500, _("The knowledge base workflow has not been published")
            )
        ...
        work_flow_manage = KnowledgeWorkflowManage(
            Workflow.new_instance(
                knowledge_workflow_version.work_flow, WorkflowMode.KNOWLEDGE
            ),
            {
                "knowledge_id": self.data.get("knowledge_id"),
                "knowledge_action_id": knowledge_action_id,
    ...
    class Operate(serializers.Serializer):
        ...
        def one(self, is_valid=True):
            ...
            knowledge_action = (
                QuerySet(KnowledgeAction).filter(id=knowledge_action_id).first()
            )
            return {
                "id": knowledge_action_id,
        ...
        def cancel(self, is_valid=True):
            ...
            QuerySet(KnowledgeAction).filter(
                id=knowledge_action_id, state__in=[State.STARTED, State.PENDING]
...
class KnowledgeWorkflowSerializer(serializers.Serializer):
    class Datasource(serializers.Serializer):
        ...
        def action(self):
            ...
            if self.data.get("type") == "local":
                ...
                return node.__getattribute__(node, self.data.get("function_name"))(
                ...
                return tool_executor.exec_code(
                    tool.code,
                    {**init_params, **self.data.get("params")},
    ...
    class Create(serializers.Serializer):
        ...
        def save_workflow(self, instance: Dict):
            ...
            save_workflow_mapping(
                instance.get("work_flow", {}), ResourceType.KNOWLEDGE, str(knowledge_id)
            )
            ...
            if instance.get("work_flow_template") is not None:
                ...
                KnowledgeWorkflowSerializer.Import(
                    ...
                    {"file": bytes_to_uploaded_file(res.content, "file.kbwf")},
                    is_import_tool=True,
                )

                try:
                    requests.get(
                        template_instance.get("downloadCallbackUrl"), timeout=5
                    )
    ...
    class Import(serializers.Serializer):
        ...
        def import_(self, instance: dict, is_import_tool, with_valid=True):
            if with_valid:
                self.is_valid()
                KnowledgeWorkflowImportRequest(data=instance).is_valid(
                    raise_exception=True
                )
            user_id = self.data.get("user_id")
            workspace_id = self.data.get("workspace_id")
            ...
            tool_model_list = [
                self.to_tool(tool, workspace_id, user_id) for tool in tool_list
            ]
            KnowledgeWorkflow.objects.filter(
        ...
        def to_tool(tool, workspace_id, user_id):
            return Tool(
                ...
                scope=(
                    ToolScope.SHARED if workspace_id == "None" else ToolScope.WORKSPACE
                ),
    ...
    class Export(serializers.Serializer):
        ...
        def export(self, with_valid=True):
            try:
                ...
                knowledge_workflow_dict = KnowledgeWorkflowModelSerializer(
                    knowledge_workflow
                ).data
                ...
                response = HttpResponse(
                    content_type="text/plain", content=knowledge_workflow_pickle
                )
                response["Content-Disposition"] = (
                    f'attachment; filename="{knowledge.name}.kbwf"'
                )
                return response
            except Exception as e:
                return result.error(
