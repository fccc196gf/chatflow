...
class PGVector(BaseVectorStore):
    ...
    def delete_by_source_ids(self, source_ids: List[str], source_type: str):
        if len(source_ids) == 0:
            return
        QuerySet(Embedding).filter(
            source_id__in=source_ids, source_type=source_type
        ).delete()
        ...
        embedding_list = [
            Embedding(
                ...
                search_vector=SearchVector(
                    Value(to_ts_vector(text_list[index]["text"]))
                ),
            )
    ...
    def query(
        ...
        if knowledge_id_list is None or len(knowledge_id_list) == 0:
            return []
        query_set = QuerySet(Embedding).filter(
            knowledge_id__in=knowledge_id_list, is_active=is_active
        )
    ...
    def handle(
        ...
        exec_sql, exec_params = generate_sql_by_query_dict(
            ...
            select_string=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "knowledge", "sql", "embedding_search.sql"
                )
            ),
...
class KeywordsSearch(ISearch):
    def handle(
        ...
        exec_sql, exec_params = generate_sql_by_query_dict(
            ...
            select_string=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "knowledge", "sql", "keywords_search.sql"
                )
            ),
            with_table_name=True,
        )
        embedding_model = select_list(
            exec_sql, [to_query(query_text), *exec_params, similarity, top_number]
        )
        return embedding_model
...
class BlendSearch(ISearch):
    def handle(
        ...
        exec_sql, exec_params = generate_sql_by_query_dict(
            ...
            select_string=get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "knowledge", "sql", "blend_search.sql"
                )
            ),
            with_table_name=True,
        )
