...
class Status:
    ...
    def __init__(self, status: str = None):
        ...
        for _type in self.type_cls:
            index = _type.value - 1
            _state = self.state_cls(
                status_list[index] if len(status_list) > index else "n"
            )
    ...
    def __str__(self):
        ...
        for _type in sorted(self.type_cls, key=lambda item: item.value, reverse=True):
            result.insert(
                len(self.type_cls) - _type.value, self.task_status[_type].value
            )
        return "".join(result)
...
class KnowledgeFolder(MPTTModel, AppModelMixin):
    id = models.CharField(
        primary_key=True, max_length=64, editable=False, verbose_name="主键id"
    )
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, db_constraint=False, blank=True, null=True
    )
    workspace_id = models.CharField(
        max_length=64, verbose_name="工作空间id", default="default", db_index=True
    )
...
class KnowledgeWorkflow(AppModelMixin):
    ...
    is_publish = models.BooleanField(
        verbose_name="是否发布", default=False, db_index=True
    )
...
class KnowledgeWorkflowVersion(AppModelMixin):
    ...
    knowledge = models.ForeignKey(
        Knowledge, on_delete=models.CASCADE, verbose_name="知识库", db_constraint=False
    )
    workspace_id = models.CharField(
        max_length=64, verbose_name="工作空间id", default="default", db_index=True
    )
    ...
    work_flow = models.JSONField(verbose_name="工作流数据", default=dict)
    publish_user_id = models.UUIDField(
        verbose_name="发布者id", max_length=128, default=None, null=True
    )
    publish_user_name = models.CharField(
        verbose_name="发布者名称", max_length=128, default=""
    )
...
class Document(AppModelMixin):
    ...
    knowledge = models.ForeignKey(
        Knowledge, on_delete=models.DO_NOTHING, verbose_name="知识库id"
    )
    ...
    status = models.CharField(
        verbose_name="状态", max_length=20, default=get_default_status, db_index=True
    )
    status_meta = models.JSONField(
        verbose_name="状态统计数据", default=default_status_meta
    )
    ...
    directly_return_similarity = models.FloatField(
        verbose_name="直接回答相似度", default=0.9
    )
...
class DocumentTag(AppModelMixin):
    ...
    document = models.ForeignKey(
        Document, on_delete=models.DO_NOTHING, verbose_name="文档", db_constraint=False
    )
    tag = models.ForeignKey(
        Tag, on_delete=models.DO_NOTHING, verbose_name="标签", db_constraint=False
    )
...
class Paragraph(AppModelMixin):
    ...
    document = models.ForeignKey(
        Document, on_delete=models.DO_NOTHING, db_constraint=False
    )
    ...
    title = models.CharField(
        max_length=256, verbose_name="标题", default="", db_index=True
    )
    status = models.CharField(
        verbose_name="状态", max_length=20, default=get_default_status, db_index=True
    )
...
class Problem(AppModelMixin):
    ...
    knowledge = models.ForeignKey(
        Knowledge, on_delete=models.DO_NOTHING, db_constraint=False
    )
...
class ProblemParagraphMapping(AppModelMixin):
    ...
    problem = models.ForeignKey(
        Problem, on_delete=models.DO_NOTHING, db_constraint=False
    )
    paragraph = models.ForeignKey(
        Paragraph, on_delete=models.DO_NOTHING, db_constraint=False
    )
                ...
                result = select_one(
                    ...
                    [
                        ...
                        (
                            ...
                            if end and (end - offset) < chunk_size
                            else chunk_size
                        ),
                    ],
                )
