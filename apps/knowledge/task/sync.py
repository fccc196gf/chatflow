...
@celery_app.task(
    base=QueueOnce, once={"keys": ["knowledge_id"]}, name="celery:sync_web_knowledge"
)
def sync_web_knowledge(knowledge_id: str, url: str, selector: str):
    ...
    try:
        chatflow_logger.info(
            _(
                "Start--->Start synchronization web knowledge base:{knowledge_id}"
            ).format(knowledge_id=knowledge_id)
        )
        ForkManage(url, selector.split(" ") if selector is not None else []).fork(
            2, set(), get_save_handler(knowledge_id, selector)
        )

        chatflow_logger.info(
            _("End--->End synchronization web knowledge base:{knowledge_id}").format(
        ...
        chatflow_logger.error(
            _(
                "Synchronize web knowledge base:{knowledge_id} error{error}{traceback}"
            ).format(
                knowledge_id=knowledge_id,
                error=str(e),
...
def sync_replace_web_knowledge(knowledge_id: str, url: str, selector: str):
    ...
    try:
        ...
        ForkManage(url, selector.split(" ") if selector is not None else []).fork(
            2, set(), get_sync_handler(knowledge_id)
        )
        chatflow_logger.info(
            _("End--->End synchronization web knowledge base:{knowledge_id}").format(
                knowledge_id=knowledge_id
...
def sync_web_document(knowledge_id, source_url_list: List[str], selector: str):
    ...
    for source_url in source_url_list:
        try:
            result = Fork(
                base_fork_url=source_url, selector_list=selector.split(" ")
            ).fork()
