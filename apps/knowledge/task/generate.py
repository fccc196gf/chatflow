...
def get_llm_model(model_id, model_params_setting=None):
    ...
    return ModelManage.get_model(
        model_id, lambda _id: get_model(model, **(model_params_setting or {}))
    )
...
def generate_problem_by_paragraph(paragraph, llm_model, prompt):
    try:
        ListenerManagement.update_status(
            QuerySet(Paragraph).filter(id=paragraph.id),
        ...
        res = llm_model.invoke(
            [
                HumanMessage(
                    content=prompt.replace("{data}", paragraph.content).replace(
        ...
        for problem in problems:
            save_problem(
                paragraph.knowledge_id, paragraph.document_id, paragraph.id, problem
            )
        ListenerManagement.update_status(
            QuerySet(Paragraph).filter(id=paragraph.id),
            TaskType.GENERATE_PROBLEM,
...
def get_generate_problem(
    llm_model, prompt, post_apply=lambda: None, is_the_task_interrupted=lambda: False
):
...
def get_is_the_task_interrupted(document_id):
    def is_the_task_interrupted():
        ...
        if (
            document is None
            or Status(document.status)[TaskType.GENERATE_PROBLEM] == State.REVOKE
        ):
            return True
        return False
...
def generate_related_by_knowledge_id(
    knowledge_id, model_id, model_params_setting, prompt, state_list=None
):
    document_list = QuerySet(Document).filter(knowledge_id=knowledge_id)
    for document in document_list:
        try:
            generate_related_by_document_id.delay(
                document.id, model_id, model_params_setting, prompt, state_list
            )
        except Exception as e:
            pass
...
def generate_related_by_document_id(
    document_id, model_id, model_params_setting, prompt, state_list=None
):
    ...
    try:
        ...
        # 生成问题函数
        generate_problem = get_generate_problem(
            llm_model,
            prompt,
        ...
        chatflow_logger.error(
            f"根据文档生成问题:{document_id}出现错误{str(e)}{traceback.format_exc()}"
        )
        chatflow_logger.error(
            _(
        ...
        ListenerManagement.post_update_document_status(
            document_id, TaskType.GENERATE_PROBLEM
        )
        chatflow_logger.info(
            _("End--->Generate problem: {document_id}").format(document_id=document_id)
        )
...
def generate_related_by_paragraph_id_list(
