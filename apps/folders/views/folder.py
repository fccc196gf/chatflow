...
from folders.api.folder import (
    FolderCreateAPI,
    FolderEditAPI,
    FolderReadAPI,
    FolderTreeReadAPI,
...
from folders.serializers.folder import (
    FolderSerializer,
...
class FolderView(APIView):
    ...
    @has_permissions(
        lambda r, kwargs: Permission(
            group=Group(f"{kwargs.get('source')}_FOLDER"),
    ...
    @has_permissions(
        lambda r, kwargs: Permission(
            group=Group(f"{kwargs.get('source')}_WORKSPACE_USER_RESOURCE_PERMISSION"),
            operate=Operate.READ,
            resource_path=f"/WORKSPACE/{kwargs.get('workspace_id')}",
        ),
        lambda r, kwargs: Permission(
            group=Group(kwargs.get("source")),
            operate=Operate.READ,
            resource_path=f"/WORKSPACE/{kwargs.get('workspace_id')}",
        ),
        RoleConstants.WORKSPACE_MANAGE.get_workspace_role(),
    ...
    class Operate(APIView):
        ...
        @log(
            menu="folder",
            operate="Edit folder",
            get_operation_object=lambda r, k: get_folder_operation_object(
                k.get("folder_id"), k.get("source")
            ),
        )
        def put(self, request: Request, workspace_id: str, source: str, folder_id: str):
            return result.success(
                FolderSerializer.Operate(
                    data={
                        "id": folder_id,
                        "workspace_id": workspace_id,
                        "source": source,
        ...
        def delete(
            self, request: Request, workspace_id: str, source: str, folder_id: str
        ):
            return result.success(
                FolderSerializer.Operate(
                    data={
