...
def has_exact_permission_by_role(
    user_id: str, workspace_id: str, permission_id: str, role_type: str
):
    ...
    if is_x_pack_ee:
        return (
            ...
            .filter(
                Q(role__rolepermission__permission_id=permission_id)
                | Q(role__internal=True),
    ...
    if parent_id != workspace_id:
        ...
        if depth + current_depth > FOLDER_DEPTH:
            raise serializers.ValidationError(
                _("Folder depth cannot exceed 10000 levels")
            )
...
def has_target_permission(workspace_id, source, user_id, target):
    return (
        ...
        .filter(
            workspace_id=workspace_id,
            user_id=user_id,
            auth_target_type=source,
            target=target,
            permission_list__contains=["MANAGE"],
        )
        .exists()
    )
...
class FolderSerializer(serializers.Serializer):
    ...
    desc = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("folder description")
    )
    ...
    class Create(serializers.Serializer):
        ...
        def insert(self, instance, with_valid=True):
            ...
            if (
                QuerySet(Folder)
                .filter(name=name, workspace_id=workspace_id, parent_id=parent_id)
                .exists()
            ):
    ...
    class Operate(serializers.Serializer):
        ...
        workspace_id = serializers.CharField(
            required=True, allow_null=True, allow_blank=True, label=_("workspace id")
        )
        source = serializers.CharField(required=True, label=_("source"))
        user_id = serializers.UUIDField(required=True, label=_("user id"))
        ...
        def edit(self, instance):
            ...
            if (
                ...
                and current_id != current_node.workspace_id
                and current_node.parent_id != parent_id
            ):
                ...
                if has_target_permission(
                    ...
                    parent_id,
                ) or is_workspace_manage(
                    self.data.get("user_id"), current_node.workspace_id
                ):
        ...
        def delete(self):
            ...
            if workspace_manage:
                nodes = Folder.objects.filter(id=self.data.get("id")).get_descendants(
                    include_self=True
                )
                for node in nodes:
                    # print(node)
            ...
            # 普通用户删除的文件夹内全部都得是自己有权限的资源
                ...
                for node in nodes:
                    # 删除相关的资源
                    source_ids = (
                        Source.objects.filter(folder_id=node.id)
                    ...
                    if auth_list != len(source_ids):
                        raise AppApiException(
                            500,
                            _(
                                "This folder contains resources that you dont have permission"
                            ),
                        )
                    self.delete_source(node)
                    node.delete()
        ...
        def delete_source(self, node):
            Source = get_source_type(self.data.get("source"))  # noqa
            source_ids = Source.objects.filter(folder_id=node.id).values_list(
                "id", flat=True
            )
...
class FolderTreeSerializer(serializers.Serializer):
    ...
    def _having_read_permission_by_role(user_id: str, workspace_id: str, source: str):
        ...
        if is_x_pack_ee:
            return (
                ...
                .select_related("role", "user")
                .filter(
                    Q(role__rolepermission__permission_id=f"{source}_FOLDER:READ")
                    | Q(role__internal=True),
                    workspace_id=workspace_id,
                    user_id=user_id,
    ...
    def get_folder_tree(self, current_user, name=None):
        self.is_valid(raise_exception=True)
        ...
        if not workspace_manage:
            having_read_permission_by_role = has_exact_permission_by_role(
                user_id,
                workspace_id,
            ...
            base_q &= Q(
                id__in=WorkspaceUserResourcePermission.objects.filter(
                    user_id=current_user.id,
                    auth_target_type=self.data.get("source"),
                    workspace_id=self.data.get("workspace_id"),
                    permission_list__overlap=permission_condition,
                ).values_list("target", flat=True)
            ) | Q(id=self.data.get("workspace_id"))
        ...
        return (
            [d for d in serializer.data if d.get("id") == d.get("workspace_id")]
            if name is None
