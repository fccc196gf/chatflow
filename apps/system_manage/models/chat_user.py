...
class ChatUser(models.Model):
    ...
    nick_name = models.CharField(
        max_length=150, verbose_name="昵称", unique=True, db_index=True
    )
    username = models.CharField(
        max_length=150, unique=True, verbose_name="用户名", db_index=True
    )
    password = models.CharField(max_length=150, verbose_name="密码")
    source = models.CharField(
        max_length=10, verbose_name="来源", default="LOCAL", db_index=True
    )
    is_active = models.BooleanField(default=True, db_index=True)
    create_time = models.DateTimeField(
        verbose_name="创建时间", auto_now_add=True, null=True, db_index=True
    )
    update_time = models.DateTimeField(
        verbose_name="修改时间", auto_now=True, null=True, db_index=True
    )
...
class UserGroup(models.Model):
    ...
    name = models.CharField(
        max_length=150, verbose_name="名称", unique=True, db_index=True
    )
...
class UserGroupRelation(models.Model):
    ...
    group = models.ForeignKey(
        UserGroup, on_delete=models.CASCADE, verbose_name="用户组"
    )
...
class ResourceChatUserAuthorize(models.Model):
    ...
    resource_type = models.CharField(
        verbose_name="资源类型", choices=ResourceType.choices, db_index=True
    )
