...
def get_initialization_resource_mapping():
    ...
    for app_id in ids:
        try:
            ...
            workflow_mapping = get_workflow_resource(
                application.work_flow,
                get_node_handle_callback(ResourceType.APPLICATION, application.id),
            )
            ...
            if knowledge.type == 4:
                ...
                if knowledge_workflow:
                    workflow_mapping = get_workflow_resource(
                        ...
                        get_node_handle_callback(
                            ResourceType.KNOWLEDGE, str(knowledge_workflow.knowledge_id)
                        ),
                    )
                    resource_mapping_list += workflow_mapping
            instance_mapping = get_instance_resource(
    ...
    application_knowledge_mapping = [
        ResourceMapping(
            ...
            source_id=str(akm.application_id),
            target_id=str(akm.knowledge_id),
        )
        for akm in QuerySet(ApplicationKnowledgeMapping).all()
    ]
            ...
            fields=[
                ...
                (
                    ...
                    models.CharField(
                        ...
                        db_index=True,
                        verbose_name="被关联资源类型",
                    ),
                ),
                (
                    "source_id",
                    models.CharField(
                        db_index=True, max_length=128, verbose_name="关联资源id"
                    ),
                ),
                (
                    "target_id",
                    models.CharField(
                        db_index=True, max_length=128, verbose_name="被关联资源id"
                    ),
                ),
            ],
            options={
