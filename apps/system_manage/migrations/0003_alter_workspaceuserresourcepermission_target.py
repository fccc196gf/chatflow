...
def delete_auth(apps, folder_model):
    workspace_user_resource_permission_model = apps.get_model(
        "system_manage", "WorkspaceUserResourcePermission"
    )
    QuerySet(workspace_user_resource_permission_model).filter(
        target__in=QuerySet(folder_model).values_list("id")
    ).delete()


def get_workspace_user_resource_permission_list(
    apps,
    auth_target_type,
        ...
        [
            [
                workspace_user_resource_permission_model(
                    ...
                    user_id=wurm.user_id,
                    auth_target_type=auth_target_type,
                    auth_type="RESOURCE_PERMISSION_GROUP",
                    permission_list=(
                        ["VIEW", "MANAGE"] if wurm.user_id == f.user_id else ["VIEW"]
                    ),
                )
                for wurm in workspace_user_role_mapping_model_workspace_dict.get(
                    f.workspace_id, []
                )
            ]
            for f in QuerySet(folder_model).all()
        ],
        [],
    )
...
def auth_folder(apps, schema_editor):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        future = executor.submit(
            workspace_user_role_mapping_model_exists, workspace_user_role_mapping_model
        )
    ...
    if workspace_user_role_mapping_model is None:
        workspace_user_role_mapping_model_workspace_dict = {
            "default": [
                WorkspaceUserRoleMapping("default", "", u.id)
                for u in QuerySet(user_model).all()
            ]
        }
    else:
        workspace_user_role_mapping_model_workspace_dict = group_by(
            [
                v
                for v in {
                    str(wurm.user_id) + str(wurm.workspace_id): wurm
                    for wurm in QuerySet(workspace_user_role_mapping_model)
                }.values()
            ],
    ...
    workspace_user_resource_permission_list = (
        get_workspace_user_resource_permission_list(
            apps,
            "APPLICATION",
            workspace_user_role_mapping_model_workspace_dict,
    ...
    QuerySet(workspace_user_resource_permission_model).bulk_create(
        workspace_user_resource_permission_list
    )
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.CharField(
                db_index=True, max_length=128, verbose_name="知识库/应用id"
            ),
        ),
