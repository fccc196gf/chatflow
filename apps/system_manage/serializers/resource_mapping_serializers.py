...
class ResourceMappingSerializer(serializers.Serializer):
    ...
    user_name = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("creator")
    )
    ...
    def get_query_set(self):
        ...
        queryset = queryset.filter(
            target_id=self.data.get("resource_id"),
            target_type=self.data.get("resource"),
        )

        if self.data.get("resource_name"):
            queryset = queryset.filter(
                **{"sdc.name__icontains": self.data.get("resource_name")}
            )
        if self.data.get("user_name"):
            queryset = queryset.filter(
                **{"u.username__icontains": self.data.get("user_name")}
            )
        if self.data.get("source_type"):
            queryset = queryset.filter(
                **{"rm.source_type__in": self.data.get("source_type")}
            )
        if (
            self.data.get("workspace_ids") is not None
            and len(self.data.get("workspace_ids")) > 0
        ):
            ...
            get_file_content(
                os.path.join(
                    ...
                    (
                        ...
                        if is_x_pack_ee
                        else "list_resource_mapping.sql"
                    ),
                )
            ),
    ...
    def get_resource_count(self, result_list):
        ...
        if isinstance(data_to_process, list) and data_to_process:
            ...
            if ids:  # 只有在ids非空时才执行查询
                mapping_counts = (
                    ResourceMapping.objects.filter(target_id__in=ids)
                    .values("target_id")
                ...
                # 构建目标ID到计数的映射
                count_dict = {
                    str(item["target_id"]): item["count"] for item in mapping_counts
                }
