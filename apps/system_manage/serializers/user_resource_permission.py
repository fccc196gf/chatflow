...
from chatflow.settings import edition
...
class UpdateUserResourcePermissionRequest(serializers.Serializer):
    user_resource_permission_list = UpdateTeamMemberItemPermissionSerializer(
        required=True, many=True
    )

    def is_valid(
        self, *, auth_target_type=None, workspace_id=None, raise_exception=False
    ):
        ...
        if illegal_target_id_list is not None and len(illegal_target_id_list) > 0:
            raise AppApiException(
                500, _("Non-existent id") + "[" + str(illegal_target_id_list) + "]"
            )
...
class UserResourcePermissionUserListRequest(serializers.Serializer):
    name = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("resource name")
    )
...
class UserResourcePermissionSerializer(serializers.Serializer):
    ...
    def get_queryset(self, instance):
        ...
        return {
            ...
            "folder_query_set": QuerySet(
                m_map.get(self.data.get("auth_target_type"))
            ).filter(workspace_id=self.data.get("workspace_id")),
            "workspace_user_resource_permission_query_set": QuerySet(
    ...
    def is_auth(self, resource_id: str):
        ...
        if wurp.auth_type == ResourceAuthType.ROLE.value:
            if workspace_user_role_mapping_model and role_permission_mapping_model:
                inner = QuerySet(workspace_user_role_mapping_model).filter(
                    workspace_id=workspace_id, user_id=user_id
                )
                return (
                    QuerySet(role_permission_mapping_model)
                    .filter(
                        role_id__in=inner, permission_id=(auth_target_type + ":READ")
                    )
                    .exists()
                )
            else:
    ...
    def auth_resource_batch(self, resource_id_list: list):
        ...
        workspace_user_resource_permission = [
            WorkspaceUserResourcePermission(
                ...
                auth_target_type=auth_target_type,
                permission_list=(
                    [ResourcePermission.VIEW, ResourcePermission.MANAGE]
                    if auth_type == ResourceAuthType.RESOURCE_PERMISSION_GROUP
                    else [ResourcePermissionRole.ROLE]
                ),
                workspace_id=workspace_id,
                user_id=user_id,
                auth_type=auth_type,
        ...
        QuerySet(WorkspaceUserResourcePermission).bulk_create(
            workspace_user_resource_permission
        )
        # 刷新缓存
    ...
    def list(self, instance, user, with_valid=True):
        if with_valid:
            self.is_valid(raise_exception=True)
            UserResourcePermissionUserListRequest(data=instance).is_valid(
        ...
        user_resource_permission_list = native_search(
            ...
            get_file_content(
                os.path.join(
                    ...
                    "system_manage",
                    "sql",
                    sql_map.get(self.data.get("auth_target_type")),
                )
            ),
        )
    ...
    def edit(self, instance, user, with_valid=True):
        if with_valid:
            ...
            UpdateUserResourcePermissionRequest(
                data={"user_resource_permission_list": instance}
            ).is_valid(
                raise_exception=True,
                auth_target_type=self.data.get("auth_target_type"),
                workspace_id=self.data.get("workspace_id"),
        ...
        for user_resource_permission in instance:
            ...
            exist_list = [
                ...
                if user_resource_permission.get("target_id")
                == str(user_resource_permission_exist.target)
            ]
            if len(exist_list) > 0:
        ...
        # 批量更新
        (
            QuerySet(WorkspaceUserResourcePermission).bulk_update(
...
class ResourceUserPermissionSerializer(serializers.Serializer):
    ...
    auth_target_type = serializers.CharField(required=True, label=_("resource"))
    users_permission = ResourceUserPermissionEditRequest(
        required=False, many=True, label=_("users_permission")
    )
    ...
    def get_queryset(self, instance, is_x_pack_ee: bool):
        ...
        if workspace_user_role_mapping_model:
            user_query_set = user_query_set.filter(
                **{
                    "u.id__in": QuerySet(workspace_user_role_mapping_model)
                    .filter(workspace_id=self.data.get("workspace_id"))
                    .values("user_id")
                }
            )
        if is_x_pack_ee:
            ...
            role_name_and_type_query_set = QuerySet(
                model=get_dynamics_model(
                    {
                        "user_role_relation.workspace_id": models.CharField(),
                        "role_setting.type": models.CharField(),
            ...
            user_query_set = user_query_set.filter(**{"role": "USER"})
            return {
                "workspace_user_resource_permission_query_set": workspace_user_resource_permission_query_set,
                "user_query_set": user_query_set,
            }
    ...
    def list(self, instance, with_valid=True):
        if with_valid:
            self.is_valid(raise_exception=True)
            ResourceUserPermissionUserListRequest(data=instance).is_valid(
                raise_exception=True
            )
            ...
            get_file_content(
                os.path.join(
                    ...
                    (
                        ...
                        if is_x_pack_ee
                        else "get_resource_user_permission_detail.sql"
                    ),
                )
            ),
        )
    ...
    def get_has_manage_permission_resource_under_folders(
        self, current_user_id, folder_ids
    ):
