...
def heartbeat(sender, **kwargs):
    ...
    heartbeat_path = Path(
        "/opt/chatflow-app/tmp/worker_heartbeat_{}".format(worker_name)
    )
...
def worker_ready(sender, **kwargs):
    ...
    ready_path = Path("/opt/chatflow-app/tmp/worker_ready_{}".format(worker_name))
    ready_path.touch()
...
def worker_shutdown(sender, **kwargs):
    ...
    for signal in ["ready", "heartbeat"]:
        path = Path("/opt/chatflow-app/tmp/worker_{}_{}".format(signal, worker_name))
