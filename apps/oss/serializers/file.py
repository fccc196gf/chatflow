...
class FileSerializer(serializers.Serializer):
    ...
    def upload(self, with_valid=True):
        ...
        file = File(
            ...
            or FileSourceType.TEMPORARY_120_MINUTE.value,
            source_type=self.data.get("source_type")
            or FileSourceType.TEMPORARY_120_MINUTE,
        )
    ...
    class Operate(serializers.Serializer):
        ...
        def get(self, with_valid=True):
            ...
            if file_type in audio_types and self.data.get("http_range"):
                response = self.handle_audio(
                    file_size, file_bytes, content_type, encoded_filename
                )
            if response:
                return response
...
def get_url_content(url, application_id: str):
    ...
    if application.file_upload_setting and application.file_upload_setting.get(
        "fileLimit"
    ):
