            ...
            try:
                ...
                if self.current_node_chunk.is_end():
                    ...
                    if self.work_flow.answer_is_not_empty():
                        chunk = (
                            self.work_flow.base_to_response.to_stream_chunk_response(
...
class WorkflowManage:
        ...
        for node in self.flow.nodes:
            ...
            if node_config is not None:
                ...
                if fields is not None:
                    for field in fields:
                        field_list.append(
                            {**field, "node_id": node_id, "node_name": node_name}
                        )
                global_fields = node_config.get("globalFields")
                if global_fields is not None:
                    for global_field in global_fields:
                        global_field_list.append(
                            {**global_field, "node_id": node_id, "node_name": node_name}
                        )
                chat_fields = node_config.get("chatFields")
                if chat_fields is not None:
                    for chat_field in chat_fields:
                        chat_field_list.append(
                            {**chat_field, "node_id": node_id, "node_name": node_name}
                        )
        field_list.sort(
            key=lambda f: len(f.get("node_name") + f.get("value")), reverse=True
        )
        global_field_list.sort(
            key=lambda f: len(f.get("node_name") + f.get("value")), reverse=True
        )
        chat_field_list.sort(
            key=lambda f: len(f.get("node_name") + f.get("value")), reverse=True
        )
    ...
    def load_node(self, chat_record, start_node_id, start_node_data):
        ...
        for node_details in sorted(
            chat_record.details.values(), key=lambda d: d.get("index")
        ):
            ...
            if node_details.get("runtime_node_id") == start_node_id:
                ...
                def get_node_params(n):
                    ...
                    if n.type == "loop-node":
                        is_result = True
                    return {
                        **n.properties.get("node_data"),
                        "form_data": start_node_data,
                        "node_data": start_node_data,
                        "child_node": self.child_node,
                        "is_result": is_result,
                    }

                self.start_node = self.get_node_cls_by_id(
                    node_id,
                    node_details.get("up_node_id_list"),
                    get_node_params=get_node_params,
                )
                self.start_node.valid_args(
                    {**self.start_node.node_params, "form_data": start_node_data},
                    self.start_node.workflow_params,
                )
                if self.start_node.type == "loop-node":
                    ...
                    self.start_node.context["current_index"] = node_details.get(
                        "current_index"
                    )
                    self.start_node.context["current_item"] = node_details.get(
                        "current_item"
                    )
                    self.start_node.context["loop_answer_data"] = node_details.get(
                        "loop_answer_data", {}
                    )
                if self.start_node.type == "application-node":
                    application_node_dict = node_details.get(
                        "application_node_dict", {}
                    )
                    self.start_node.context["application_node_dict"] = (
                        application_node_dict
                    )
    ...
    def run_block(self, language="zh"):
        ...
        try:
            ...
            answer_text = "\n\n".join(
                "\n\n".join([a.get("content") for a in answer])
            ...
            res = self.base_to_response.to_block_response(
                self.params["chat_id"],
    ...
    def await_result(self, is_cleanup=True):
        try:
            ...
            self.work_flow_post_handler.handler(self)
            yield self.base_to_response.to_stream_chunk_response(
    ...
    def run_chain_async(self, current_node, node_result_future, language="zh"):
        future = executor.submit(
            self.run_chain_manage, current_node, node_result_future, language
        )
    ...
    def run_chain_manage(self, current_node, node_result_future, language="zh"):
        ...
        if current_node is None:
            ...
            current_node = get_node(start_node.type, self.flow.workflow_mode)(
                start_node, self.params, self
            )
            ...
            result_list = [
                {
                    "node": node,
                    "future": executor.submit(
                        self.run_chain_manage, node, None, language
                    ),
                }
    ...
    def run_chain(self, current_node, node_result_future=None):
        ...
        try:
            ...
            result = (
                self.hand_event_node_result(current_node, node_result_future)
                if is_stream
                else self.hand_node_result(current_node, node_result_future)
            )
            return result
        except Exception as e:
            chatflow_logger.error(f"Exception: {e}", exc_info=True)
        return None
    ...
    def append_node(self, current_node):
        for index in range(len(self.node_context)):
            ...
            if (
                current_node.id == n.node.id
                and current_node.runtime_node_id == n.runtime_node_id
            ):
    ...
    def hand_event_node_result(self, current_node, node_result_future):
        ...
        try:
            ...
            if current_node.status == 500:
                ...
                r = NodeResult(
                    {"branch_id": "exception", "exception": current_node.err_message},
                    {},
    ...
    def is_result(self, current_node, current_node_result):
        return (
            current_node.node_params.get(
                "is_result", not self._has_next_node(current_node, current_node_result)
            )
            if current_node.node_params is not None
            else False
        )
    ...
    def _has_next_node(self, current_node, node_result: NodeResult | None):
        ...
        for next_edge_node in next_edge_node_list:
            if node_result is not None and node_result.is_assertion_result():
                ...
                if (
                    edge.sourceNodeId == current_node.id
                    and f"{edge.sourceNodeId}_{node_result.node_variable.get('branch_id')}_right"
                    == edge.sourceAnchorId
                ):
                    return True
    ...
    def has_next_node(self, node_result: NodeResult | None):
        ...
        是否有下一个可运行的节点
        """
        return self._has_next_node(
            self.get_start_node() if self.current_node is None else self.current_node,
            node_result,
        )
    ...
    def get_runtime_details(self, get_details=lambda n, index: n.get_details(index)):
        ...
        for index in range(len(self.node_context)):
            ...
            if (
                self.chat_record is not None
                and self.chat_record.details is not None
                and self.start_node
            ):
                details = self.chat_record.details.get(node.runtime_node_id)
                if (
                    details is not None
                    and self.start_node.runtime_node_id != node.runtime_node_id
                ):
    ...
    def get_answer_text_list(self):
        ...
        answer_list = reduce(
            ...
            [
                n.get_answer_list()
                for n in self.node_context
                if n.get_answer_list() is not None
            ],
            [],
        )
        up_node = None
        for index in range(len(answer_list)):
            ...
            if len(current_answer.content) > 0:
                if (
                    up_node is None
                    or current_answer.view_type == "single_view"
                    or (
                        current_answer.view_type == "many_view"
                        and up_node.view_type == "single_view"
                    )
                ):
    ...
    def dependent_node(edge, node):
        ...
        if node.id == up_node_id:
            if node.context.get("branch_id", None):
                if (
                    edge.sourceAnchorId
                    == f"{node.id}_{node.context.get('branch_id', None)}_right"
                ):
                    return True
                else:
                    return False
    ...
    def dependent_node_been_executed(self, node_id):
        ...
        """
        up_edge_list = [
            edge for edge in self.flow.edges if edge.targetNodeId == node_id
        ]
        return all(
            [
                any(
                    [
                        self.dependent_node(edge, node)
    ...
    def get_next_node_list(self, current_node, current_node_result):
        ...
        next_edge_node_list = self.flow.get_next_edge_nodes(current_node.id) or []
        if (
            current_node_result is not None
            and current_node_result.is_assertion_result()
        ):
            for edge_node in next_edge_node_list:
                edge = edge_node.edge
                next_node = edge_node.node
                if (
                    f"{edge.sourceNodeId}_{current_node_result.node_variable.get('branch_id')}_right"
                    == edge.sourceAnchorId
                ):
                    if next_node.properties.get("condition", "AND") == "AND":
                        if self.dependent_node_been_executed(edge.targetNodeId):
                            ...
                            if up_nodes and len(up_nodes) > 1:
                                ...
                                up_node_id_list = [
                                    n_c
                                    for n_c in self.node_context
                                    if n_c.node.id == first.id
                                ][0].up_node_id_list
                                up_node_id_list = [*up_node_id_list, first.id]
                            node_list.append(
                                self.get_node_cls_by_id(
                                    edge.targetNodeId, up_node_id_list
                                )
                            )
                    else:
                        node_list.append(
    ...
    def get_workflow_content(self):
        context = {"global": self.context, "chat": self.chat_context}
    ...
    def reset_prompt(self, prompt: str):
        ...
        for field in self.global_field_list:
            ...
            prompt = prompt.replace(globeLabel, globeValue).replace(
                globeLabelNew, globeValue
            )
    ...
    def get_node_cls_by_id(
        ...
        for node in self.flow.nodes:
            if node.id == node_id:
                node_instance = get_node(node.type, self.flow.workflow_mode)(
                    node, self.params, self, up_node_id_list, get_node_params
                )
