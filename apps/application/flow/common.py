...
class Edge:
    def __init__(
        self, _id: str, _type: str, sourceNodeId: str, targetNodeId: str, **keywords
    ):
        self.id = _id
        self.type = _type
...
class Node:
    def __init__(
        self, _id: str, _type: str, x: int, y: int, properties: dict, **kwargs
    ):
        self.id = _id
        self.type = _type
        self.x = x
...
class Workflow:
    ...
    def __init__(
        ...
        self.up_node_map = {
            key: [
                EdgeNode(edge, self.node_map.get(edge.sourceNodeId)) for edge in edges
            ]
            for key, edges in group_by(
                edges, key=lambda edge: edge.targetNodeId
            ).items()
        }

        self.next_node_map = {
            key: [
                EdgeNode(edge, self.node_map.get(edge.targetNodeId)) for edge in edges
            ]
            for key, edges in group_by(
                edges, key=lambda edge: edge.sourceNodeId
            ).items()
        }
        ...
        if node.type == "condition-node":
            ...
            for branch in branch_list:
                ...
                edge_list = [
                    ...
                    if edge.sourceAnchorId == source_anchor_id
                ]
                if len(edge_list) == 0:
                    raise AppApiException(
                        500,
                        _(
                            "The branch {branch} of the {node} node needs to be connected"
                        ).format(
                            node=node.properties.get("stepName"),
                            branch=branch.get("type"),
                        ),
                    )
            ...
            if len(edge_list) == 0 and not end_nodes.__contains__(node.type):
                raise AppApiException(
                    500,
                    _("{node} Nodes cannot be considered as end nodes").format(
                        node=node.properties.get("stepName")
                    ),
                )
    ...
    def is_valid_model_params(self):
        node_list = [
            ...
            if (
                node.type == "ai-chat-node"
                or node.type == "question-node"
                or node.type == "parameter-extraction-node"
            )
        ]
        for node in node_list:
            model = (
                QuerySet(Model)
                .filter(id=node.properties.get("node_data", {}).get("model_id"))
                .first()
            )
            if model is None:
                raise ValidationError(
                    ErrorDetail(
                        _("The node {node} model does not exist").format(
                            node=node.properties.get("stepName")
                        )
                    )
                )
            ...
            model_params_setting = node.properties.get("node_data", {}).get(
            ...
            if node.properties.get("status", 200) != 200:
                raise ValidationError(
                    ErrorDetail(
                        _("Node {node} is unavailable").format(
                            node.properties.get("stepName")
                        )
                    )
                )
        node_list = [node for node in self.nodes if (node.type == "function-lib-node")]
        for node in node_list:
            function_lib_id = node.properties.get("node_data", {}).get(
                "function_lib_id"
            )
            if function_lib_id is None:
                raise ValidationError(
                    ErrorDetail(
                        _("The library ID of node {node} cannot be empty").format(
                            node=node.properties.get("stepName")
                        )
                    )
                )
            ...
            if f_lib is None:
                raise ValidationError(
                    ErrorDetail(
                        _(
                            "The function library for node {node} is not available"
                        ).format(node=node.properties.get("stepName"))
                    )
                )
    ...
    def is_valid_base_node(self):
        ...
        if len(base_node_list) > 1:
            raise AppApiException(
                500, _("There can only be one basic information node")
            )
