...
from application.flow.step_node.search_document_node.i_search_document_node import (
    ISearchDocumentStepNode,
)
...
class BaseSearchDocumentNode(ISearchDocumentStepNode):
    ...
    def execute(
        self,
        knowledge_id_list: List,
        search_mode: str,
        search_scope_type: str,
        search_scope_source: str,
        search_scope_reference: List,
        question_reference: List,
        search_condition_type: str,
        ...
        **kwargs,
        ...
        if search_scope_type == "custom":  # 手动选择知识库
            document_id_list = (
                QuerySet(Document)
                .filter(knowledge_id__in=knowledge_id_list)
            ...
            if search_scope_source == "document":  # 文档
                ...
                document_id_list = (
                    QuerySet(Document)
                    .filter(
                        knowledge_id__in=self.get_reference_content(
        ...
        # 权限过滤
        get_knowledge_list_of_authorized = DatabaseModelManage.get_model(
            "get_knowledge_list_of_authorized"
        )
        chat_user_type = self.workflow_manage.get_body().get("chat_user_type")

        if (
            get_knowledge_list_of_authorized is not None
            and RoleConstants.CHAT_USER.value.name == chat_user_type
        ):
            # 获取授权的知识库ID列表
            authorized_knowledge_ids = get_knowledge_list_of_authorized(
                self.workflow_manage.get_body().get("chat_user_id"), knowledge_id_list
            )
            ...
            document_id_list = (
                QuerySet(Document)
                .filter(
                    id__in=document_id_list, knowledge_id__in=authorized_knowledge_ids
                )
                .values_list("id", flat=True)
            )

        if search_mode == "auto":  # 通过问题自动检索
            matched_doc_ids = self.handle_auto_tags(
                document_id_list, question_reference
            )
        ...
        final_knowledge_ids = list(
            set(str(doc["knowledge_id"]) for doc in document_items)
        )
        knowledge_items = (
            QuerySet(Knowledge).filter(id__in=final_knowledge_ids).values()
        )
        ...
        if search_condition_type == "AND":
            ...
            for condition in search_condition_list:
                ...
                if compare_type == "not_contain":
                    ...
                    if compare_type == "contain":
                        q_filter = Q(
                            tag__key=tag_key, tag__value__icontains=field_value
                        )
            ...
            for condition in search_condition_list:
                ...
                if compare_type == "not_contain":
                    ...
                    matched_docs = matched_docs.union(
                        set(document_id_list) - exclude_docs
                    )
                else:
                    if compare_type == "contain":
                        q_filter = Q(
