...
class DataSourceLocalNodeParamsSerializer(serializers.Serializer):
    file_type_list = serializers.ListField(
        child=serializers.CharField(label=("")), label=""
    )
    file_size_limit = serializers.IntegerField(
        required=True, label=_("Number of uploaded files")
    )
    file_count_limit = serializers.IntegerField(
        required=True, label=_("Upload file size")
    )
...
class IDataSourceLocalNode(INode):
    ...
    def execute(
        self, file_type_list, file_size_limit, file_count_limit, **kwargs
    ) -> NodeResult:
        pass
