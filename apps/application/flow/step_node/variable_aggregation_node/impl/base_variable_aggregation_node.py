...
from application.flow.step_node.variable_aggregation_node.i_variable_aggregation_node import (
    IVariableAggregation,
)
...
class BaseVariableAggregationNode(IVariableAggregation):
    ...
    def get_first_non_null(self, variable_list):
        ...
        for variable in variable_list:
            v = self.workflow_manage.get_reference_field(
                variable.get("variable")[0], variable.get("variable")[1:]
            )
    ...
    def reset_variable(self, variable):
        ...
        return {
            "value": value,
            "node_name": (
                node.properties.get("stepName") if node is not None else node_id
            ),
    ...
    def reset_group_list(self, group_list):
        ...
        for g in group_list:
            b = {
                "label": g.get("label"),
                "variable_list": [
                    self.reset_variable(variable) for variable in g.get("variable_list")
                ],
            }
            result.append(b)
        return result
    ...
    def execute(self, strategy, group_list, **kwargs) -> NodeResult:
        ...
        result = {
            item.get("field"): strategy_map[strategy](item.get("variable_list"))
        ...
        return NodeResult(
            {
                "result": result,
                "strategy": strategy,
                "group_list": self.reset_group_list(group_list),
                **result,
            },
            {},
        )
