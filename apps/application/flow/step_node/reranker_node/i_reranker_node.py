...
class RerankerSettingSerializer(serializers.Serializer):
    ...
    similarity = serializers.FloatField(
        required=True, max_value=2, min_value=0, label=_("Reference segment number")
    )
...
class RerankerStepNodeSerializer(serializers.Serializer):
    ...
    reranker_reference_list = serializers.ListField(
        required=True, child=serializers.ListField(required=True)
    )
...
class IRerankerNode(INode):
    ...
    def _run(self):
        ...
        reranker_list = [
            self.workflow_manage.get_reference_field(reference[0], reference[1:])
            for reference in self.node_params_serializer.data.get(
