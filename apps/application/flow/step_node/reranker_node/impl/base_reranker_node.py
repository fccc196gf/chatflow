...
def merge_reranker_list(reranker_list, result=None):
    ...
    for document in reranker_list:
        if isinstance(document, list):
            ...
            result.append(
                Document(
                    page_content=str(document) if len(content) == 0 else content,
...
def filter_result(
    document_list: List[Document], max_paragraph_char_number, top_n, similarity
):
    use_len = 0
    result = []
...
def reset_result_list(result_list: List[Document], document_list: List[Document]):
    ...
    for result in result_list:
        filter_result_list = [
            ...
            if document.page_content == result.page_content
        ]
...
def get_none_result(question):
    return NodeResult(
        {"document_list": [], "question": question, "result_list": [], "result": ""}, {}
    )
...
class BaseRerankerNode(IRerankerNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        self.context["document_list"] = [
            {
                ...
                "metadata": reset_metadata(document.metadata),
            }
        ...
        return NodeResult(
            {
                "result_list": r,
                "result": "".join([item.get("page_content") for item in r]),
            },
            {},
        )
    ...
    def get_details(self, index: int, **kwargs):
        return {
            ...
            "type": self.node.type,
            "reranker_setting": self.node_params_serializer.data.get(
                "reranker_setting"
            ),
