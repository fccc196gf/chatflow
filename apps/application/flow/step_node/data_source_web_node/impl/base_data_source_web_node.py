...
from application.flow.step_node.data_source_web_node.i_data_source_web_node import (
    IDataSourceWebNode,
)
...
class BaseDataSourceWebNodeForm(BaseForm):
    source_url = forms.TextInputField(
        _("Web source url"),
        required=True,
        attrs={"placeholder": _("Please enter the Web root address")},
    )
    selector = forms.TextInputField(
        _("Web knowledge selector"),
        required=False,
        attrs={
            "placeholder": _(
                "The default is body, you can enter .classname/#idname/tagname"
            )
        },
    )
...
class BaseDataSourceWebNode(IDataSourceWebNode):
    ...
    def execute(self, **kwargs) -> NodeResult:
        ...
        try:
            ForkManage(
                source_url, selector.split(" ") if selector is not None else []
            ).fork(3, set(), collect_handler)
    ...
    def get_details(self, index: int, **kwargs):
        return {
            ...
            "type": self.node.type,
            "input_params": {
                "source_url": self.context.get("source_url"),
                "selector": self.context.get("selector"),
            },
