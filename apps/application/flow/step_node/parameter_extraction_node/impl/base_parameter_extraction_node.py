...
from application.flow.step_node.parameter_extraction_node.i_parameter_extraction_node import (
    IParameterExtractionNode,
)
...
def json_loads(response, expected_fields):
    ...
    extraction_strategies = [
        ...
        lambda: json.loads(
            re.search(r"```(?:json)?\s*(\{.*?\})\s*```", cleaned, re.DOTALL).group(1)
        ),
...
class BaseParameterExtractionNode(IParameterExtractionNode):
    ...
    def execute(
        self, input_variable, variable_list, model_params_setting, model_id, **kwargs
    ) -> NodeResult:
