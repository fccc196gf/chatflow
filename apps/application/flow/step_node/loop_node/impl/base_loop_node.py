...
def write_context_stream(
    node_variable: Dict, workflow_variable: Dict, node: INode, workflow
):
    """
    写入上下文数据 (流式)
    ...
    for chunk in response:
        ...
        yield {"content": content_chunk, "reasoning_content": reasoning_content_chunk}
    runtime_details = workflow_manage.get_runtime_details()
    _write_context(
        node_variable, workflow_variable, node, workflow, answer, reasoning_content
    )
...
def write_context(node_variable: Dict, workflow_variable: Dict, node: INode, workflow):
    ...
    reasoning = Reasoning(
        model_setting.get("reasoning_content_start"),
    ...
    if "reasoning_content" in response.response_metadata:
        ...
        reasoning_content = reasoning_result.get(
            "reasoning_content"
        ) + reasoning_result_end.get("reasoning_content")
    _write_context(
        node_variable, workflow_variable, node, workflow, content, reasoning_content
    )
...
def generate_loop_array(array):
    def i(current_index: int):
        return iter(
            [(array[index], index) for index in range(current_index, len(array))]
        )
...
def loop(workflow_manage_new_instance, node: INode, generate_loop):
    ...
    for item, index in generate_loop(current_index):
        ...
        @return:
        """
        instance = workflow_manage_new_instance(
        ...
        for chunk in response:
            if (
                chunk.get("node_type") == "loop-break-node"
                and chunk.get("content", "") == "BREAK"
            ):
                break_outer = True
                continue
            ...
            if chunk.get("node_status", "SUCCESS") == "ERROR":
                ...
                insert_or_replace(
                    ...
                    index,
                    get_answer_list(
                        instance, child_node_node_dict, node.runtime_node_id
                    ),
                )
...
def get_write_context(loop_type, array, number, loop_body):
    def inner_write_context(
        node_variable: Dict, workflow_variable: Dict, node: INode, workflow
    ):
        if loop_type == "ARRAY":
            return loop(
                node_variable["workflow_manage_new_instance"],
                node,
                generate_loop_array(array),
            )
        if loop_type == "LOOP":
            return loop(
                node_variable["workflow_manage_new_instance"], node, generate_while_loop
            )
        return loop(
            node_variable["workflow_manage_new_instance"],
            node,
            generate_loop_number(number),
        )
...
class BaseLoopNode(ILoopNode):
    ...
    def execute(self, loop_type, array, number, loop_body, **kwargs) -> NodeResult:
        ...
        from application.flow.knowledge_loop_workflow_manage import (
            KnowledgeLoopWorkflowManage,
        )
        ...
        def workflow_manage_new_instance(
            ...
            workflow_mode = (
                ...
                if WorkflowMode.KNOWLEDGE == self.workflow_manage.flow.workflow_mode
                else WorkflowMode.APPLICATION_LOOP
            )
            c = (
                KnowledgeLoopWorkflowManage
                if workflow_mode == WorkflowMode.KNOWLEDGE_LOOP
                else LoopWorkflowManage
            )
            workflow_manage = c(
                Workflow.new_instance(loop_body, workflow_mode),
        ...
        return NodeResult(
            {"workflow_manage_new_instance": workflow_manage_new_instance},
            {},
    ...
    def get_loop_context_data(self):
        ...
        return {
            f.get("value"): self.context.get(f.get("value"))
            for f in fields
            if self.context.get(f.get("value")) is not None
        }
