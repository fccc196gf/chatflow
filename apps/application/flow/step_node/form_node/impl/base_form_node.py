...
multi_select_list = ["MultiSelect", "MultiRow"]


def get_default_option(option_list, _type, value_field):
    try:
        if (
            option_list is not None
            and isinstance(option_list, list)
            and len(option_list) > 0
        ):
            default_value_list = [
                o.get(value_field) for o in option_list if o.get("default")
            ]
            if len(default_value_list) == 0:
                return (
                    [option_list[0].get(value_field)]
                    if multi_select_list.__contains__(_type)
                    else option_list[0].get(value_field)
                )
            else:
                if multi_select_list.__contains__(_type):
...
class BaseFormNode(IFormNode):
        ...
        for f in reset_field:
            ...
            if isinstance(_value, str):
                ...
                if tooltip is not None:
                    _value.get("attrs")["tooltip"] = generate_prompt(
                        self.workflow_manage, tooltip
                    )
    ...
    def execute(
        self, form_field_list, form_content_format, form_data, **kwargs
    ) -> NodeResult:
        if form_data is not None:
        ...
        form = (
            f"<form_rander>{json.dumps(form_setting, ensure_ascii=False)}</form_rander>"
        )
        ...
        prompt_template = PromptTemplate.from_template(
            form_content_format, template_format="jinja2"
        )
        ...
        return NodeResult(
            {
                "result": value,
                "form_field_list": form_field_list,
                "form_content_format": form_content_format,
            },
            {},
