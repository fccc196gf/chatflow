...
class ImageToVideoNodeSerializer(serializers.Serializer):
    ...
    dialogue_type = serializers.CharField(
        required=False, default="NODE", label=_("Conversation storage type")
    )
    ...
    model_params_setting = serializers.JSONField(
        required=False, default=dict, label=_("Model parameter settings")
    )
...
class IImageToVideoNode(INode):
    ...
    def _run(self):
        ...
        last_frame_url = None
        if (
            self.node_params_serializer.data.get("last_frame_url") is not None
            and self.node_params_serializer.data.get("last_frame_url") != []
        ):
        ...
        node_params_data = {
            k: v
            for k, v in self.node_params_serializer.data.items()
