...
class BaseImageToVideoNode(IImageToVideoNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        ttv_model = get_model_instance_by_model_workspace_id(
        ...
        video_urls = ttv_model.generate_video(
            question, negative_prompt, first_frame_url, last_frame_url
        )
        # 保存图片
    ...
    def get_file_base64(self, image_url):
        try:
            if isinstance(image_url, list):
                image_url = (
                    image_url[0].get("file_id")
                    if "file_id" in image_url[0]
