...
class ApplicationNodeSerializer(serializers.Serializer):
    ...
    question_reference_address = serializers.ListField(
        required=True, label=_("User Questions")
    )
    api_input_field_list = serializers.ListField(
        required=False, label=_("API Input Fields")
    )
    user_input_field_list = serializers.ListField(
        required=False, label=_("User Input Fields")
    )
    ...
    node_data = serializers.DictField(
        required=False, allow_null=True, label=_("Form Data")
    )
...
class IApplicationNode(INode):
    ...
    def _run(self):
        ...
        kwargs = {}
        for api_input_field in self.node_params_serializer.data.get(
            "api_input_field_list", []
        ):
            value = (
                api_input_field.get("value", [""])[0]
                if api_input_field.get("value")
                else ""
            )
            kwargs[api_input_field["variable"]] = (
                self.workflow_manage.get_reference_field(
                    value, api_input_field["value"][1:]
                )
                if value != ""
                else ""
            )

        for user_input_field in self.node_params_serializer.data.get(
            "user_input_field_list", []
        ):
            value = (
                user_input_field.get("value", [""])[0]
                if user_input_field.get("value")
                else ""
            )
            kwargs[user_input_field["field"]] = (
                self.workflow_manage.get_reference_field(
                    value, user_input_field["value"][1:]
                )
                if value != ""
                else ""
            )
        ...
        if app_document_list and len(app_document_list) > 0:
            app_document_list = self.workflow_manage.get_reference_field(
                app_document_list[0], app_document_list[1:]
            )
            for document in app_document_list:
                if "file_id" not in document:
                    raise ValueError(
                        _(
                            "Parameter value error: The uploaded document lacks file_id, and the document upload fails"
                        )
                    )
        ...
        if app_image_list and len(app_image_list) > 0:
            app_image_list = self.workflow_manage.get_reference_field(
                app_image_list[0], app_image_list[1:]
            )
            for image in app_image_list:
                if "file_id" not in image:
                    raise ValueError(
                        _(
                            "Parameter value error: The uploaded image lacks file_id, and the image upload fails"
                        )
                    )
        ...
        if app_audio_list and len(app_audio_list) > 0:
            app_audio_list = self.workflow_manage.get_reference_field(
                app_audio_list[0], app_audio_list[1:]
            )
            for audio in app_audio_list:
                if "file_id" not in audio:
                    raise ValueError(
                        _(
                            "Parameter value error: The uploaded audio lacks file_id, and the audio upload fails."
                        )
                    )
        ...
        if app_video_list and len(app_video_list) > 0:
            app_video_list = self.workflow_manage.get_reference_field(
                app_video_list[0], app_video_list[1:]
            )
            for video in app_video_list:
                if "file_id" not in video:
                    raise ValueError(
                        _(
                            "Parameter value error: The uploaded video lacks file_id, and the video upload fails."
                        )
                    )
        return self.execute(
            **{**self.flow_params_serializer.data, **self.node_params_serializer.data},
            ...
            source=self.workflow_params.get("source")
            or {"type": ChatSourceChoices.ONLINE.value},
