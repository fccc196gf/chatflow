...
from application.flow.step_node.application_node.i_application_node import (
    IApplicationNode,
)
    ...
    for chunk in response:
        ...
        if real_node_id is not None:
            ...
            if application_node is None:
                ...
                application_node_dict[real_node_id] = {
                    "content": content,
                    "runtime_node_id": runtime_node_id,
                    "chat_record_id": chat_record_id,
                    "child_node": child_node,
...
def write_context(node_variable: Dict, workflow_variable: Dict, node: INode, workflow):
    ...
    answer = (
        ...
        or "抱歉，没有查找到相关内容，请重新描述您的问题或提供更多信息。"
    )
    reasoning_content = response.get("reasoning_content", "")
    answer_list = response.get("answer_list", [])
    node_variable["application_node_dict"] = {
        answer.get("real_node_id"): {**answer, "index": index}
        for answer, index in zip(answer_list, range(len(answer_list)))
    }
    _write_context(
        node_variable, workflow_variable, node, workflow, answer, reasoning_content
    )
        ...
        for key in application_node_dict:
            ...
            if application_node.get("runtime_node_id") == runtime_node_id:
                ...
                if match:
                    ...
                    res = re.sub("<form_rander>.*?</form_rander>", "${value}", content)
        ...
        if application_node_dict is None or len(application_node_dict) == 0:
            return [
                Answer(
                    ...
                    self.workflow_params["chat_record_id"],
            ...
            return [
                Answer(
                    ...
                    self.workflow_params["chat_record_id"],
                    {
                        "runtime_node_id": n.get("runtime_node_id"),
                        "chat_record_id": n.get("chat_record_id"),
                        "child_node": n.get("child_node"),
                    },
                    n.get("real_node_id"),
                    n.get("reasoning_content", ""),
                )
                for n in sorted(
                    application_node_dict.values(), key=lambda item: item.get("index")
                )
            ]
    ...
    def execute(
        ...
        **kwargs,
        ...
        if child_node is not None:
            ...
            application_node_dict = self.context.get("application_node_dict")
            reset_application_node_dict(
                application_node_dict, runtime_node_id, node_data
            )
        response = ChatSerializers(
            data={
        ...
        if response.status_code == 200:
            if stream:
                ...
                return NodeResult(
                    {"result": content_generator, "question": message},
                    {},
