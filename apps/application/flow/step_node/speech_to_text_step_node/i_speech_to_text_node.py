...
class SpeechToTextNodeSerializer(serializers.Serializer):
    ...
    audio_list = serializers.ListField(
        required=True, label=_("The audio file cannot be empty")
    )
...
class ISpeechToTextNode(INode):
    ...
    def _run(self):
        res = self.workflow_manage.get_reference_field(
            self.node_params_serializer.data.get("audio_list")[0],
    ...
    def execute(
        self, stt_model_id, audio, model_params_setting=None, **kwargs
    ) -> NodeResult:
        pass
