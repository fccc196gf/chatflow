...
from application.flow.step_node.speech_to_text_step_node.i_speech_to_text_node import (
    ISpeechToTextNode,
)
...
class BaseSpeechToTextNode(ISpeechToTextNode):
    ...
    def execute(
        ...
        stt_model = get_model_instance_by_model_workspace_id(
            stt_model_id, workspace_id, **model_params_setting
        )
        audio_list = audio
        ...
        def process_audio_items(audio_list, model):
            with ThreadPoolExecutor(max_workers=5) as executor:
                results = list(
                    executor.map(
                        lambda item: process_audio_item(item, model), audio_list
                    )
                )
            return results
