...
from application.flow.step_node.loop_continue_node.i_loop_continue_node import (
    ILoopContinueNode,
)
...
class BaseLoopContinueNode(ILoopContinueNode):
    ...
    def execute(self, condition, condition_list, **kwargs) -> NodeResult:
        condition_list = [
            self.assertion(row.get("field"), row.get("compare"), row.get("value"))
    ...
    def assertion(self, field_list: List[str], compare: str, value):
        ...
        field_value = None
        try:
            field_value = self.workflow_manage.get_reference_field(
                field_list[0], field_list[1:]
            )
        except Exception as e:
            pass
        for compare_handler in compare_handle_list:
            if compare_handler.support(
                field_list[0], field_list[1:], field_value, compare, value
            ):
