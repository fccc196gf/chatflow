...
class BaseIntentNode(IIntentNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            # 解析分类结果获取分支信息
            matched_branch = self.parse_classification_result(
                classification_result, branch
            )
    ...
    def get_history_message(history_chat_record, dialogue_number):
        ...
        history_message = reduce(
            ...
            [
                [
                    history_chat_record[index].get_human_message(),
    ...
    def build_classification_prompt(self, user_input: str, branch: List[Dict]) -> str:
        ...
        if other_branch:
            classification_list.append(
                {"classificationId": 0, "content": other_branch.get("content")}
            )
        # 添加正常分支
        classification_id = 1
        for b in branch:
            if not b.get("isOther"):
                classification_list.append(
                    {"classificationId": classification_id, "content": b["content"]}
                )
                classification_id += 1

        return PROMPT_TEMPLATE.format(
            classification_list=classification_list, user_input=user_input
        )
    ...
    def parse_classification_result(
        self, result: str, branch: List[Dict]
    ) -> Dict[str, Any]:
        """解析分类结果"""
        ...
        # 如果都解析失败，返回“other”
        return other_branch or (
            normal_intents[0]
            if normal_intents
            else {"id": "unknown", "content": "unknown"}
        )
