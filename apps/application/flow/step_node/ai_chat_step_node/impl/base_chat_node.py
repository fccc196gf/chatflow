...
def _write_context(
    ...
    message_tokens = chat_model.get_num_tokens_from_messages(
        node_variable.get("message_list")
    )
...
def write_context_stream(
    ...
    for chunk in response:
        ...
        if "reasoning_content" in chunk.additional_kwargs:
            ...
            reasoning_content_chunk = chunk.additional_kwargs.get(
                "reasoning_content", ""
            )
        else:
...
def get_node_message(chat_record, runtime_node_id):
    ...
    if node_details is None:
        return []
    return [
        HumanMessage(node_details.get("question")),
        AIMessage(node_details.get("answer")),
    ]
...
def get_message(chat_record, dialogue_type, runtime_node_id):
    return (
        get_node_message(chat_record, runtime_node_id)
        if dialogue_type == "NODE"
        else get_workflow_message(chat_record)
    )
...
class BaseChatNode(IChatNode):
    ...
    def _handle_mcp_request(
        ...
        if application_ids and len(application_ids) > 0:
            ...
            for application_id in application_ids:
                app = (
                    ...
                    .filter(id=application_id, is_publish=True)
                    .first()
                )
                if app is None:
                    continue
                app_key = (
                    ...
                    .filter(application_id=application_id, is_active=True)
                    .first()
                )
                if app_key is not None:
                    ...
                    application_access_token = (
                        ...
                        .filter(application_id=app_key.application_id)
                        .first()
                    )
                    if (
                        application_access_token is not None
                    ...
                    raise AppApiException(
                        500,
                        _("Agent Key is required for agent tool 【{name}】").format(
                            name=app.name
                        ),
                    )
                executor = ToolExecutor()
        ...
        if len(mcp_servers_config) > 0:
            ...
            if (
                ...
                and self.workflow_manage.work_flow_post_handler.chat_info
            ):
                application_id = (
                    self.workflow_manage.work_flow_post_handler.chat_info.application.id
                )
    ...
    def get_history_message(
        history_chat_record, dialogue_number, dialogue_type, runtime_node_id
    ):
        ...
        for message in history_message:
            if isinstance(message.content, str):
                message.content = re.sub(
                    "<form_rander>[\d\D]*?<\/form_rander>", "", message.content
                )
