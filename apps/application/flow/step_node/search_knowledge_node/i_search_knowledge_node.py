...
class SearchDatasetStepNodeSerializer(serializers.Serializer):
    ...
    search_scope_source = serializers.ChoiceField(
        required=False,
        choices=["document", "knowledge"],
        label=_("search scope variable type"),
        default="knowledge",
    )
...
class ISearchKnowledgeStepNode(INode):
    ...
    def _run(self):
        ...
        if self.flow_params_serializer.data.get("re_chat", False):
            history_chat_record = self.flow_params_serializer.data.get(
                "history_chat_record", []
            )
            paragraph_id_list = [
                p.get("id")
                for p in flat_map(
                    [
                        get_paragraph_list(chat_record, self.runtime_node_id)
