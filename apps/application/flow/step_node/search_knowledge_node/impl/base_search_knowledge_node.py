...
from application.flow.step_node.search_knowledge_node.i_search_knowledge_node import (
    ISearchKnowledgeStepNode,
)
...
class BaseSearchKnowledgeNode(ISearchKnowledgeStepNode):
    def save_context(self, details, workflow_manage):
        ...
        directly_return = "\n".join(
            [
                f"{paragraph.get('title', '')}:{paragraph.get('content')}"
        ...
        self.context["is_hit_handling_method_list"] = [
        ...
        return NodeResult(
            {
                ...
                "data": "\n".join(
                    [
                        f"{reset_title(paragraph.get('title', ''))}{paragraph.get('content')}"
                        for paragraph in result
                    ]
                )[0 : knowledge_setting.get("max_paragraph_char_number", 5000)],
                "directly_return": "\n".join(
        ...
        if filter_embedding_list is not None and len(filter_embedding_list) > 0:
            ...
            return {
                **paragraph,
                ...
                "is_hit_handling_method": find_embedding.get("similarity")
                > paragraph.get("directly_return_similarity")
                and paragraph.get("hit_handling_method") == "directly_return",
                "update_time": paragraph.get("update_time").strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
                "create_time": paragraph.get("create_time").strftime(
                    "%Y-%m-%d %H:%M:%S"
                ),
