...
def get_global_variable(node):
    ...
    history_chat_record = node.flow_params_serializer.data.get(
        "history_chat_record", []
    )
    history_context = [
        {"question": chat_record.problem_text, "answer": chat_record.answer_text}
...
class BaseStartStepNode(IStarNode):
    def save_context(self, details, workflow_manage):
        ...
        default_global_variable = get_default_global_variable(
            base_node.properties.get("user_input_field_list", [])
        )
        default_api_global_variable = get_default_global_variable(
            base_node.properties.get("api_input_field_list", [])
        )
        workflow_variable = {
            **default_global_variable,
            **default_api_global_variable,
        ...
        self.workflow_manage.chat_context = (
            self.workflow_manage.get_chat_info().get_chat_variable()
        )
            ...
            global_fields.append(
                {
                    ...
                    "key": key,
                    "value": (
                        self.workflow_manage.context[key]
                        if key in self.workflow_manage.context
                        else ""
                    ),
                }
            )
