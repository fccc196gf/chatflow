...
def write_context(step_variable: Dict, global_variable: Dict, node, workflow):
    if step_variable is not None:
        ...
        if (
            workflow.is_result(node, NodeResult(step_variable, global_variable))
...
def valid_reference_value(_type, value, name):
    ...
    if not isinstance(value, instance_type):
        raise Exception(
            _("Field: {name} Type: {_type} Value: {value} Type error").format(
...
def convert_value(name: str, value, _type, is_required, source, node):
    if not is_required and (
        value is None
        or ((isinstance(value, str) or isinstance(value, list)) and len(value) == 0)
    ):
        return None
    if source == "reference":
...
def valid_function(tool_lib, workspace_id):
    ...
    if (
        tool_lib
        and tool_lib.workspace_id != workspace_id
        and get_authorized_tool is not None
    ):
        tool_lib = get_authorized_tool(
            QuerySet(Tool).filter(id=tool_lib.id), workspace_id
        ).first()
    if tool_lib is None:
        raise Exception(_("Tool does not exist"))
...
def _get_result_detail(result):
    if isinstance(result, dict):
        result_dict = {
            k: (str(v)[:500] if len(str(v)) > 500 else v) for k, v in result.items()
        }
    elif isinstance(result, list):
        result_dict = [
            str(item)[:500] if len(str(item)) > 500 else item for item in result
        ]
...
class BaseToolLibNodeNode(IToolLibNode):
    ...
    def execute(self, tool_lib_id, input_field_list, **kwargs) -> NodeResult:
        ...
        init_params_default_value = {
            i["field"]: i.get("default_value") for i in tool_lib.init_field_list
        }
        if tool_lib.init_params is not None:
            all_params = (
                init_params_default_value
                | json.loads(rsa_long_decrypt(tool_lib.init_params))
                | params
            )
        else:
            ...
            if exist:
                ...
                for item in download_list:
                    ...
                    download_file_list.append(
                        {"file_id": file_url.split("/")[-1], "name": result.get("name")}
                    )
    ...
    def tool_exec_record(self, tool_lib, all_params):
        ...
        try:
            ...
            tool_init_params = (
                json.loads(rsa_long_decrypt(tool_lib.init_params))
                if tool_lib.init_params
                else {}
            )
            if tool_init_params:
                filtered_args = {
                    k: v for k, v in all_params.items() if k not in tool_init_params
                }
            else:
            ...
            ToolRecord(
                ...
                tool_id=tool_lib.id,
                source_type=(
                    ToolTaskTypeChoices.KNOWLEDGE.value
                    if self.workflow_manage.params.get("knowledge_id")
                    else ToolTaskTypeChoices.APPLICATION.value
                ),
                source_id=self.workflow_manage.params.get("knowledge_id")
                or self.workflow_manage.params.get("application_id"),
            ...
            chatflow_logger.error(f"Tool execution error: {traceback.format_exc()}")
            QuerySet(ToolRecord).filter(id=task_record_id).update(
                state=State.FAILURE,
                run_time=time.time() - start_time,
