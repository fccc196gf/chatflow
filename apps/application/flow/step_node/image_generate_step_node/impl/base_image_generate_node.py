...
from application.flow.step_node.image_generate_step_node.i_image_generate_node import (
    IImageGenerateNode,
)
...
class BaseImageGenerateNode(IImageGenerateNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        tti_model = get_model_instance_by_model_workspace_id(
        ...
        self.context["image_list"] = [
            {"file_id": path.split("/")[-1], "url": path} for path in file_urls
        ]
        answer = " ".join([f"![Image]({path})" for path in file_urls])
        return NodeResult(
            {
                "answer": answer,
                "chat_model": tti_model,
        ...
        for val in chat_record.details.values():
            if self.node.id == val["node_id"] and "image_list" in val:
                ...
                return AIMessage(
                    content=[
                        *[
                            {"type": "image_url", "image_url": {"url": f"{file_url}"}}
