...
from application.flow.step_node.video_understand_step_node.i_video_understand_node import (
    IVideoUnderstandNode,
)
...
def _write_context(
    node_variable: Dict, workflow_variable: Dict, node: INode, workflow, answer: str
):
    chat_model = node_variable.get("chat_model")
    message_tokens = (
        node_variable["usage_metadata"]["output_tokens"]
        if "usage_metadata" in node_variable
        else 0
    )
...
class BaseVideoUnderstandNode(IVideoUnderstandNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        video_model = get_model_instance_by_model_workspace_id(
        ...
        message_list = self.generate_message_list(
            video_model,
            system,
            prompt,
            self.get_history_message(history_chat_record, dialogue_number, video_model),
            video,
        )
    ...
    def get_history_message_for_details(self, history_chat_record, dialogue_number):
        ...
        history_message = reduce(
            ...
            [
                [
                    self.generate_history_human_message_for_details(
                        history_chat_record[index]
                    ),
                    self.generate_history_ai_message(history_chat_record[index]),
                ]
                for index in range(
                    start_index if start_index > 0 else 0, len(history_chat_record)
                )
            ],
            [],
        )
    ...
    def generate_history_ai_message(self, chat_record):
        for val in chat_record.details.values():
            if self.node.id == val["node_id"] and "video_list" in val:
                ...
                return AIMessage(
                    content=val.get("answer") or val.get("err_message") or ""
                )
        return chat_record.get_ai_message()
        ...
        for data in chat_record.details.values():
            if self.node.id == data["node_id"] and "video_list" in data:
                ...
                if (
                    not video_list
                    or len(video_list) == 0
                    or data["dialogue_type"] == "WORKFLOW"
                ):
                ...
                return HumanMessage(
                    content=[
                        ...
                        *[
                            {
                                "type": "video_url",
                                "video_url": {"url": f"./oss/file/{file_id}"},
                            }
    ...
    def get_history_message(self, history_chat_record, dialogue_number, video_model):
        ...
        history_message = reduce(
            ...
            [
                [
                    self.generate_history_human_message(
                        history_chat_record[index], video_model
                    ),
                    self.generate_history_ai_message(history_chat_record[index]),
                ]
                for index in range(
            ...
            if self.node.id == data["node_id"] and "video_list" in data:
                ...
                for image in video_list:
                    if "file_id" in image:
                        ...
                        url_list.append(image.get("url"))
                video_base64_list = [
                    file_id_to_base64(video.get("file_id"), video_model)
        ...
        if isinstance(image, str) and image.startswith("http"):
            ...
            for img in image:
                if "file_id" in img:
                    file_id = img["file_id"]
                    file = QuerySet(File).filter(id=file_id).first()
                    url = video_model.upload_file_and_get_url(
                        file.get_bytes(), file.file_name
                    )
    ...
    def generate_message_list(
        self, video_model, system: str, prompt: str, history_message, video
    ):
        ...
        if videos:
            messages = [
                HumanMessage(content=[{"type": "text", "text": prompt_text}, *videos])
            ]
        else:
        ...
        return {
            ...
            "history_message": [
                ...
                for message in (
                    ...
                    if self.context.get("history_message") is not None
                    else []
                )
            ],
