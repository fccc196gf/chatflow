...
from application.flow.step_node.knowledge_write_node.i_knowledge_write_node import (
    IKnowledgeWriteNode,
)
...
class ParagraphInstanceSerializer(serializers.Serializer):
    ...
    problem_list = serializers.ListField(
        required=False, child=serializers.CharField(required=False, allow_blank=True)
    )
    is_active = serializers.BooleanField(required=False, label=_("Is active"))
    chunks = serializers.ListField(
        required=False, child=serializers.CharField(required=True)
    )
...
class KnowledgeWriteParamSerializer(serializers.Serializer):
    ...
    tags = serializers.ListField(
        required=False, label=_("Tags"), child=TagInstanceSerializer()
    )
    ...
    paragraph = Paragraph(
        ...
        chunks=[
            ...
            for c in (
                ...
                else text_to_chunk(instance.get("content"))
            )
        ],
    )
...
def save_knowledge_tags(knowledge_id: str, tags: List[Dict[str, Any]]):
    existed_tags_dict = {
        ...
        for key, value, tag_id in QuerySet(Tag)
        .filter(knowledge_id=knowledge_id)
        .values_list("key", "value", "id")
    }
    ...
    for tag in tags:
        ...
        if (key, value) not in existed_tags_dict:
            tag_model = Tag(
                id=uuid.uuid7(), knowledge_id=knowledge_id, key=key, value=value
            )
...
def batch_add_document_tag(document_tag_map: Dict[str, List[str]]):
    ...
    all_tag_ids = list(
        set(tag_id for tag_ids in document_tag_map.values() for tag_id in tag_ids)
    )
    ...
    def save(self, document_list):
        ...
        if knowledge_tag_list:
            all_tag_dict, new_tag_dict = save_knowledge_tags(
                knowledge_id, knowledge_tag_list
            )
            # 构建文档-标签ID映射
            document_tag_id_map = {}
            # 为每个文档添加其对应的标签
