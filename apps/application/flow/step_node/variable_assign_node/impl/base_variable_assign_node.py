...
class BaseVariableAssignNode(IVariableAssignNode):
    ...
    def global_evaluation(self, variable, value):
        ...
        if isinstance(self.workflow_manage, LoopWorkflowManage):
            self.workflow_manage.parentWorkflowManage.context[variable["fields"][1]] = (
                value
            )
        else:
    ...
    def handle(self, variable, evaluation):
        ...
        if variable["source"] == "custom":
            if variable["type"] == "json":
                if isinstance(variable["value"], dict) or isinstance(
                    variable["value"], list
                ):
                    val = variable["value"]
                else:
    ...
    def execute(self, variable_list, **kwargs) -> NodeResult:
        ...
        if is_chat:
            ...
            if isinstance(self.workflow_manage, LoopWorkflowManage):
                ...
                self.workflow_manage.get_chat_info().set_chat_variable(
                    self.workflow_manage.chat_context
                )
        return NodeResult(
            {"variable_list": variable_list, "result_list": result_list}, {}
        )
