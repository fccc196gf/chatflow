...
from application.flow.step_node.document_split_node.i_document_split_node import (
    IDocumentSplitNode,
)
...
class BaseDocumentSplitNode(IDocumentSplitNode):
    ...
    def execute(
        ...
        **kwargs,
        ...
        for doc in file_list:
            ...
            file_mem = bytes_to_uploaded_file(
                doc["content"].encode("utf-8"), doc["name"]
            )
            if split_strategy == "qa":
                result = md_qa_split_handle.handle(
                    file_mem, get_buffer, self._save_image
                )
            else:
    ...
    def _generate_problem_list(
        ...
        if paragraph_title_relate_problem_type == "referencing":
            paragraph_title_relate_problem = self.get_reference_content(
                paragraph_title_relate_problem_reference
            )
        if document_name_relate_problem_type == "referencing":
            document_name_relate_problem = self.get_reference_content(
                document_name_relate_problem_reference
            )
