...
class KnowledgeWorkflowManage(WorkflowManage):
    ...
    def get_start_node(self):
        start_node_list = [
            ...
            if self.params.get("data_source", {}).get("node_id") == node.id
        ]
    ...
    def _run(self):
        QuerySet(KnowledgeAction).filter(
            id=self.params.get("knowledge_action_id")
    ...
    def run_chain(self, current_node, node_result_future=None):
        QuerySet(KnowledgeAction).filter(
            ...
            details=self.get_runtime_details(
                lambda node, index: self.get_node_details(current_node, node, index)
            )
        )
    ...
    def hand_node_result(self, current_node, node_result_future):
        try:
            ...
            QuerySet(KnowledgeAction).filter(
                id=self.params.get("knowledge_action_id")
            ).update(state=State.FAILURE)
        finally:
            current_node.node_chunk.end()
            QuerySet(KnowledgeAction).filter(
                id=self.params.get("knowledge_action_id")
            ).update(details=self.get_runtime_details())
