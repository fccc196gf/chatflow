    ...
    def handler(self, workflow):
        ...
        if workflow.chat_record is not None:
            ...
            chat_record = ChatRecord(
                ...
                run_time=(
                    time.time() - workflow.context.get("start_time")
        ...
        if not self.chat_info.debug and [
            ChatUserType.ANONYMOUS_USER.value,
            ChatUserType.CHAT_USER.value,
        ].__contains__(workflow_body.get("chat_user_type")):
...
def get_workflow_state(workflow):
    ...
    write_is_exist = any(
        [True for value in all_node if value.get("type") == "knowledge-write-node"]
    )
...
class NodeResult:
    ...
    def is_interrupt_exec(self, current_node):
        ...
        return self._is_interrupt(
            current_node, self.node_variable, self.workflow_variable
        )
...
class ReferenceAddressSerializer(serializers.Serializer):
    ...
    fields = serializers.ListField(
        child=serializers.CharField(required=True, label="节点字段"),
        required=True,
        label="节点字段数组",
    )
...
class KnowledgeFlowParamsSerializer(serializers.Serializer):
    ...
    workspace_id = serializers.CharField(required=True, label="工作空间id")
    knowledge_action_id = serializers.UUIDField(
        required=True, label="知识库任务执行器id"
    )
...
class INode:
    ...
    def get_answer_list(self) -> List[Answer] | None:
        if self.answer_text is None:
            return None
        reasoning_content_enable = self.context.get("model_setting", {}).get(
    ...
    def __init__(
        ...
        self.runtime_node_id = sha1(
            ...
            + bytes(
                str(
                    uuid.uuid5(
                        uuid.NAMESPACE_DNS, "".join([*sorted(up_node_id_list), node.id])
                    )
                ),
                "utf-8",
            )
    ...
    def get_write_error_context(self, e):
        ...
        self.context["run_time"] = current_time - (
            self.context.get("start_time") or current_time
        )
