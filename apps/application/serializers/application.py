...
from common.cache_data.application_access_token_cache import (
    del_application_access_token,
)
...
from chatflow.conf import PROJECT_DIR
from models_provider.models import Model
from models_provider.tools import get_model_instance_by_model_workspace_id
from system_manage.models import WorkspaceUserResourcePermission, AuthTargetType
from system_manage.models.resource_mapping import ResourceMapping
from system_manage.serializers.resource_mapping_serializers import (
...
def hand_node(node, update_tool_map):
    if node.get("type") == "tool-lib-node":
        tool_lib_id = (
            node.get("properties", {}).get("node_data", {}).get("tool_lib_id") or ""
        )
        node.get("properties", {}).get("node_data", {})["tool_lib_id"] = (
            update_tool_map.get(tool_lib_id, tool_lib_id)
        )
    ...
    if node.get("type") == "ai-chat-node":
        ...
        node_data["mcp_tool_ids"] = [
            update_tool_map.get(tool_id, tool_id) for tool_id in mcp_tool_ids
        ]
        tool_ids = node_data.get("tool_ids") or []
        node_data["tool_ids"] = [
            update_tool_map.get(tool_id, tool_id) for tool_id in tool_ids
        ]
    if node.get("type") == "mcp-node":
        mcp_tool_id = (
            node.get("properties", {}).get("node_data", {}).get("mcp_tool_id") or ""
        )
        node.get("properties", {}).get("node_data", {})["mcp_tool_id"] = (
            update_tool_map.get(mcp_tool_id, mcp_tool_id)
        )
...
class NoReferencesSetting(serializers.Serializer):
    status = serializers.ChoiceField(
        required=True,
        choices=NoReferencesChoices.choices,
...
class KnowledgeSettingSerializer(serializers.Serializer):
    top_n = serializers.FloatField(
        required=True, max_value=10000, min_value=1, label=_("Reference segment number")
    )
    similarity = serializers.FloatField(
        required=True, max_value=1, min_value=0, label=_("Acquaintance")
    )
    ...
    no_references_setting = NoReferencesSetting(
        required=True, label=_("Segment settings not referenced")
    )
...
class ModelKnowledgeAssociation(serializers.Serializer):
    ...
    model_id = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("Model id")
    )
    ...
    def is_valid(self, *, raise_exception=True):
        ...
        exist_knowledge_id_list = [
            str(knowledge.id)
            for knowledge in QuerySet(Knowledge).filter(
                id__in=knowledge_id_list, user_id=user_id
            )
        ]
        for knowledge_id in knowledge_id_list:
            if not exist_knowledge_id_list.__contains__(knowledge_id):
                raise AppApiException(
                    500,
                    f'{_("The knowledge base id does not exist")}【{knowledge_id}】',
                )
...
class ModelSettingSerializer(serializers.Serializer):
    ...
    reasoning_content_enable = serializers.BooleanField(
        required=False, label=_("Thinking process switch")
    )
...
class ApplicationCreateSerializer(serializers.Serializer):
    ...
    class WorkflowRequest(serializers.Serializer):
        name = serializers.CharField(
            required=True, max_length=64, min_length=1, label=_("Application Name")
        )
        ...
        def to_application_model(user_id: str, workspace_id: str, application: Dict):
            ...
            for node in default_workflow.get("nodes"):
                if node.get("id") == "base-node":
                    node.get("properties")["node_data"]["desc"] = application.get(
                        "desc"
                    )
                    node.get("properties")["node_data"]["name"] = application.get(
                        "name"
                    )
                    node.get("properties")["node_data"]["prologue"] = application.get(
                        "prologue"
                    )
            return Application(
                id=uuid.uuid7(),
    ...
    class SimplateRequest(serializers.Serializer):
        ...
        folder_id = serializers.CharField(required=True, label=_("folder id"))
        model_id = serializers.CharField(
            required=False, allow_null=True, allow_blank=True, label=_("Model")
        )
        ...
        problem_optimization = serializers.BooleanField(
            required=True, label=_("Question completion")
        )
        ...
        model_params_setting = serializers.DictField(
            required=False, label=_("Model parameters")
        )

        tts_model_enable = serializers.BooleanField(
            required=False, label=_("Voice playback enabled")
        )

        tts_model_id = serializers.UUIDField(
            required=False, allow_null=True, label=_("Voice playback model ID")
        )

        tts_type = serializers.CharField(required=False, label=_("Voice playback type"))

        tts_autoplay = serializers.BooleanField(
            required=False, label=_("Voice playback autoplay")
        )

        stt_model_enable = serializers.BooleanField(
            required=False, label=_("Voice recognition enabled")
        )

        stt_model_id = serializers.UUIDField(
            required=False, allow_null=True, label=_("Speech recognition model ID")
        )

        stt_autosend = serializers.BooleanField(
            required=False, label=_("Voice recognition automatic transmission")
        )
...
class ApplicationListResponse(serializers.Serializer):
    id = serializers.CharField(
        required=True, label=_("Primary key id"), help_text=_("Primary key id")
    )
    name = serializers.CharField(
        required=True, label=_("Application Name"), help_text=_("Application Name")
    )
    desc = serializers.CharField(
        required=True,
        label=_("Application Description"),
    ...
    is_publish = serializers.BooleanField(
        required=True, label=_("Model id"), help_text=_("Model id")
    )
    type = serializers.CharField(
        required=True, label=_("Application type"), help_text=_("Application type")
    )
    resource_type = serializers.CharField(
        required=True, label=_("Resource type"), help_text=_("Resource type")
    )
    user_id = serializers.CharField(
        required=True, label=_("Affiliation user"), help_text=_("Affiliation user")
    )
    create_time = serializers.CharField(
        required=True, label=_("Creation time"), help_text=_("Creation time")
    )
    update_time = serializers.CharField(
        required=True, label=_("Modification time"), help_text=_("Modification time")
    )
...
class Query(serializers.Serializer):
    ...
    def get_query_set(self, instance: Dict, workspace_manage: bool, is_x_pack_ee: bool):
        ...
        if workspace_id is not None:
            ...
            application_query_set = application_query_set.filter(
                workspace_id=workspace_id
            )
        ...
        resource_and_folder_query_set = QuerySet(
    ...
    @staticmethod
    def is_x_pack_ee():
        workspace_user_role_mapping_model = DatabaseModelManage.get_model(
            "workspace_user_role_mapping"
        )
        role_permission_mapping_model = DatabaseModelManage.get_model(
                ...
                os.path.join(
                    ...
                    (
                        ...
                        if workspace_manage
                            ...
                            if is_x_pack_ee
                            else "list_application_user.sql"
                        )
                    ),
                )
            ),
...
class ApplicationEditSerializer(serializers.Serializer):
    name = serializers.CharField(
        required=False, max_length=64, min_length=1, label=_("Application Name")
    )
    desc = serializers.CharField(
        required=False,
        max_length=256,
    ...
    model_id = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Model")
    )
    dialogue_number = serializers.IntegerField(
    ...
    # 数据集相关设置
    knowledge_setting = KnowledgeSettingSerializer(
        required=False, allow_null=True, label=_("Dataset settings")
    )
    # 模型相关设置
    model_setting = ModelSettingSerializer(
        required=False, allow_null=True, label=_("Model setup")
    )
    # 问题补全
    problem_optimization = serializers.BooleanField(
        required=False, allow_null=True, label=_("Question completion")
    )
...
class ApplicationSerializer(serializers.Serializer):
    ...
    def insert_template_workflow(self, instance: Dict):
        ...
        try:
            ...
            chatflow_logger.error(f"callback appstore tool download error: {e}")
        return app
    ...
    def insert_simple(self, instance: Dict):
        ...
        user_id = self.data.get("user_id")
        workspace_id = self.data.get("workspace_id")
        ApplicationCreateSerializer.SimplateRequest(data=instance).is_valid(
            user_id=user_id, raise_exception=True
        )
        application_model = (
    ...
    def import_(self, instance: dict, is_import_tool, with_valid=True):
        ...
        if len(tool_list) > 0:
            ...
            exits_tool_id_list = [
                str(tool.id)
                for tool in QuerySet(Tool).filter(
                    id__in=tool_id_list, workspace_id=workspace_id
                )
            ]
            # 需要更新的工具集合
            update_tool_map = {
        ...
        application_model = self.to_application(
            application, workspace_id, user_id, update_tool_map, folder_id
        )
    ...
    def to_tool(tool, workspace_id, user_id):
        ...
        return Tool(
            ...
            is_active=(
                False
                if len((tool.get("init_field_list") or [])) > 0
    ...
    class StoreApplication(serializers.Serializer):
        user_id = serializers.UUIDField(required=True, label=_("User ID"))
        name = serializers.CharField(
            required=False, label=_("tool name"), allow_null=True, allow_blank=True
        )
        ...
        def get_appstore_templates(self):
            ...
            try:
                res = requests.get(
                    "https://apps-assets.fit2cloud.com/stable/maxkb.json.zip", timeout=5
                )
                ...
                try:
                    ...
                    tag_dict = {
                        tag["name"]: tag["key"]
                        for tag in tool_store["additionalProperties"]["tags"]
                    }
                    filter_apps = []
                    for tool in tool_store["apps"]:
                        if self.data.get("name", "") != "":
                            if (
                                self.data.get("name").lower()
                                not in tool.get("name", "").lower()
                            ):
                                continue
                        ...
                        tool["label"] = (
                            tag_dict[tool.get("tags")[0]] if tool.get("tags") else ""
                        )
                        tool["version"] = next(
                            (
                                version.get("name")
...
class PlayDemoTextRequest(serializers.Serializer):
    tts_model_id = serializers.UUIDField(
        required=True, label=_("Text to speech model ID")
    )
...
class ApplicationOperateSerializer(serializers.Serializer):
    ...
    workspace_id = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("Workspace ID")
    )
    ...
    def get_mcp_servers(self, instance, with_valid=True):
        ...
        for server, config in servers.items():
            if config.get("transport") not in ["sse", "streamable_http"]:
                raise AppApiException(
                    500, _("Only support transport=sse or transport=streamable_http")
                )
        tools = []
    ...
    def delete(self, with_valid=True):
        ...
        trigger_ids = list(
            ...
            .filter(source_type="APPLICATION", source_id=application_id)
            .values("trigger_id")
            .distinct()
        )
    ...
    def export(self, with_valid=True):
        try:
            ...
            response["Content-Disposition"] = (
                f'attachment; filename="{application.name}.mk"'
            )
            return response
        except Exception as e:
            return result.error(
                str(e), response_status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def publish(self, instance, with_valid=True):
        ...
        application = (
            ...
            .filter(id=self.data.get("application_id"), workspace_id=workspace_id)
            .first()
        )
        ...
        QuerySet(TriggerTask).filter(
            source_type="APPLICATION", source_id=self.data.get("application_id")
    ...
    def update_work_flow_model(instance):
        ...
        for node in nodes:
            if node["id"] == "base-node":
                ...
                if "tts_model_params_setting" in node_data:
                    instance["tts_model_params_setting"] = node_data[
                        "tts_model_params_setting"
                    ]
                if "stt_model_params_setting" in node_data:
                    instance["stt_model_params_setting"] = node_data[
                ...
                break
        knowledge_node_list = ApplicationOperateSerializer.get_search_node(
            instance.get("work_flow")
        )
        for knowledge_node in knowledge_node_list:
            ...
            view_knowledge_id_list = [
                knowledge.get("id") for knowledge in knowledge_list
            ]
    ...
    def edit(self, instance: Dict, with_valid=True):
        ...
        if (
            instance.get("stt_model_id") is None
            or len(instance.get("stt_model_id")) == 0
        ):
            ...
            if model is None:
                raise AppApiException(500, _("Model does not exist"))
        if (
            instance.get("tts_model_id") is None
            or len(instance.get("tts_model_id")) == 0
        ):
        ...
        application_knowledge_id_list = [
            ...
            for knowledge in self.list_knowledge(with_valid=False)
        ]
        knowledge_id_list = []
        if "knowledge_id_list" in instance:
            ...
            for knowledge_id in knowledge_id_list:
                if not application_knowledge_id_list.__contains__(knowledge_id):
                    message = lazy_format(
                        _(
                            "Unknown knowledge base id {dataset_id}, unable to associate"
                        ),
        ...
        update_resource_mapping_by_application(
            application_id,
            self.get_application_knowledge_mapping(
                application_knowledge_id_list, knowledge_id_list, application_id
            ),
        )
        return self.one(with_valid=False)
    ...
    def update_template_workflow(self, instance: Dict, app: Application):
        ...
        if len(tool_list) > 0:
            ...
            exits_tool_id_list = [
                str(tool.id)
                for tool in QuerySet(Tool).filter(
                    id__in=tool_id_list, workspace_id=app.workspace_id
                )
            ]
            # 需要更新的工具集合
            update_tool_map = {
        ...
        tool_model_list = [
            self.to_tool(f, app.workspace_id, self.data.get("user_id"))
            for f in tool_list
        ]
        work_flow = application.get("work_flow")
        for node in work_flow.get("nodes", []):
    ...
    def one(self, with_valid=True):
        ...
        available_knowledge_dict = {
            knowledge.get("id"): knowledge for knowledge in available_knowledge_list
        }
        knowledge_list = []
        knowledge_id_list = []
    ...
    def update_knowledge_node(self, workflow, available_knowledge_dict):
        ...
        for search_node in knowledge_node_list:
            ...
            knowledge_list = [
                ...
                for knowledge_id in knowledge_id_list
                if available_knowledge_dict.__contains__(knowledge_id)
            ]
            node_data["all_knowledge_id_list"] = knowledge_id_list
            node_data["knowledge_id_list"] = [
                knowledge.get("id") for knowledge in knowledge_list
            ]
    ...
    def list_knowledge(self, with_valid=True):
        ...
        workspace_id = self.data.get("workspace_id")
        user_id = self.data.get("user_id")
        knowledge_workspace_authorization_model = DatabaseModelManage.get_model(
            "knowledge_workspace_authorization"
        )
    ...
    @staticmethod
    def save_application_knowledge_mapping(
        application_knowledge_id_list, knowledge_id_list, application_id
    ):
        # 需要排除已删除的数据集
        knowledge_id_list = [
            knowledge.id
            for knowledge in QuerySet(Knowledge).filter(id__in=knowledge_id_list)
        ]
    ...
    @staticmethod
    def get_application_knowledge_mapping(
        application_knowledge_id_list, knowledge_id_list, application_id
    ):
        """
    ...
    def play_demo_text(self, instance, with_valid=True):
        ...
        model = get_model_instance_by_model_workspace_id(
            tts_model_id, self.data.get("workspace_id"), **instance
        )
