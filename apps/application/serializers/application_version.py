...
class ApplicationVersionQuerySerializer(serializers.Serializer):
    ...
    name = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("summary")
    )
...
class ApplicationVersionSerializer(serializers.Serializer):
    ...
    class Query(serializers.Serializer):
        ...
        def get_query_set(self, query):
            query_set = QuerySet(ApplicationVersion).filter(
        ...
        def list(self, query, with_valid=True):
            if with_valid:
                ...
                ApplicationVersionQuerySerializer(data=query).is_valid(
        ...
        def page(self, query, current_page, page_size, with_valid=True):
            ...
            return page_search(
                ...
                post_records_handler=lambda v: ApplicationVersionModelSerializer(
                    v
                ).data,
            )
    ...
    class Operate(serializers.Serializer):
        ...
        application_id = serializers.UUIDField(required=True, label=_("Application ID"))
        application_version_id = serializers.UUIDField(
            required=True, label=_("Application version ID")
        )
        ...
        def one(self, with_valid=True):
            ...
            application_version = (
                ...
                .filter(
                    ...
                    id=self.data.get("application_version_id"),
                )
                .first()
            )
        ...
        def edit(self, instance: Dict, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                ApplicationVersionEditSerializer(data=instance).is_valid(
                    raise_exception=True
                )
            application_version = (
                QuerySet(ApplicationVersion)
