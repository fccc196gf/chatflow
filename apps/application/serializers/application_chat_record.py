...
class ChatRecordOperateSerializer(serializers.Serializer):
    ...
    chat_record_id = serializers.UUIDField(
        required=True, label=_("Conversation record id")
    )
    ...
    def is_valid(self, *, debug=False, raise_exception=False):
        ...
        application_access_token = (
            ...
            .filter(application_id=self.data.get("application_id"))
            .first()
        )
        if application_access_token is None:
            raise AppApiException(
                500, gettext("Application authentication information does not exist")
            )
    ...
    def get_chat_record(self):
        ...
        if chat_info is not None:
            chat_record_list = [
                ...
                if str(chat_record.id) == str(chat_record_id)
            ]
...
class ApplicationChatRecordQuerySerializers(serializers.Serializer):
    ...
    order_asc = serializers.BooleanField(
        required=False, allow_null=True, label=_("Is it in order")
    )
    ...
    def list(self, with_valid=True):
        ...
        order_by = (
            "create_time"
            if self.data.get("order_asc") is None or self.data.get("order_asc")
            else "-create_time"
        )
        return [
    ...
    def reset_chat_record(chat_record, show_source, show_exec):
        ...
        paragraph_list = []
        if (
            "search_step" in chat_record.details
            and chat_record.details.get("search_step").get("paragraph_list") is not None
        ):
        ...
        for item in [
            ...
            *ApplicationChatRecordQuerySerializers.get_loop_workflow_node(
                chat_record.details
            ),
        ]:
            if item.get("type") == "search-knowledge-node" and item.get(
                "show_knowledge", False
            ):
                paragraph_list = paragraph_list + (item.get("paragraph_list") or [])

            if item.get("type") == "reranker-node" and item.get(
                "show_knowledge", False
            ):
                paragraph_list = paragraph_list + [
                    rl.get("metadata")
                    for rl in (item.get("result_list") or [])
        ...
        knowledge_list = knowledge_list + [
            {"id": knowledge_id, **knowledge}
            for knowledge_id, knowledge in reduce(
                lambda x, y: {**x, **y},
                [
                    {
        ...
        if len(chat_record.improve_paragraph_id_list) > 0:
            paragraph_model_list = QuerySet(Paragraph).filter(
                id__in=chat_record.improve_paragraph_id_list
            )
            if len(paragraph_model_list) < len(chat_record.improve_paragraph_id_list):
                ...
                chat_record.improve_paragraph_id_list = list(
                    filter(
                        lambda p_id: paragraph_model_id_list.__contains__(p_id),
        ...
        show_exec_dict = {
            "execution_details": [
                ...
                if (
                    True
                    if show_exec
                    else chat_record.details[key].get("type") == "start-node"
                )
            ]
        }
        return {
            **ChatRecordSerializerModel(chat_record).data,
            "padding_problem_text": (
                chat_record.details.get("problem_padding").get("padding_problem_text")
                if "problem_padding" in chat_record.details
                else None
            ),
    ...
    def page(
        ...
        page = page_search(
            ...
            .filter(chat_id=self.data.get("chat_id"))
            .order_by(order_by),
            post_records_handler=lambda chat_record: self.reset_chat_record(
...
class ChatRecordImproveSerializer(serializers.Serializer):
    ...
    def get(self, with_valid=True):
        ...
        chat_id = self.data.get("chat_id")
        chat_record = (
            QuerySet(ChatRecord).filter(id=chat_record_id, chat_id=chat_id).first()
        )
        if chat_record is None:
            raise AppApiException(500, gettext("Conversation record does not exist"))
        if (
            chat_record.improve_paragraph_id_list is None
            or len(chat_record.improve_paragraph_id_list) == 0
        ):
            return []
...
class ApplicationChatRecordAddKnowledgeSerializer(serializers.Serializer):
    ...
    chat_ids = serializers.ListSerializer(
        child=serializers.UUIDField(), required=True, label=_("Conversation ID")
    )
    ...
    def is_valid(self, *, raise_exception=False):
        ...
        if not Document.objects.filter(
            id=self.data["document_id"], knowledge_id=self.data["knowledge_id"]
        ).exists():
    ...
    @transaction.atomic
    def post_improve(
        self, instance: Dict, request=None, scope="WORKSPACE", with_valid=True
    ):
        if with_valid:
            ApplicationChatRecordAddKnowledgeSerializer(data=instance).is_valid(
                raise_exception=True
            )
        self.is_valid(raise_exception=True)
        if scope == "WORKSPACE":
            is_permission = get_is_permissions(
                request=request,
                workspace_id=self.data.get("workspace_id"),
        ...
        if len(chat_record_list) < len(chat_ids):
            raise AppApiException(
                500, gettext("Conversation records that do not exist")
            )
        ...
        for chat_record in chat_record_list:
            ...
            problem, _ = Problem.objects.get_or_create(
                content=chat_record.problem_text, knowledge_id=knowledge_id
            )
    ...
    def prepend_paragraphs(document_id, paragraphs):
        # 获取所有现有段落
        existing_paragraphs = list(
            Paragraph.objects.filter(document_id=document_id).order_by("position")
        )
...
class ApplicationChatRecordImproveSerializer(serializers.Serializer):
    ...
    def improve(self, instance: Dict, request=None, scope="WORKSPACE", with_valid=True):
        ...
        if not is_permission:
            raise AppUnauthorizedFailed(403, gettext("No permission to access"))
        ApplicationChatRecordImproveInstanceSerializer(data=instance).is_valid(
            raise_exception=True
        )
        chat_record_id = self.data.get("chat_record_id")
        chat_id = self.data.get("chat_id")
        ...
        problem_text = (
            instance.get("problem_text")
            if instance.get("problem_text") is not None
            else chat_record.problem_text
        )
        problem, _ = QuerySet(Problem).get_or_create(
            content=problem_text, knowledge_id=knowledge_id
        )
        problem_paragraph_mapping = ProblemParagraphMapping(
            id=uuid.uuid7(),
            knowledge_id=knowledge_id,
    ...
    class Operate(serializers.Serializer):
        ...
        knowledge_id = serializers.UUIDField(
            required=True, label=_("Knowledge base id")
        )
        ...
        def delete(self, request=None, scope="WORKSPACE", with_valid=True):
            if with_valid:
                ...
                if scope == "WORKSPACE":
                    ...
                    is_permission = get_is_permissions(
                        ...
                        PermissionConstants.RESOURCE_KNOWLEDGE_DOCUMENT_EDIT,
                        RoleConstants.ADMIN,
                    )
            ...
            if chat_record is None:
                raise AppApiException(
                    500, gettext("Conversation record does not exist")
                )
            if not chat_record.improve_paragraph_id_list.__contains__(
                uuid.UUID(paragraph_id)
            ):
                message = lazy_format(
                    gettext(
            ...
            chat_record.improve_paragraph_id_list = [
                row
                for row in chat_record.improve_paragraph_id_list
                if str(row) != paragraph_id
            ]
            chat_record.save()
