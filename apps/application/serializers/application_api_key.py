...
from common.cache_data.application_api_key_cache import (
    get_application_api_key,
    del_application_api_key,
)
...
class ApplicationKeySerializer(serializers.Serializer):
    ...
    def page(self, current_page: int, page_size: int, with_valid=True):
        ...
        order_by = (
            "-create_time"
            if self.data.get("order_by") is None or self.data.get("order_by") == ""
    ...
    class Operate(serializers.Serializer):
        ...
        def delete(self, with_valid=True):
            ...
            application_api_key = (
                ...
                .filter(id=api_key_id, application_id=application_id)
                .first()
            )
        ...
        def edit(self, instance, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                EditApplicationKeySerializer(data=instance).is_valid(
            ...
            if (
                ...
                application_api_key.allow_cross_domain = instance.get(
                    "allow_cross_domain"
                )
            if (
                ...
                application_api_key.cross_domain_list = instance.get(
                    "cross_domain_list"
                )
