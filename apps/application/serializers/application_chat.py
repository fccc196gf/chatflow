...
from chatflow.settings import TIME_ZONE, edition
...
class ApplicationChatQuerySerializers(serializers.Serializer):
    ...
    abstract = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("summary")
    )
    username = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("username")
    )
    ...
    application_id = serializers.UUIDField(required=True, label=_("Application ID"))
    min_star = serializers.IntegerField(
        required=False, min_value=0, label=_("Minimum number of likes")
    )
    min_trample = serializers.IntegerField(
        required=False, min_value=0, label=_("Minimum number of clicks")
    )
    comparer = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                ...
                message=_("Only supports and|or"),
                code=500,
            )
        ],
    )
    ...
    def get_query_set(self, select_ids=None):
        ...
        if "abstract" in self.data and self.data.get("abstract") is not None:
            base_query_dict["application_chat.abstract__icontains"] = self.data.get(
                "abstract"
            )
        if "username" in self.data and self.data.get("username") is not None:
            base_query_dict["application_chat.asker__username__icontains"] = (
        ...
        if min_star_query is not None and min_trample_query is not None:
            if (
                self.data.get("comparer") is not None
                and self.data.get("comparer") == "or"
            ):
        ...
        return {
            "default_queryset": query_set.filter(condition).order_by(
                "-application_chat.update_time"
            )
        }
    ...
    def to_row(row: Dict):
        ...
        reference_paragraph_len = "\n".join(
            [
                (
                    ...
                    + ":"
                    + str(
                        len(node.get("paragraph_list", []))
                        if node.get("paragraph_list", []) is not None
                        else "0"
                    )
                )
        ...
        reference_paragraph = "\n----------\n".join(
            [
                (
                    ApplicationChatQuerySerializers.paragraph_list_to_string(
                        node.get("paragraph_list", [])
                    )
                    if key == "search_step"
                    else node.get("name")
                    + ":\n"
...
class ChatCountSerializer(serializers.Serializer):
    ...
    def update_chat(self):
        ...
        count_chat_record = native_search(
            ...
            get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "application", "sql", "count_chat_record.sql"
                )
            ),
...
def get_source_display(source):
    ...
    source_mapping = {
        ...
        ChatSourceChoices.ENTERPRISE_WECHAT_ROBOT.value: gettext(
