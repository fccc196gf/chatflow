...
class ChatInfo:
    ...
    def get_no_references_setting(knowledge_setting, model_setting):
        ...
        if no_references_setting.get("status") == "ai_questioning":
            ...
            no_references_setting["value"] = (
                no_references_prompt if len(no_references_prompt) > 0 else "{question}"
            )
    ...
    def get_application(self):
        if self.debug:
            ...
            application = (
                ...
                .filter(application_id=self.application_id)
        ...
        if application.type == ApplicationTypeChoices.SIMPLE.value:
            ...
            exclude_document_id_list = [
                ...
                for document in QuerySet(Document).filter(
                    knowledge_id__in=knowledge_id_list, is_active=False
                )
            ]
            self.knowledge_id_list = knowledge_id_list
            self.exclude_document_id_list = exclude_document_id_list
    ...
    def to_base_pipeline_manage_params(self):
        ...
        return {
            ...
            "top_n": (
                3
                if knowledge_setting.get("top_n") is None
                else knowledge_setting.get("top_n")
            ),
            "similarity": (
                0.6
                if knowledge_setting.get("similarity") is None
                else knowledge_setting.get("similarity")
            ),
            "max_paragraph_char_number": knowledge_setting.get(
            ...
            "problem_optimization_prompt": (
                self.application.problem_optimization_prompt
                if self.application.problem_optimization_prompt is not None
                and len(self.application.problem_optimization_prompt) > 0
                else _(
                    "() contains the user's question. Answer the guessed user's question based on the context ({question}) Requirement: Output a complete question and put it in the <data></data> tag"
                )
            ),
            "prompt": (
                model_setting.get("prompt")
                if "prompt" in model_setting and len(model_setting.get("prompt")) > 0
                else Application.get_default_model_prompt()
            ),
            "system": model_setting.get("system", None),
            "model_id": model_id,
            ...
            "model_params_setting": (
                model_params_setting
                if self.application.model_params_setting is None
                or len(self.application.model_params_setting.keys()) == 0
                else self.application.model_params_setting
            ),
            "search_mode": self.application.knowledge_setting.get("search_mode")
            or "embedding",
            "no_references_setting": self.get_no_references_setting(
                self.application.knowledge_setting, model_setting
            ),
    ...
    def to_pipeline_manage_params(
        ...
        return {
            **params,
            ...
            "post_response_handler": post_response_handler,
    ...
    def set_chat_variable(self, chat_context):
        if not self.debug:
            ...
            if chat:
                chat.meta = {
                    **(chat.meta if isinstance(chat.meta, dict) else {}),
                    **chat_context,
                }
                chat.save()
        else:
            cache.set(
                Cache_Version.CHAT_VARIABLE.get_key(key=self.chat_id),
                chat_context,
    ...
    def append_chat_record(self, chat_record: ChatRecord):
        chat_record.problem_text = (
            chat_record.problem_text[0:10240]
            if chat_record.problem_text is not None
            else ""
        )
        chat_record.answer_text = (
            chat_record.answer_text[0:40960]
            if chat_record.problem_text is not None
            else ""
        )
        is_save = True
        ...
        if not self.debug:
            if not QuerySet(Chat).filter(id=self.chat_id).exists():
                ...
                QuerySet(Chat).filter(id=self.chat_id).update(
    ...
    def to_dict(self):
        ...
        return {
            ...
            "application_id": self.application_id,
            "chat_record_list": [
                self.chat_record_to_map(c) for c in self.chat_record_list
            ][-20:],
            "debug": self.debug,
        }
    ...
    def set_cache(self):
        cache.set(
            Cache_Version.CHAT.get_key(key=self.chat_id),
            self.to_dict(),
    ...
    def map_to_chat_info(chat_info_dict):
        c = ChatInfo(
            chat_info_dict.get("chat_id"),
            chat_info_dict.get("chat_user_id"),
            chat_info_dict.get("chat_user_type"),
            chat_info_dict.get("ip_address"),
        ...
        c.chat_record_list = [
            ...
            for c_r in chat_info_dict.get("chat_record_list")
        ]
        return c

    @staticmethod
    def get_cache(chat_id):
        chat_info_dict = cache.get(
            Cache_Version.CHAT.get_key(key=chat_id),
            version=Cache_Version.CHAT_INFO.get_version(),
        )
        if chat_info_dict:
...
def update_resource_mapping_by_application(
    application_id: str, other_resource_mapping=None
):
