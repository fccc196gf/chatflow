...
class AccessTokenEditSerializer(serializers.Serializer):
    access_token_reset = serializers.BooleanField(
        required=False, label=_("Reset Token")
    )
    is_active = serializers.BooleanField(required=False, label=_("Is it enabled"))
    access_num = serializers.IntegerField(
        required=False, min_value=0, label=_("Number of visits")
    )
    white_active = serializers.BooleanField(
        required=False, label=_("Whether to enable whitelist")
    )
    white_list = (
        serializers.ListSerializer(
            required=False,
            child=serializers.CharField(required=True, label=_("Whitelist")),
            label=_("Whitelist"),
        ),
    )
    show_source = serializers.BooleanField(
        required=False, label=_("Whether to display knowledge sources")
    )
    show_exec = serializers.BooleanField(
        required=False, label=_("Display execution details")
    )
    language = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("language")
    )
    authentication = serializers.BooleanField(
        default=False, label="Do you need authentication"
    )

    authentication_value = serializers.JSONField(
        required=False, label="Certified value", default=dict
    )
...
class AccessTokenSerializer(serializers.Serializer):
    ...
    def edit(self, instance):
        ...
        license_is_valid = cache.get(
            Cache_Version.SYSTEM.get_key(key="license_is_valid"),
            version=Cache_Version.SYSTEM.get_version(),
        )
        if license_is_valid:
            if (
                instance.get("authentication") is not None
                and instance.get("authentication_value") is not None
            ):
                application_access_token.authentication = instance.get("authentication")
                application_access_token.authentication_value = instance.get(
