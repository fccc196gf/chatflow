...
class Application(AppModelMixin):
    ...
    model_params_setting = models.JSONField(
        verbose_name="模型参数相关设置", default=dict
    )
    tts_model_params_setting = models.JSONField(
        verbose_name="模型参数相关设置", default=dict
    )
    stt_model_params_setting = models.JSONField(
        verbose_name="STT模型参数相关设置", default=dict
    )
    problem_optimization = models.BooleanField(verbose_name="问题优化", default=False)
    icon = models.CharField(
        max_length=256, verbose_name="应用icon", default="./favicon.ico"
    )
    ...
    tts_model_enable = models.BooleanField(
        verbose_name="语音合成模型是否启用", default=False
    )
    stt_model_enable = models.BooleanField(
        verbose_name="语音识别模型是否启用", default=False
    )
    tts_type = models.CharField(
        verbose_name="语音播放类型", max_length=20, default="BROWSER"
    )
    ...
    publish_time = models.DateTimeField(
        verbose_name="发布时间", default=None, null=True, blank=True
    )
    file_upload_enable = models.BooleanField(
        verbose_name="文件上传是否启用", default=False
    )
    file_upload_setting = models.JSONField(
        verbose_name="文件上传相关设置", default=dict
    )
    ...
    mcp_source = models.CharField(
        verbose_name="MCP Source", max_length=20, default="referencing"
    )
    ...
    mcp_output_enable = models.BooleanField(
        verbose_name="MCP输出是否启用", default=True
    )
...
class ApplicationVersion(AppModelMixin):
    ...
    tts_model_id = models.UUIDField(
        verbose_name="文本转语音模型id", blank=True, null=True
    )
    stt_model_id = models.UUIDField(
        verbose_name="语音转文本模型id", blank=True, null=True
    )
    tts_model_enable = models.BooleanField(
        verbose_name="语音合成模型是否启用", default=False
    )
