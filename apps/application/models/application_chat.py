...
class Chat(AppModelMixin):
    ...
    asker = models.JSONField(
        verbose_name="访问者", default=default_asker, encoder=SystemEncoder
    )
...
class ChatRecord(AppModelMixin):
    ...
    vote_other_content = models.CharField(
        verbose_name="其他原因", max_length=1024, default=""
    )
    ...
    answer_text_list = ArrayField(
        verbose_name="改进标注列表", base_field=models.JSONField(), default=list
    )
    message_tokens = models.IntegerField(verbose_name="请求token数量", default=0)
    answer_tokens = models.IntegerField(verbose_name="响应token数量", default=0)
    const = models.IntegerField(verbose_name="总费用", default=0)
    details = models.JSONField(
        verbose_name="对话详情", default=dict, encoder=SystemEncoder
    )
    ...
    def get_human_message(self):
        if "problem_padding" in self.details:
            return HumanMessage(
                content=self.details.get("problem_padding").get("padding_problem_text")
            )
...
class ApplicationChatUserStats(AppModelMixin):
    ...
    chat_user_id = models.UUIDField(
        max_length=128, default=uuid.uuid7, verbose_name="对话用户id"
    )
    chat_user_type = models.CharField(
        max_length=64,
        verbose_name="对话用户类型",
    ...
    application = models.ForeignKey(
        Application, on_delete=models.CASCADE, verbose_name="应用id"
    )
