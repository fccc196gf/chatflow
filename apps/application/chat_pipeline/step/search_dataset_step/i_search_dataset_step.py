...
class ISearchDatasetStep(IBaseChatPipelineStep):
    class InstanceSerializer(serializers.Serializer):
        ...
        padding_problem_text = serializers.CharField(
            required=False, label=_("System completes question text")
        )
        # 需要查询的数据集id列表
        ...
        top_n = serializers.IntegerField(
            required=True, label=_("Reference segment number")
        )
        # 相似度 0-1之间
        similarity = serializers.FloatField(
            required=True, max_value=1, min_value=0, label=_("Similarity")
        )
