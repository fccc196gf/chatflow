...
def get_embedding_id(knowledge_id_list):
    ...
    if len(knowledge_list) == 0:
        raise Exception(
            _("The knowledge base setting is wrong, please reset the knowledge base")
        )
...
class BaseSearchDatasetStep(ISearchDatasetStep):
        ...
        **kwargs,
        ...
        if (
            ...
            knowledge_id_list = get_knowledge_list_of_authorized(
                manage.context.get("chat_user_id"), knowledge_id_list
            )
        ...
        embedding_model = ModelManage.get_model(
            model_id, lambda _id: get_model(model, **{**default_params})
        )
    ...
    def list_paragraph(embedding_list: List, vector):
        ...
        if len(hit_handling_method_paragraph) > 0:
            ...
            return [
                sorted(
                    hit_handling_method_paragraph,
                    key=lambda p: BaseSearchDatasetStep.get_similarity(
                        p, embedding_list
                    ),
                )[-1]
            ]
    ...
    def get_details(self, manage, **kwargs):
        ...
        return {
            ...
            "paragraph_list": [
                row.to_dict() for row in (self.context.get("paragraph_list") or [])
            ],
