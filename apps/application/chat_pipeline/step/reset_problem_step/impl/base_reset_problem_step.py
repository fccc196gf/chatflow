...
from application.chat_pipeline.step.reset_problem_step.i_reset_problem_step import (
    IResetProblemStep,
)
...
class BaseResetProblemStep(IResetProblemStep):
    def execute(
        self,
        problem_text: str,
        history_chat_record: List[ChatRecord] = None,
        model_id: str = None,
        ...
        **kwargs,
        ...
        chat_model = (
            get_model_instance_by_model_workspace_id(model_id, workspace_id)
        ...
        reset_prompt = (
            problem_optimization_prompt if problem_optimization_prompt else prompt
        )
        message_list = [
            *flat_map(history_message),
        ...
        if response.content.__contains__("<data>") and response.content.__contains__(
            "</data>"
        ):
            ...
            if (
                padding_problem_data is not None
                and len(padding_problem_data.strip()) > 0
            ):
    ...
    def get_details(self, manage, **kwargs):
        return {
            ...
            "model_id": (
                str(manage.context["model_id"])
                if "model_id" in manage.context
                else None
            ),
