...
class IResetProblemStep(IBaseChatPipelineStep):
    ...
    def _run(self, manage: PipelineManage):
        ...
        # 累加tokens
        manage.context["message_tokens"] = manage.context.get(
            "message_tokens", 0
        ) + self.context.get("message_tokens", 0)
        manage.context["answer_tokens"] = manage.context.get(
            "answer_tokens", 0
        ) + self.context.get("answer_tokens", 0)
