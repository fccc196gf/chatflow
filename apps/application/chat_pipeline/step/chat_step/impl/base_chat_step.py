...
from application.chat_pipeline.step.chat_step.i_chat_step import (
    IChatStep,
    PostResponseHandler,
)
...
def event_content(
    ...
    try:
        ...
        if not manage.debug:
            add_access_num(
                chat_user_id, chat_user_type, manage.context.get("application_id")
            )
    except Exception as e:
...
class BaseChatStep(IChatStep):
    ...
    def get_details(self, manage, **kwargs):
        return {
            ...
            "message_list": self.reset_message_list(
                self.context["step_args"].get("message_list"),
    ...
    def get_stream_result(
        ...
        if (
            directly_return_chunk_list is not None
            and len(directly_return_chunk_list) > 0
        ):
            return iter(directly_return_chunk_list), False
        elif (
            len(paragraph_list) == 0
            and no_references_setting.get("status") == "designated_answer"
        ):
            return (
                iter(
                    [
    ...
    def get_block_result(
        ...
        if (
            ...
            return (
                AIMessage(
                    no_references_setting.get("value").replace(
                        "{question}", problem_text
                    )
                ),
                False,
    ...
    def execute_block(
        ...
        reasoning_content_enable = model_setting.get("reasoning_content_enable", False)
        reasoning_content_start = model_setting.get(
        ...
        try:
            ...
            if is_ai_chat:
                ...
                response_token = 0
            write_context(
                self, manage, request_token, response_token, chat_result.content
            )
            reasoning_result = reasoning.get_reasoning_content(chat_result)
            reasoning_result_end = reasoning.get_end_reasoning_content()
            content = reasoning_result.get("content") + reasoning_result_end.get(
                "content"
            )
            if "reasoning_content" in chat_result.response_metadata:
                reasoning_content = (
                    chat_result.response_metadata.get("reasoning_content", "") or ""
                )
            else:
                reasoning_content = (
                    reasoning_result.get("reasoning_content") or ""
            ...
            return manage.get_base_to_response().to_block_response(
                ...
                {
                    "reasoning_content": (
                        reasoning_content if reasoning_content_enable else ""
                    ),
                    "answer_list": [
                        {
                            "content": content,
