...
from trigger.serializers.trigger_task import (
...
class TriggerTaskRecordResultSerializer(serializers.Serializer):
    id = serializers.UUIDField(
        required=True, help_text="任务记录id", label="任务记录id"
    )
    state = serializers.CharField(
        required=True, help_text="任务记录状态", label="任务记录状态"
    )
    source_type = serializers.CharField(
        required=True, help_text="资源类型", label="资源类型"
    )
    source_name = serializers.CharField(
        required=True, help_text="资源名称", label="资源名称"
    )
    source_id = serializers.CharField(required=True, help_text="资源id", label="资源id")
    task_record_id = serializers.CharField(
        required=True, help_text="资源任务记录id", label="资源任务记录id"
    )
    trigger_id = serializers.CharField(
        required=True, help_text="触发器id", label="触发器id"
    )
    type = serializers.CharField(required=True, help_text="资源类型", label="资源类型")
    create_time = serializers.CharField(
        required=True, help_text="创建时间", label="创建时间"
    )
    update_time = serializers.CharField(
        required=True, help_text="修改时间", label="修改时间"
    )
...
class TriggerTaskAPI(APIMixin):
    ...
    def get_system_parameters():
        return [
            parameter
            for parameter in TriggerTaskRecordExecutionDetailsAPI.get_parameters()
            if not parameter.name == "workspace_id"
        ]
