...
class TriggerQueryResponseSerializer(serializers.Serializer):
    ...
    workspace_id = serializers.CharField(
        required=True, help_text="触发器工作空间", label="触发器工作空间"
    )
    name = serializers.CharField(
        required=True, help_text="触发器名称", label="触发器名称"
    )
    desc = serializers.CharField(
        required=True, help_text="触发器描述", label="触发器描述"
    )
    trigger_type = serializers.CharField(
        required=True, help_text="触发器类型", label="触发器类型"
    )
    type = serializers.CharField(required=True, help_text="资源类型", label="资源类型")
    is_active = serializers.BooleanField(
        required=True, help_text="是否激活", label="是否激活"
    )
    source_name = serializers.CharField(
        required=True, help_text="资源类型", label="资源类型"
    )
    source_icon = serializers.CharField(
        required=True, help_text="资源图标", label="资源图标"
    )
...
class TriggerQueryPageAPI(APIMixin):
    ...
    def get_parameters():
        return [
            TriggerQueryAPI.get_parameters(),
            OpenApiParameter(
                name="current_page",
