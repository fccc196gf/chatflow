...
from trigger.serializers.task_source_trigger import (
    TaskSourceTriggerListSerializer,
    TaskSourceTriggerOperateSerializer,
    TaskSourceTriggerSerializer,
)
from trigger.serializers.trigger import TriggerQuerySerializer, TriggerOperateSerializer

from trigger.api.trigger import (
    TriggerCreateAPI,
    TriggerOperateAPI,
    TriggerEditAPI,
    TriggerBatchDeleteAPI,
    TriggerBatchActiveAPI,
    TaskSourceTriggerOperateAPI,
    TaskSourceTriggerAPI,
    TaskSourceTriggerCreateAPI,
    TriggerQueryAPI,
    TriggerQueryPageAPI,
)
...
def get_trigger_operation_object(trigger_id):
    ...
    if trigger_model is not None:
        return {"name": trigger_model.name}
...
def get_trigger_operation_object_batch(trigger_id_list):
    ...
    if trigger_model_list is not None:
        return {
            ...
            "trigger_list": [
                {"name": trigger_model.name, "type": trigger_model.type}
                for trigger_model in trigger_model_list
            ],
        }
...
class TriggerView(APIView):
    ...
    class Operate(APIView):
        ...
        @log(
            menu="Trigger",
            operate="Get trigger details",
            get_operation_object=lambda r, k: get_trigger_operation_object(
                k.get("trigger_id")
            ),
        )
    ...
    class BatchDelete(APIView):
        ...
        @log(
            menu="Trigger",
            operate="Delete trigger in batches",
            get_operation_object=lambda r, k: get_trigger_operation_object_batch(
                r.data.get("id_list")
            ),
        )
        def put(self, request: Request, workspace_id: str):
            return result.success(
                TriggerSerializer.Batch(
                    data={"workspace_id": workspace_id, "user_id": request.user.id}
    ...
    class BatchActivate(APIView):
        ...
        @log(
            menu="Trigger",
            operate="Activate trigger in batches",
            get_operation_object=lambda r, k: get_trigger_operation_object_batch(
                r.data.get("id_list")
            ),
        )
...
class TaskSourceTriggerView(APIView):
    ...
    @has_permissions(
        lambda r, kwargs: Permission(
            group=Group(kwargs.get("source_type")),
    ...
    @log(
        menu="Trigger",
        operate="Create trigger in source",
        get_operation_object=lambda r, k: r.data.get("name"),
    )
    def post(
    ...
    class Operate(APIView):
        ...
        @log(
            menu="Trigger",
            operate="Modify the source point trigger",
            get_operation_object=lambda r, k: get_trigger_operation_object(
                k.get("trigger_id")
            ),
        )
        ...
        @log(
            menu="Trigger",
            operate="Delete the source point trigger",
            get_operation_object=lambda r, k: get_trigger_operation_object(
                k.get("trigger_id")
            ),
        )
