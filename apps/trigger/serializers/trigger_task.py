...
class TriggerTaskQuerySerializer(serializers.Serializer):
    ...
    def get_query_set(self):
        query_set = (
            ...
            .filter(trigger_id=self.data.get("trigger_id"))
        )
        return query_set
...
class TriggerTaskRecordOperateSerializer(serializers.Serializer):
    ...
    trigger_task_record_id = serializers.CharField(
        required=True, label=_("Trigger task record ID")
    )
    ...
    def get_execution_details(self, is_valid=True):
        ...
        task_record = (
            ...
            .filter(
                ...
                id=self.data.get("trigger_task_record_id"),
            )
            .first()
        )
        if not task_record:
        ...
        if task_record.source_type == "APPLICATION":
            chat_record = (
                QuerySet(ChatRecord).filter(id=task_record.task_record_id).first()
            )
            if chat_record:
                return ChatRecordSerializerModel(chat_record).data
            return {"state": "TRIGGER_ERROR", "meta": task_record.meta}
        if task_record.source_type == "TOOL":
            tool_record = (
                QuerySet(ToolRecord).filter(id=task_record.task_record_id).first()
            )
            if tool_record:
                return {
                    "id": tool_record.id,
                    ...
                    "details": {
                        "tool_call": {
                            ...
                            "status": (
                                500
                                if tool_record.state == State.FAILURE
                                else 200 if tool_record.state == State.SUCCESS else 201
                            ),
                            "type": "tool-node",
...
class TriggerTaskRecordQuerySerializer(serializers.Serializer):
    ...
    state = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Trigger state")
    )
    name = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Trigger name")
    )
    source_type = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Source type")
    )
    order = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("Order field")
    )
    ...
    def get_query_set(self):
        ...
        if self.data.get("state"):
            trigger_query_set = trigger_query_set.filter(
                **{"ett.state": self.data.get("state")}
            )
        if self.data.get("name"):
            trigger_query_set = trigger_query_set.filter(
                **{"sdc.name__contains": self.data.get("name")}
            )
