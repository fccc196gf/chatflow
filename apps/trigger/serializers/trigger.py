    ...
    def is_valid(self, *, model=None, raise_exception=False):
        ...
        if model is not None:
            ...
            if len(model_list) != len(id_list):
                ...
                error_id_list = list(
                    filter(
                        lambda row_id: not model_id_list.__contains__(row_id), id_list
                    )
                )
                raise AppApiException(
                    500,
                    _("The following id does not exist: %s")
                    % ",".join(map(str, error_id_list)),
                )
...
class InputField(serializers.Serializer):
    ...
    value = ObjectField(
        required=True, label=_("Variable Value"), model_type_list=[str, list]
    )
        ...
        for key, val in value.items():
            ...
            if not serializer.is_valid():
                raise serializers.ValidationError(
                    {f"{field_name}.{key}": serializer.errors}
                )
        return value
...
class TriggerValidationMixin:
    ...
    def _validate_required_field(setting, field_name, trigger_type):
        if field_name not in setting:
            raise serializers.ValidationError(
                {
                    "trigger_setting": _("%s type requires %s field")
                    % (trigger_type, field_name)
                }
            )
    ...
    def _validate_number_range(values, field_name, min_val, max_val):
        for val in values:
            try:
                ...
                raise serializers.ValidationError(
                    {
                        "trigger_setting": _("%s values must be between %s and %s")
    ...
    def _validate_time_format(time_str):
        ...
        if not re.match(pattern, str(time_str)):
            raise serializers.ValidationError(
                {
                    "trigger_setting": _(
                        "Invalid time format: %s, must be HH:MM (e.g., 09:00)"
                    )
                    % time_str
                }
            )
    ...
    def _validate_scheduled_setting(self, setting):
        ...
        if schedule_type not in valid_types:
            raise serializers.ValidationError(
                {
                    "trigger_setting": _("schedule_type must be one of %s")
                    % ", ".join(valid_types)
                }
            )
    ...
    def _validate_interval(self, setting):
        ...
        try:
            ...
            raise serializers.ValidationError(
                {
                    "trigger_setting": _(
                        "interval_value must be an integer greater than or equal to 1"
                    )
                }
            )
        ...
        if interval_unit not in valid_units:
            raise serializers.ValidationError(
                {
                    "trigger_setting": _("interval_unit must be one of %s")
                    % ", ".join(valid_units)
                }
            )
...
class TriggerTaskEditRequest(serializers.Serializer):
    source_type = serializers.ChoiceField(
        required=False, choices=TriggerTaskTypeChoices
    )
...
class TriggerSerializer(serializers.Serializer):
        ...
        if trigger_tasks:
            ...
            trigger_task_models = [
                TriggerTask(
                    ...
                    is_active=is_active_map.get(
                        (task_data.get("source_type"), task_data.get("source_id"))
                    )
                    or False,
    ...
    def batch_get_source_active_status(trigger_tasks: list) -> Dict[tuple, bool]:
        ...
        for source_type, source_ids in source_ids_by_type.items():
            ...
            source_query_set = (
                QuerySet(source_model).filter(id__in=source_ids).values("id", field)
            )
    ...
    class Batch(serializers.Serializer):
        ...
        def batch_delete(self, instance: Dict, with_valid=True):
            ...
            if with_valid:
                BatchSerializer(data=instance).is_valid(
                    model=Trigger, raise_exception=True
                )
                self.is_valid(raise_exception=True)
            workspace_id = self.data.get("workspace_id")
            ...
            Trigger.objects.filter(
                workspace_id=workspace_id, id__in=trigger_id_list
            ).delete()
        ...
        def batch_switch(self, instance: Dict, with_valid=True):
            ...
            if with_valid:
                BatchActiveSerializer(data=instance).is_valid(
                    model=Trigger, raise_exception=True
                )
                self.is_valid(raise_exception=True)
            workspace_id = self.data.get("workspace_id")
...
class TriggerOperateSerializer(serializers.Serializer):
    ...
    def edit(self, instance: Dict, with_valid=True):
        ...
        trigger_direct_edit_field_list = [
            "name",
            "desc",
            "trigger_type",
            "trigger_setting",
        ...
        trigger_deploy_edit_field_list = [
            "trigger_type",
            "trigger_setting",
            "is_active",
        ]
        # is need to redeploy
        need_redeploy = any(
            field in instance for field in trigger_deploy_edit_field_list
        )
        ...
        if trigger_tasks is not None:
            ...
            if not trigger_tasks:
                raise serializers.ValidationError(
                    _("Trigger must have at least one task")
                )

            is_active_map = TriggerSerializer.batch_get_source_active_status(
                trigger_tasks
            )

            trigger_task_model_list = [
                TriggerTask(
    ...
    def delete(self):
        ...
        trigger = (
            QuerySet(Trigger)
            .filter(workspace_id=self.data.get("workspace_id"), id=trigger_id)
            .first()
        )
        if trigger:
            undeploy(TriggerModelSerializer(trigger).data, **{})
    ...
    def one(self, with_valid=True):
        ...
        workspace_id = self.data.get("workspace_id")
        trigger = (
            QuerySet(Trigger).filter(workspace_id=workspace_id, id=trigger_id).first()
        )
        ...
        if application_ids:
            applications = Application.objects.filter(
                workspace_id=workspace_id, id__in=application_ids
            )
            application_task_list = ApplicationTriggerTaskSerializer(
...
class TriggerQuerySerializer(serializers.Serializer):
    name = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("Trigger name")
    )
    type = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Trigger type")
    )
    is_active = serializers.BooleanField(
        required=False, allow_null=True, label=_("Is active")
    )
    task = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Trigger task")
    )
    create_user = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Create user")
    )
    workspace_id = serializers.CharField(required=True, label=_("workspace id"))
    ...
    def get_query_set(self):
        ...
        trigger_query_set = trigger_query_set.filter(
            **{"t.workspace_id": self.data.get("workspace_id")}
        )
        if self.data.get("name"):
            trigger_query_set = trigger_query_set.filter(
                **{"t.name__icontains": self.data.get("name")}
            )
        ...
        if self.data.get("is_active") is not None:
            trigger_query_set = trigger_query_set.filter(
                **{"t.is_active": self.data.get("is_active")}
            )
        if self.data.get("task"):
            task_query_set = task_query_set.filter(
                trigger_task_str__icontains=self.data.get("task")
            )
        if self.data.get("create_user"):
            trigger_query_set = trigger_query_set.filter(
                **{"t.user_id": self.data.get("create_user")}
            )

        return {
            "trigger_query_set": trigger_query_set,
            "task_query_set": task_query_set,
        }
    ...
    def page(self, current_page: int, page_size: int, with_valid=True):
        ...
        return native_page_search(
            ...
            get_file_content(
                os.path.join(
                    PROJECT_DIR, "apps", "trigger", "sql", "get_trigger_page_list.sql"
                )
            ),
        )
