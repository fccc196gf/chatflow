...
from trigger.serializers.trigger import (
    TriggerModelSerializer,
    TriggerSerializer,
    ApplicationTriggerTaskSerializer,
    ToolTriggerTaskSerializer,
    TriggerTaskModelSerializer,
)
...
class TaskSourceTriggerSerializer(serializers.Serializer):
    ...
    def insert(self, instance, with_valid=True):
        ...
        if not (
            instance.get("source_id") == source_id
            and source_trigger_task.get("source_type") == source_type
        ):
...
class TaskSourceTriggerOperateSerializer(serializers.Serializer):
    ...
    def one(self, with_valid=True):
        ...
        trigger_task = TriggerTaskModelSerializer(
            TriggerTask.objects.filter(
                trigger_id=trigger_id, source_id=source_id, source_type=source_type
            ).first()
        ).data
        ...
        if source_type == TriggerTaskTypeChoices.APPLICATION:
            application_task = ApplicationTriggerTaskSerializer(
                Application.objects.filter(
                    workspace_id=workspace_id, id=source_id
                ).first()
            ).data
            return {
        ...
        if source_type == TriggerTaskTypeChoices.TOOL:
            tool_task = ToolTriggerTaskSerializer(
                Tool.objects.filter(workspace_id=workspace_id, id=source_id).first()
            ).data
            return {
                **TriggerModelSerializer(trigger).data,
                "trigger_task": trigger_task,
    ...
    def edit(self, instance: Dict, with_valid=True):
        ...
        task_source_trigger_edit_field_list = [
            "name",
            "desc",
            "trigger_type",
            "trigger_setting",
        ...
        if trigger_task is not None:
            ...
            TriggerTask.objects.filter(
                source_id=source_id, source_type=source_type, trigger_id=trigger_id
            ).update(
                parameter=trigger_task[0].get("parameter"),
                meta=trigger_task[0].get("meta"),
            )
        else:
    ...
    def delete(self):
        ...
        delete_count = TriggerTask.objects.filter(
            trigger_id=trigger_id, source_id=source_id, source_type=source_type
        ).delete()[0]
        if delete_count == 0:
