...
def get_application_execute_parameters(
    parameter_setting, application_parameters_setting, kwargs
):
        ...
        if setting:
            if many_field.__contains__(key):
                for ck, cv in value.items():
                    ...
                    if _setting:
                        _value = get_field_value(
                            _setting,
                            kwargs,
                            cv.get("type"),
                            cv.get("required"),
                            cv.get("default_value"),
                            ck,
                        )
            ...
            if value.get("default_value"):
                parameters["message" if key == "question" else key] = value.get(
                    "default_value"
                )
            else:
                if value.get("required"):
                    raise Exception(
                        f'{"message" if key == "question" else key} is required'
                    )
...
def get_workflow_state(details):
    ...
    err = any(
        [
            True
            for value in all_node
            if value.get("status") == 500 and not value.get("enableException")
        ]
    )
    if err:
...
def get_application_parameters_setting(application):
    application_parameter_setting = {"question": {"required": True, "type": "string"}}
    if application.type == "SIMPLE":
        return application_parameter_setting
    else:
        base_node_list = [
            n
            for n in application.work_flow.get("nodes")
            if n.get("type") == "base-node"
        ]
        if len(base_node_list) == 0:
            raise Exception("Incorrect application workflow information")
        base_node = base_node_list[0]
        api_input_field_list = (
            base_node.get("properties").get("api_input_field_list") or []
        )
        api_input_field_list = {
            user_field.get("variable"): {
        ...
        user_input_field_list = (
            base_node.get("properties").get("user_input_field_list") or []
        )
        user_input_field_list = {
            user_field.get("field"): {
                "required": user_field.get("required"),
                "default_value": user_field.get("default_value"),
                "type": get_user_field_component_input_type(
                    user_field.get("input_type")
                ),
            }
        ...
        if file_upload_enable:
            ...
            for field in ["audio", "document", "image", "other", "video"]:
                ...
                if v:
                    application_parameter_setting[field + "_list"] = {
                        "required": False,
                        "default_value": [],
                        "type": "array",
                    }
    ...
    def execute(self, trigger_task, **kwargs):
        ...
        try:
            application = (
                ...
                .filter(id=trigger_task.get("source_id"))
                .only("type", "work_flow")
                .first()
            )
            ...
            application_parameters_setting = get_application_parameters_setting(
                application
            )
            parameters = get_application_execute_parameters(
                parameter_setting, application_parameters_setting, kwargs
            )
            ...
            Chat.objects.get_or_create(
                ...
                defaults={
                    ...
                    "source": {"type": ChatSourceChoices.TRIGGER.value},
                },
            )
            ...
            chatflow_logger.error(
                f"Application execution error: {traceback.format_exc()}"
            )
            QuerySet(TaskRecord).filter(id=task_record_id).update(
                state=State.FAILURE,
                run_time=time.time() - start_time,
                meta={
                    "input": {"parameter_setting": parameter_setting, **kwargs},
                    "output": None,
