...
def get_tool_execute_parameters(input_field_list, parameter_setting, kwargs):
    type_map = {
        f.get("name"): f.get("type") for f in (input_field_list or []) if f.get("name")
    }
...
class ToolTask(BaseTriggerTask):
    ...
    def execute(self, trigger_task, **kwargs):
        ...
        try:
            ...
            if not tool:
                chatflow_logger.info(f"Tool with id {tool_id} not found or inactive.")
                return
            ...
            parameters = get_tool_execute_parameters(
                tool.input_field_list, parameter_setting, kwargs
            )
            init_params_default_value = {
                i["field"]: i.get("default_value") for i in tool.init_field_list
            }

            if tool.init_params is not None:
                all_params = (
                    init_params_default_value
                    | json.loads(rsa_long_decrypt(tool.init_params))
                    | parameters
                )
            else:
            ...
            chatflow_logger.debug(f"Tool execution result: {result}")
