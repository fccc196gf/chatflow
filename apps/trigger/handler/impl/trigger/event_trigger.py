    ...
    if not isinstance(value, instance_type):
        raise Exception(
            _("Field: {name} Type: {_type} Value: {value} Type error").format(
                name=desc, _type=_type, value=value
            )
        )
    return value
...
class EventTriggerView(APIView):
    ...
    @extend_schema(
        ...
        request={"application/json": {"schema": {"type": "object", "example": {}}}},
...
class EventTrigger(BaseTrigger):
    ...
    def execute(trigger, request=None, **kwargs):
        ...
        trigger_task_list = [
            TriggerTaskResponse(trigger_task).data
            for trigger_task in QuerySet(TriggerTask).filter(
                trigger__id=trigger.get("id"), is_active=True
            )
        ]
