...
def _deploy_daily(
    ...
    for t in times:
        try:
            ...
            chatflow_logger.warning(f"invalid time={t}, trigger_id={trigger_id}")
            continue

        for task in trigger_tasks:
            job_id = (
                f"trigger:{trigger_id}:task:{task['id']}:daily:{hour:02d}{minute:02d}"
            )
            scheduler.add_job(
...
def _deploy_weekly(
    ...
    if not times or not days:
        chatflow_logger.warning(f"empty weekly setting, trigger_id={trigger_id}")
        return
    ...
    for d in days:
        try:
            ...
            chatflow_logger.warning(f"invalid weekday={d}, trigger_id={trigger_id}")
            continue
...
def _deploy_monthly(
    ...
    days = setting.get("days") or []
    if not times or not days:
        chatflow_logger.warning(f"empty monthly setting, trigger_id={trigger_id}")
        return
    ...
    for d in days:
        try:
            ...
            chatflow_logger.warning(f"invalid day={d}, trigger_id={trigger_id}")
            continue
...
def _deploy_interval(
    ...
    try:
        ...
        chatflow_logger.warning(
            f"invalid interval_value={value}, trigger_id={trigger_id}"
        )
        return

    if unit not in {"seconds", "minutes", "hours", "days"}:
        chatflow_logger.warning(
            f"invalid interval_unit={unit}, trigger_id={trigger_id}"
        )
        return
    ...
    for job in scheduler.get_jobs():
        if getattr(job, "id", "").startswith(prefix):
            try:
                ...
                chatflow_logger.warning(f"remove job failed, job_id={job.id}, err={e}")


@celery_app.task(name="celery:deploy_scheduled_trigger")
def deploy_scheduled_trigger(
    trigger: dict, trigger_tasks: list[dict], setting: dict, schedule_type: str
) -> None:
    ...
    if not fn:
        chatflow_logger.warning(
            f"unsupported schedule_type={schedule_type}, trigger_id={trigger['id']}"
        )
        return
...
class ScheduledTrigger(BaseTrigger):
    ...
    def execute(trigger, **kwargs):
        ...
        if not trigger_task:
            chatflow_logger.warning(f"unsupported task={trigger_task}")
            return
        ...
        if source_type == "APPLICATION":
            ...
            chatflow_logger.warning(
                f"unsupported source_type={source_type}, task_id={trigger_task['id']}"
            )
    ...
    def deploy(self, trigger, **kwargs):
        ...
        if not trigger_tasks:
            chatflow_logger.warning(f"no active trigger_tasks, trigger_id={trigger_id}")
            self.undeploy(trigger, **kwargs)
            return
