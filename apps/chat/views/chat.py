    ...
    def get(self, request: Request):
        ...
        try:
            ...
            response = requests.get(
                image_url, stream=True, allow_redirects=True, timeout=10  # 启用流式响应
            )
            content_type = response.headers.get("Content-Type", "").split(";")[0]
            # 创建Django流式响应
            django_response = StreamingHttpResponse(
                stream_image(response), content_type=content_type  # 使用生成器
            )
...
class AuthProfile(APIView):
    ...
    def get(self, request: Request):
        return result.success(
            AuthProfileSerializer(
                data={"access_token": request.query_params.get("access_token")}
            ).profile()
        )
    ...
    def post(self, request: Request, chat_id: str):
        ...
        return ChatSerializers(
            data={
                ...
                "source": {
                    "type": (
                        ChatSourceChoices.API_CALL.value
                        if request.auth.chat_user_type
...
class CaptchaView(APIView):
    ...
    def get(self, request: Request):
        ...
        return result.success(
            CaptchaSerializer().chat_generate(username, "chat", accessToken)
        )
...
class UploadFile(APIView):
    ...
    def post(self, request: Request, chat_id: str):
        ...
        for file in files:
            ...
            file_ids.append(
                {"name": file.name, "url": file_url, "file_id": file_url.split("/")[-1]}
            )
