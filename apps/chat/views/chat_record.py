...
from chat.api.chat_api import (
    HistoricalConversationAPI,
    PageHistoricalConversationAPI,
    PageHistoricalConversationRecordAPI,
    HistoricalConversationRecordAPI,
    HistoricalConversationOperateAPI,
)
from chat.api.vote_api import VoteAPI
from chat.serializers.chat_record import (
    VoteSerializer,
    HistoricalConversationSerializer,
...
class HistoricalConversationView(APIView):
    ...
    class BatchDelete(APIView):
        ...
        def delete(self, request: Request):
            return result.success(
                HistoricalConversationOperateSerializer.Clear(
                    data={
                        "application_id": request.auth.application_id,
                        "chat_user_id": request.auth.chat_user_id,
                    }
...
class HistoricalConversationRecordView(APIView):
    ...
    class PageView(APIView):
        ...
        def get(
            self, request: Request, chat_id: str, current_page: int, page_size: int
        ):
            return result.success(
                HistoricalConversationRecordSerializer(
                    data={
