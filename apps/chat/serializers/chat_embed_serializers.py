...
class ChatEmbedSerializer(serializers.Serializer):
    ...
    def get_embed(self, with_valid=True, params=None):
        ...
        application_access_token = (
            ...
            .filter(access_token=self.data.get("token"))
            .first()
        )
        is_draggable = "false"
        show_guide = "true"
        float_icon = f"{self.data.get('protocol')}://{self.data.get('host')}{CONFIG.get_chat_path()}/ChatFlow.gif"
        print("float_icon", float_icon)
        is_license_valid = DatabaseModelManage.get_model("license_is_valid")
        X_PACK_LICENSE_IS_VALID = (
            is_license_valid() if is_license_valid is not None else False
        )
        # 获取接入的query参数
        ...
        if application_setting_model is not None and X_PACK_LICENSE_IS_VALID:
            ...
            if application_setting is not None:
                ...
                if (
                    application_setting.float_icon is not None
                    and len(application_setting.float_icon) > 0
                ):
                    float_icon = (
                        application_setting.float_icon[1:]
                        if application_setting.float_icon.startswith(".")
                ...
                if (
                    application_setting.custom_theme is not None
                    and len(
                        application_setting.custom_theme.get(
                            "header_font_color", "rgb(100, 106, 115)"
                        )
                    )
                    > 0
                ):
                    header_font_color = application_setting.custom_theme.get(
                        "header_font_color", "rgb(100, 106, 115)"
                    )
        ...
        is_auth = (
            ...
            and application_access_token.is_active
            else "false"
        )
        t = Template(content)
        s = t.render(
            Context(
                {
                    ...
                    "white_list_str": ",".join(
                        ...
                        if application_access_token.white_list is not None
                        else []
                    ),
                    "white_active": (
                        "true" if application_access_token.white_active else "false"
                    ),
                    ...
                    "chat_flow_id": str(uuid.uuid7()).replace("-", ""),
                    "header_font_color": header_font_color,
                }
            )
        )
        response = HttpResponse(
            s, status=200, headers={"Content-Type": "text/javascript"}
        )
        return response
            ...
            if work_flow is not None:
                for node in work_flow.get("nodes", []):
                    if node["id"] == "base-node":
                        input_field_list = node.get("properties", {}).get(
                            "api_input_field_list",
                            node.get("properties", {}).get("input_field_list", []),
                        )
                        if input_field_list is not None:
                            for field in input_field_list:
                                if (
                                    field["assignment_method"] == "api_input"
                                    and field["variable"] in params
                                ):
