...
class AuthProfileSerializer(serializers.Serializer):
    ...
    def profile(self):
        ...
        if application_setting_model and chat_platform:
            ...
            types = (
                QuerySet(chat_platform)
                .filter(is_active=True, is_valid=True)
                .values_list("auth_type", flat=True)
            )
            login_value = application_access_token.authentication_value.get(
                "login_value", []
            )
            max_attempts = application_access_token.authentication_value.get(
                "max_attempts", 1
            )
...
class ApplicationProfileSerializer(serializers.Serializer):
    ...
    def profile(self, with_valid=True):
        ...
        base_node = [
            node
            for node in ((application.work_flow or {}).get("nodes", []) or [])
