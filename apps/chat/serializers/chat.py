...
class GeneratePromptSerializers(serializers.Serializer):
    ...
    messages = serializers.ListSerializer(
        child=ChatMessagesSerializers(), required=True, label=_("Chat context")
    )
        ...
        for index in range(len(messages)):
            ...
            if role == "ai" and index % 2 != 1:
                raise AppApiException(
                    400,
                    _(
                        "Authentication failed. Please verify that the parameters are correct."
                    ),
                )
...
class ChatMessageSerializers(serializers.Serializer):
    ...
    stream = serializers.BooleanField(
        required=True, label=_("Is the answer in streaming mode")
    )
    re_chat = serializers.BooleanField(
        required=True, label=_("Do you want to reply again")
    )
    chat_record_id = serializers.UUIDField(
        required=False, allow_null=True, label=_("Conversation record id")
    )

    node_id = serializers.CharField(
        required=False, allow_null=True, allow_blank=True, label=_("Node id")
    )
    ...
    node_data = serializers.DictField(
        required=False, allow_null=True, label=_("Node parameters")
    )
    ...
    child_node = serializers.DictField(
        required=False, allow_null=True, label=_("Child Nodes")
    )
...
SYSTEM_ROLE = get_file_content(
    os.path.join(PROJECT_DIR, "apps", "chat", "template", "generate_prompt_system")
)
...
class PromptGenerateSerializer(serializers.Serializer):
    ...
    application_id = serializers.CharField(
        required=False, allow_blank=True, allow_null=True, label=_("Application")
    )
    ...
    def generate_prompt(self, instance: dict):
        ...
        model_exist = (
            QuerySet(Model)
            .filter(id=model_id, model_type__in=SUPPORTED_MODEL_TYPES)
            .exists()
        )
        if not model_exist:
...
class ChatSerializers(serializers.Serializer):
    ...
    application_id = serializers.UUIDField(
        required=True, allow_null=True, label=_("Application ID")
    )
    debug = serializers.BooleanField(required=False, label=_("Debug"))
    ip_address = serializers.CharField(
        required=False, label=_("IP Address"), allow_null=True, allow_blank=True
    )
    source = serializers.JSONField(required=False, label=_("Source"))
    ...
    def is_valid_chat_id(self, chat_info: ChatInfo):
        if self.data.get("application_id") is not None and self.data.get(
            "application_id"
        ) != str(chat_info.application_id):
    ...
    def is_valid_intraday_access_num(self):
        if not self.data.get("debug") and [
            ...
            if (
                application_access_token.access_num > 0
                and application_access_token.access_num
                <= access_client.intraday_access_num
            ):
                raise AppChatNumOutOfBoundsFailed(
                    1002, _("The number of visits exceeds today's visits")
                )

    def is_valid_application_simple(
        ...
        if re_chat:
            paragraph_id_list = flat_map(
                [
                    [
                        paragraph.get("id")
                        for paragraph in chat_record.details["search_step"][
                            "paragraph_list"
                        ]
                    ]
    ...
    def get_chat_record(chat_info, chat_record_id):
        ...
        chat_record = (
            QuerySet(ChatRecord)
            .filter(id=chat_record_id, chat_id=chat_info.chat_id)
            .first()
        )
    ...
    def chat_work_flow(self, chat_info: ChatInfo, instance: dict, base_to_response):
        ...
        work_flow_manage = WorkflowManage(
            Workflow.new_instance(work_flow),
            {
                ...
                "chat_record_id": (
                    str(uuid.uuid7()) if chat_record_id is None else str(chat_record_id)
                ),
                "stream": stream,
                "re_chat": re_chat,
                "chat_user_id": chat_user_id,
            ...
            instance.get("node_data"),
            chat_record,
            instance.get("child_node"),
        )
        chat_info.set_chat(message)
    ...
    def re_open_chat_simple(self, chat_id, application):
        ...
        chat_record_list = list(
            QuerySet(ChatRecord).filter(chat_id=chat_id).order_by("-create_time")[0:5]
        )
