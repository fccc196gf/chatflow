...
class VoteRequest(serializers.Serializer):
    vote_status = serializers.ChoiceField(
        choices=VoteChoices.choices, label=_("Bidding Status")
    )
    vote_reason = serializers.ChoiceField(
        choices=VoteReasonChoices.choices,
        label=_("Vote Reason"),
        required=False,
        allow_null=True,
    )

    vote_other_content = serializers.CharField(
        required=False, allow_blank=True, label=_("Vote other content")
    )
...
class HistoryChatModel(serializers.ModelSerializer):
    class Meta:
        model = Chat
        fields = ["id", "application_id", "abstract", "create_time", "update_time"]
...
class VoteSerializer(serializers.Serializer):
    ...
    def vote(self, instance: Dict, with_valid=True):
        ...
        try:
            chat_record_details_model = QuerySet(ChatRecord).get(
                id=self.data.get("chat_record_id"), chat_id=self.data.get("chat_id")
            )
            if chat_record_details_model is None:
                raise AppApiException(
                    500, gettext("Non-existent conversation chat_record_id")
                )
            ...
            # 已投票状态
                ...
                if vote_status == VoteChoices.UN_VOTE:
                    ...
                    raise AppApiException(
                        500,
                        gettext(
                            "Already voted, please cancel first and then vote again"
                        ),
                    )
        finally:
...
class HistoricalConversationSerializer(serializers.Serializer):
    ...
    def page(self, current_page, page_size):
        ...
        return page_search(
            ...
            page_size,
            self.get_queryset(),
            lambda r: HistoryChatModel(r).data,
        )
...
class HistoricalConversationOperateSerializer(serializers.Serializer):
    ...
    def edit_abstract(self, instance, with_valid=True):
        ...
        QuerySet(Chat).filter(
            ...
            application_id=self.data.get("application_id"),
            chat_user_id=self.data.get("chat_user_id"),
        ).update(abstract=instance.get("abstract"))
        return True
...
class HistoricalConversationRecordSerializer(serializers.Serializer):
    ...
    def is_valid(self, *, raise_exception=False):
        ...
        chat_exist = (
            QuerySet(Chat)
            .filter(
                application_id=application_id, chat_user_id=chat_user_id, id=chat_id
            )
            .exists()
        )
        if not chat_exist:
    ...
    def page(self, current_page, page_size):
        ...
        ).page(current_page, page_size, show_source=show_source, show_exec=show_exec)
