...
def exist_role_by_role_constants(
    ...
    """
    return any(
        [True for role in role_list if user_role.__contains__(role.value.__str__())]
    )
...
def exist_permissions_by_view_permission(
    ...
    **kwargs,
    ...
    role_list = [
        user_r(request, kwargs) if callable(user_r) else user_r
        for user_r in permission.roleList
    ]
    role_ok = any(list(map(lambda up: role_list.__contains__(up), user_role)))
    permission_list = [
