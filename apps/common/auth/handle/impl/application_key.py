...
class ApplicationKey(AuthBaseHandle):
    def handle(self, request, token: str, get_token_details):
        application_api_key = (
            QuerySet(ApplicationApiKey).filter(secret_key=token).first()
        )
        ...
        if not application_api_key.is_active:
            raise AppAuthenticationFailed(500, _("Secret key is invalid"))
        if (
            application_api_key.is_permanent is False
            and application_api_key.expire_time < timezone.now()
        ):
            raise AppAuthenticationFailed(500, _("Secret key is expired"))
        application_access_token = (
            QuerySet(ApplicationAccessToken)
            .filter(application_id=application_api_key.application_id)
            .first()
        )
        if application_access_token is not None:
            if application_access_token.authentication:
                if (
                    application_access_token.authentication_value.get(
                        "type", "password"
                    )
                    != "password"
                ):
