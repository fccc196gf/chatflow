...
from common.constants.permission_constants import (
    ...
    get_default_workspace_user_role_mapping_list,
    RoleConstants,
    ResourcePermission,
    Resource,
    WorkspaceGroup,
...
from chatflow.const import CONFIG
from system_manage.models.workspace_user_permission import (
...
def get_workspace_permission(permission_id, workspace_id, role=None):
    ...
    if role and role.type == RoleConstants.WORKSPACE_MANAGE.value.__str__():
        return [
            f"{permission_id}:/WORKSPACE/{workspace_id}:ROLE/{role.type}",
...
def get_workspace_permission_list(
    role_permission_mapping_dict, workspace_user_role_mapping_list, role_model_dict
):
    """
    获取工作空间下所有的权限
    ...
    workspace_permission_list = [
        [
            get_workspace_permission(
                role_permission_mapping.permission_id,
                w_u_r.workspace_id,
                role_model_dict.get(w_u_r.role_id, None),
            )
            for role_permission_mapping in role_permission_mapping_dict.get(
                w_u_r.role_id, []
            )
        ]
        for w_u_r in workspace_user_role_mapping_list
    ]
    return reduce(
        lambda x, y: [*x, *y],
        reduce(lambda x, y: [*x, *y], workspace_permission_list, []),
        [],
    )
...
def get_workspace_resource_permission_list(
    ...
    resource_permission_list = [
        ...
        for workspace_user_resource_permission in workspace_user_resource_permission_list
    ]
    # 将二维数组扁平为一维
...
def get_workspace_resource_permission_list_by_workspace_user_permission(
    ...
    role_permission_mapping_list = [
        ...
        for workspace_user_role_mapping in workspace_user_role_mapping_dict.get(
            workspace_user_resource_permission.workspace_id
        )
    ]
    role_permission_mapping_list = reduce(
        lambda x, y: [*x, *y], role_permission_mapping_list, []
    )
    # 如果是根据角色
    if (
        workspace_user_resource_permission.auth_type == ResourceAuthType.ROLE
        ...
        workspace_user_resource_permission.auth_type
        == ResourceAuthType.RESOURCE_PERMISSION_GROUP
    ):
        resource_permission_list = [
            [
                ...
                for permission in get_permission_list_by_resource_group(
                    ResourcePermissionGroup(
                        Resource(workspace_user_resource_permission.auth_target_type),
                        ResourcePermission(resource_permission),
                    )
                )
            ]
            for resource_permission in workspace_user_resource_permission.permission_list
            if ResourcePermission.values.__contains__(resource_permission)
        ]
        # 将二维数组扁平为一维
        return reduce(lambda x, y: [*x, *y], resource_permission_list, [])
    return []
...
def get_permission_list(
    ...
    if permission_list is None:
        if is_query_model:
            ...
            workspace_user_role_mapping_dict = group_by(
                workspace_user_role_mapping_list, lambda item: item.workspace_id
            )
            role_id_list = list(
                set(
                    [
            ...
            # 获取角色权限映射数据
            role_permission_mapping_list = QuerySet(
                role_permission_mapping_model
            ).filter(role_id__in=role_id_list)
            role_model_list = QuerySet(role_model).filter(id__in=role_id_list)

            role_model_dict = {
                role_model.id: role_model for role_model in role_model_list
            }
            ...
            workspace_user_permission_list = QuerySet(
                WorkspaceUserResourcePermission
            ).filter(
                workspace_id__in=[
            ...
            workspace_resource_permission_list = get_workspace_resource_permission_list(
                workspace_user_permission_list,
                role_permission_mapping_dict,
                workspace_user_role_mapping_dict,
            )

            workspace_permission_list = get_workspace_permission_list(
                role_permission_mapping_dict,
            ...
            permission_list = (
                system_permission_list
                + workspace_permission_list
                + workspace_resource_permission_list
            )
            permission_list = list(set(permission_list))
            cache.set(key, permission_list, version=version)
        else:
            workspace_id_list = ["default"]
            workspace_user_resource_permission_list = QuerySet(
                WorkspaceUserResourcePermission
            ).filter(workspace_id__in=workspace_id_list, user_id=user_id)
            role_permission_mapping_list = get_default_role_permission_mapping_list()
            role_permission_mapping_dict = group_by(
                role_permission_mapping_list, lambda item: item.role_id
            )
            workspace_user_role_mapping_list = (
                get_default_workspace_user_role_mapping_list([user.role])
            )
            workspace_user_role_mapping_dict = group_by(
                workspace_user_role_mapping_list, lambda item: item.workspace_id
            )
...
def reset_workspace_role(role_id, workspace_id, role_dict):
    if system_role_list.__contains__(role_id):
        ...
        return [
            f"EXTENDS_{role_type}:/WORKSPACE/{workspace_id}",
            f"EXTENDS_{role_type}",
        ]
...
def get_role_list(
    ...
    if workspace_list is None:
        if is_query_model:
            ...
            role_list = QuerySet(role_model).filter(
                id__in=[wurm.role_id for wurm in workspace_user_role_mapping_list]
            )
            ...
            if user.role == RoleConstants.ADMIN.value.__str__():
                role_list = [
                    user.role,
                    get_role_permission(RoleConstants.WORKSPACE_MANAGE, "default"),
                ]
            else:
                role_list = [
                    user.role,
                    get_role_permission(RoleConstants.USER, "default"),
                ]
...
class UserToken(AuthBaseHandle):
    def support(self, request, token: str, get_token_details):
        ...
        return (
            "id" in auth_details
            and auth_details.get("type") == AuthenticationType.SYSTEM_USER.value
        )
