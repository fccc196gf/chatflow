...
class ChatAnonymousUserToken(AuthBaseHandle):
    ...
    def handle(self, request, token: str, get_token_details):
        ...
        if application_access_token.authentication and ["PE", "EE"].__contains__(
            edition
        ):
            if (
                chat_user_token.authentication.auth_type
                != application_access_token.authentication_value.get("type", "")
            ):
                raise AppAuthenticationFailed(
                    1002, _("Authentication information is incorrect")
                )
