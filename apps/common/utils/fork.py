...
class ForkManage:
    ...
    def fork_child(
        ...
        if level < 0:
            ...
            child_url = (
                child_link.url[:-1] if child_link.url.endswith("/") else child_link.url
            )
        if not exclude_link_url.__contains__(child_url):
            ...
            for child_link in response.child_link_list:
                child_url = (
                    child_link.url[:-1]
                    if child_link.url.endswith("/")
                    else child_link.url
                )
                if not exclude_link_url.__contains__(child_url):
                    ForkManage.fork_child(
                        child_link,
                        selector_list,
                        level - 1,
                        exclude_link_url,
...
class Fork:
    class Response:
        def __init__(
            self, content: str, child_link_list: List[ChildLink], status, message: str
        ):
    ...
    def __init__(self, base_fork_url: str, selector_list: List[str]):
        ...
        self.base_fork_url = urlunparse(
            (parsed.scheme, parsed.netloc, path, None, None, None)  # fragment
        )
    ...
    def get_child_link_list(self, bf: BeautifulSoup):
        pattern = (
            "^((?!(http:|https:|tel:/|#|mailto:|javascript:))|"
            + self.base_fork_url
            + "|/).*"
        )
    ...
    def reset_url(tag, field, base_fork_url):
        ...
        if field_value.startswith("/"):
            ...
            result_url = urljoin(
                base_fork_url
                + "/"
                + (field_value if field_value.endswith("/") else field_value + "/"),
                ".",
            )
        ...
        if len(charset_list) > 0:
            ...
            if charset != encoding:
                try:
                    ...
                    chatflow_logger.error(f"{e}: {traceback.format_exc()}")
        ...
        for meta in meta_list:
            if meta.attrs is not None:
                if "charset" in meta.attrs:
                    ...
                    meta.attrs.get("http-equiv", "").lower() == "content-type"
    ...
    def fork(self):
        try:
            ...
            chatflow_logger.info(f"fork:{self.base_fork_url}")
            response = requests.get(self.base_fork_url, verify=False, headers=headers)
            if response.status_code != 200:
                chatflow_logger.error(
                    f"url: {self.base_fork_url} code:{response.status_code}"
                )
                return Fork.Response.error(
                    f"url: {self.base_fork_url} code:{response.status_code}"
                )
...
def handler(base_url, response: Fork.Response):
    chatflow_logger.info(
        base_url.url, base_url.tag.text if base_url.tag else None, response.content
    )
