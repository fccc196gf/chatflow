...
from chatflow.const import BASE_DIR, CONFIG
...
_sandbox_path = (
    CONFIG.get("SANDBOX_HOME", "/opt/chatflow-app/sandbox")
    if _enable_sandbox
    else os.path.join(PROJECT_DIR, "data", "sandbox")
)
_sandbox_python_sys_path = CONFIG.get_sandbox_python_package_paths().split(",")
_process_limit_timeout_seconds = int(
    CONFIG.get("SANDBOX_PYTHON_PROCESS_LIMIT_TIMEOUT_SECONDS", "3600")
)
...
class ToolExecutor:
    ...
    def init_sandbox_dir():
        ...
        try:
            # 只初始化一次
            fd = os.open(
                os.path.join(PROJECT_DIR, "tmp", "tool_executor_init_dir.lock"),
            ...
            # 文件已存在 → 已初始化过
            return
        chatflow_logger.info("Init sandbox dir.")
        try:
            os.system("chmod -R g-rwx /dev/shm /dev/mqueue")
            os.system("chmod o-rwx /run/postgresql")
        except Exception as e:
            chatflow_logger.warning(f"Exception: {e}", exc_info=True)
            pass
        ...
        if banned_hosts:
            ...
            banned_hosts = ",".join(
                s.strip()
                for s in banned_hosts.split(",")
                if s.strip() and s.strip().lower() != hostname.lower()
            )
        with open(sandbox_conf_file_path, "w") as f:
            f.write(f"SANDBOX_PYTHON_BANNED_HOSTS={banned_hosts}\n")
            f.write(
                f"SANDBOX_PYTHON_ALLOW_DL_PATHS={','.join(sorted(set(filter(None, sys.path + _sandbox_python_sys_path + allow_dl_paths.split(',')))))}\n"
            )
    ...
    def exec_code(self, code_str, keywords, function_name=None):
        _id = str(uuid.uuid7())
        action_function = (
            f"({function_name !a}, locals_v.get({function_name !a}))"
            if function_name
            else "locals_v.popitem()"
        )
        set_run_user = (
...
try:
    ...
    path_to_exclude = ['/opt/py3/lib/python3.11/site-packages', '/opt/chatflow-app/apps']
...
sys.stdout.flush()
"""
        chatflow_logger.debug(f"Sandbox execute code: {_exec_code}")
        ...
        if subprocess_result.returncode != 0:
            raise Exception(
                ...
                or subprocess_result.stdout
                or "Unknown exception occurred"
            )
    ...
    def _generate_mcp_server_code(
        ...
        for node in tree.body:
            if isinstance(node, ast.Import) or isinstance(node, ast.ImportFrom):
                        ...
                        if node.value is None:
                            ...
                            new_return = ast.Return(
                                value=ast.Call(
                                    ...
                                    keywords=[
                                        ast.keyword(
                                            arg="result", value=ast.Constant(value=None)
                                        ),
                                        ast.keyword(
                                            arg="tool_id",
                                            value=ast.Constant(value=tool_id),
                                        ),
                                    ],
                                )
                            )
                ...
                functions.append(
                    f"@mcp.tool(description='{escaped_desc}')\n{func_code}\n"
                )
            else:
                other_code.append(ast.unparse(node))
    ...
    def generate_mcp_server_code(self, code_str, params, name, description, tool_id):
        code = self._generate_mcp_server_code(
            code_str, params, name, description, tool_id
        )
        set_run_user = (
            f"os.setgid({pwd.getpwnam(_run_user).pw_gid});os.setuid({pwd.getpwnam(_run_user).pw_uid});"
            if _enable_sandbox
...
"""
    ...
    def get_tool_mcp_config(self, tool, params):
        _code = self.generate_mcp_server_code(
            tool.code, params, tool.name, tool.desc, str(tool.id)
        )
        chatflow_logger.debug(f"Python code of mcp tool: {_code}")
        compressed_and_base64_encoded_code_str = base64.b64encode(
            gzip.compress(_code.encode())
        ).decode()
        tool_config = {
    ...
    def _exec(self, execute_file):
        ...
        def _set_resource_limit():
            ...
            with suppress(Exception):
                resource.setrlimit(
                    resource.RLIMIT_AS, (_process_limit_mem_mb * 1024 * 1024,) * 2
                )
            with suppress(Exception):
                os.sched_setaffinity(
                    ...
                    set(
                        random.sample(
                            list(os.sched_getaffinity(0)), _process_limit_cpu_cores
                        )
                    ),
                )
        ...
        try:
            ...
            raise Exception(
                _(
                    f"Process execution timed out after {_process_limit_timeout_seconds} seconds."
                )
            )
    ...
    def validate_mcp_transport(self, code_str):
        ...
        for server, config in servers.items():
            if config.get("transport") not in ["sse", "streamable_http"]:
                raise Exception(
                    _("Only support transport=sse or transport=streamable_http")
                )
...
def execution_timer(id=""):
    ...
    try:
        ...
        chatflow_logger.debug(
            f"Tool execution({id}) takes {time.perf_counter() - start:.6f} seconds."
        )
