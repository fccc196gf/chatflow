...
SAFE_CHAR_SET = (
    [
        chr(i) for i in range(65, 91) if chr(i) not in {"I", "O"}
    ]  # 大写字母 A-H, J-N, P-Z
    + [
        chr(i) for i in range(97, 123) if chr(i) not in {"i", "l", "o"}
    ]  # 小写字母 a-h, j-n, p-z
...
def _remove_empty_lines(text):
    if not isinstance(text, str):
        raise AppApiException(
            500, _("Text-to-speech node, the text content must be of string type")
        )
    if not text:
        raise AppApiException(
            500, _("Text-to-speech node, the text content cannot be empty")
        )
...
def markdown_to_plain_text(md: str) -> str:
    ...
    text = re.sub(
        r"<(?:audio|video)(?:\s+[^>]*)?>[\s\S]*?(?:</(?:audio|video)>)?",
        "",
        text,
        flags=re.IGNORECASE,
    )
...
def sil_to_wav(silk_path, wav_path, rate: int = 24000):
    ...
    try:
        ...
        raise AppApiException(
            "import pysilk failed, wechaty voice message will not be supported."
        )
...
def split_and_transcribe(
    file_path, model, max_segment_length_ms=59000, audio_format="mp3"
):
    ...
    if audio_length_ms <= max_segment_length_ms:
        return model.speech_to_text(
            io.BytesIO(audio_data.export(format=audio_format).read())
        )
    ...
    for start_ms in range(0, audio_length_ms, max_segment_length_ms):
        ...
        text = model.speech_to_text(
            io.BytesIO(segment.export(format=audio_format).read())
        )
