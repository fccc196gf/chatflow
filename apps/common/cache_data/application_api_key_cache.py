...
@get_cache(
    cache_key=Cache_Version.APPLICATION_API_KEY.get_key_func(),
...
def get_application_api_key(secret_key, use_get_data):
    application_api_key = (
        QuerySet(ApplicationApiKey).filter(secret_key=secret_key[7:]).first()
    )
    return {
        "allow_cross_domain": application_api_key.allow_cross_domain,
...
def del_application_api_key(secret_key):
    cache.delete(
        Cache_Version.APPLICATION_API_KEY.get_key(
            secret_key=secret_key, use_get_data=True
        ),
        version=Cache_Version.APPLICATION_API_KEY.get_version(),
    )
