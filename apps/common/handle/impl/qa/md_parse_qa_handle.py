            ...
            if "|" in line and line.startswith("|"):
                ...
                while i < len(lines):
                    ...
                    if not line.startswith("|"):
                        break
                    row = [
                        self._unescape_cell_content(cell)
        ...
        try:
            ...
            for sheet_name, sheet_content in sheet_sections:
                ...
                for table in tables:
                    ...
                    # 处理表格的每一行数据
                    for row in table[1:]:
                        content_text = get_row_value(
                            row, title_row_index_dict, "content"
                        )
                        if content_text is None:
                            continue

                        problem = get_row_value(
                            row, title_row_index_dict, "problem_list"
                        )
            ...
            chatflow_logger.error(
                f"Error processing Markdown file {file.name}: {e}, {traceback.format_exc()}"
            )
