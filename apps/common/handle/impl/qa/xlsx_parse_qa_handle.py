...
def handle_sheet(file_name, sheet, image_dict):
    ...
    for row in rows:
        ...
        problem = (
            str(problem.value)
            if problem is not None and problem.value is not None
            else ""
        )
        problem_list = [
            {"content": p[0:255]} for p in problem.split("\n") if len(p.strip()) > 0
        ]
        title = get_row_value(row, title_row_index_dict, "title")
        title = (
            str(title.value) if title is not None and title.value is not None else ""
        )
