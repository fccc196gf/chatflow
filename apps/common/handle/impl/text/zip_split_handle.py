...
def file_to_paragraph(
    file, pattern_list: List, with_filter: bool, limit: int, save_inner_image
):
            ...
            for image in image_list:
                ...
                if search:
                    ...
                    if not zip_files.__contains__(image_path):
                        continue
                    if image_path.startswith("oss/file/") or image_path.startswith(
                        "oss/image/"
                    ):
                        image_id = image_path.replace("oss/file/", "").replace(
                            "oss/file/", ""
                        )
                        if is_valid_uuid(image_id):
                            ...
                            content = content.replace(
                                source_image_path, f"./oss/file/{new_image_id}"
                            )
                            p["content"] = content
                    else:
                        image_file_list.append(
                ...
                with zip_ref.open(file) as f:
                    ...
                    try:
                        ...
                        value = file_to_paragraph(
                            f, pattern_list, with_filter, limit, save_image
                        )
                ...
                with zip_ref.open(inner_name) as zf:
                    ...
                    for split_handle in split_handles:
                        if split_handle.support(zf, get_buffer):
                            row = get_buffer(zf)
                            md_text = split_handle.get_content(
                                io.BytesIO(row), save_image
                            )
                            file_content_list.append(
                                {"content": md_text, "name": real_name}
                            )
                            break
