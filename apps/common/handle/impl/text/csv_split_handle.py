...
def row_to_md(row):
    return (
        "| "
        + " | ".join([post_cell(cell) if cell is not None else "" for cell in row])
        + " |\n"
    )
...
class CsvSplitHandle(BaseSplitHandle):
    def handle(
        ...
        try:
            ...
            reader = csv.reader(
                io.TextIOWrapper(
                    io.BytesIO(buffer), encoding=detect(buffer)["encoding"]
                )
            )
            try:
            ...
            chatflow_logger.error(
                f"Error processing CSV file {file.name}: {e}, {traceback.format_exc()}"
            )
            return result
