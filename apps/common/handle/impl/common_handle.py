    ...
    for dep in deps:
        try:
            ...
            chatflow_logger.error(
                f"Error reading image {dep.target}: {e}, {traceback.format_exc()}"
            )
            continue
...
def xlsx_embed_cells_images(buffer) -> {}:
    ...
    for key, img in cell_images_xml.items():
        ...
        image_excel_id_list = [
            cell for cell in all_cells if isinstance(cell, str) and key in cell
        ]
        # print(key, img)
        if img is None:
            continue
        if len(image_excel_id_list) > 0:
            ...
            image = File(
                id=uuid.uuid7(),
                file_name=img.path,
                meta={"debug": False, "content": img_byte.getvalue()},
            )
