    ...
    system_urlpatterns += [
        path(
            ...
            SpectacularAPIView.as_view(
                patterns=[
                    URLPattern(
                        pattern=f"{chat_api_prefix}{str(url.pattern)}",
                        callback=url.callback,
                        default_args=url.default_args,
                        name=url.name,
                    )
                    for url in chat_urlpatterns
                    if ["chat", "open", "profile"].__contains__(url.name)
                ]
            ),
            name="chat_schema",
        ),  # schema的配置文件的路由，下面两个ui也是根据这个配置文件来生成的
        path(
            f"{CONFIG.get_chat_path()[1:]}/api-doc/",
            ChatSpectacularSwaggerView.as_view(url_name="chat_schema"),
            name="swagger-ui",
        ),  # swagger-ui的路由
    ]
...
init_list = [
    (
        init_app_doc,
        {
            "valid": lambda: CONFIG.get("DOC_PASSWORD") is not None
            and encrypt(CONFIG.get("DOC_PASSWORD"))
            == "d4fc097197b4b90a122b92cbd5bbe867",
            "get_call": get_call,
