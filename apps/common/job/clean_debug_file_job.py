...
def clean_debug_file_lock():
    ...
    chatflow_logger.debug(_("start clean debug file"))
    ...
    File.objects.filter(
        Q(
            create_time__lt=one_days_ago,
            source_type=FileSourceType.TEMPORARY_1_DAY.value,
        )
        | Q(
            create_time__lt=two_hours_ago,
            source_type=FileSourceType.TEMPORARY_120_MINUTE.value,
        )
        | Q(
            create_time__lt=minutes_30_ago,
            source_type=FileSourceType.TEMPORARY_30_MINUTE.value,
        )
    ).delete()
    chatflow_logger.debug(_("end clean debug file"))
...
def run():
    ...
    if rlock.try_lock("clean_debug_file", 30 * 30):
        try:
            chatflow_logger.debug("get lock clean_debug_file")
