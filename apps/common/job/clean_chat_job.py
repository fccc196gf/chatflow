...
def clean_chat_log_job_lock():
    ...
    chatflow_logger.info(_("start clean chat log"))
    now = timezone.now()

    applications = Application.objects.all().values(
        "id", "clean_time", "file_clean_time"
    )
    cutoff_dates = {
    ...
    file_cutoff_dates = {
        app["id"]: now
        - datetime.timedelta(days=app["file_clean_time"] or app["clean_time"] or 180)
        for app in applications
    }
    file_conditions = Q()
    ...
    chatflow_logger.info(_("end clean chat log"))
    ...
    while True:
        with transaction.atomic():
            ...
            max_create_times = (
                ...
                .values("chat_id")
                .annotate(max_create_time=Max("create_time"))
            )
            ...
            for record in chat_records:
                ...
                if max_create_time:
                    files_to_delete.extend(
                        File.objects.filter(
                            source_id=str(record.chat_id),
                            create_time__lt=max_create_time,
                        )
                    )
            # 删除 ChatRecord
            deleted_count = 0
            if clean_log:
                deleted_count = ChatRecord.objects.filter(
                    id__in=chat_record_ids
                ).delete()[0]

                from django.db.models import Count

                updated_counts = (
                    ChatRecord.objects.filter(chat_id__in=chat_ids)
                    .values("chat_id")
            ...
            File.objects.filter(
                loid__in=[file.loid for file in files_to_delete]
            ).delete()
...
def run():
    ...
    if rlock.try_lock("clean_chat_log_job", 30 * 30):
        try:
            chatflow_logger.debug("get lock clean_chat_log_job")

            existing_job = scheduler.get_job(job_id="clean_chat_log")
            if existing_job is not None:
                existing_job.remove()
            scheduler.add_job(
                clean_chat_log_job, "cron", hour="0", minute="5", id="clean_chat_log"
            )
        finally:
