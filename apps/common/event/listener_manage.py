...
class UpdateProblemArgs:
    def __init__(
        self, problem_id: str, problem_content: str, embedding_model: Embeddings
    ):
...
class ListenerManagement:
    ...
    def embedding_by_paragraph_list(paragraph_id_list, embedding_model: Embeddings):
        try:
            data_list = native_search(
                {
                    "problem": QuerySet(
                        get_dynamics_model(
                            {"paragraph.id": django.db.models.CharField()}
                        )
                    ).filter(**{"paragraph.id__in": paragraph_id_list}),
                    "paragraph": QuerySet(Paragraph).filter(id__in=paragraph_id_list),
                },
                select_string=get_file_content(
                    os.path.join(
                        PROJECT_DIR, "apps", "common", "sql", "list_embedding_text.sql"
                    )
                ),
            )
            ...
            chatflow_logger.error(
                _(
                    "Query vector data: {paragraph_id_list} error {error} {traceback}"
                ).format(
                    paragraph_id_list=paragraph_id_list,
                    error=str(e),
    ...
    @staticmethod
    def embedding_by_paragraph_data_list(
        data_list, paragraph_id_list, embedding_model: Embeddings
    ):
        chatflow_logger.info(
            _("Start--->Embedding paragraph: {paragraph_id_list}").format(
                paragraph_id_list=paragraph_id_list
            )
        )
        try:
            # 删除段落
            VectorStore.get_embedding_vector().delete_by_paragraph_ids(
                paragraph_id_list
            )
            ...
            VectorStore.get_embedding_vector().batch_save(
                data_list, embedding_model, is_save_function
            )
            ...
            chatflow_logger.error(
                _(
                    "Vectorized paragraph: {paragraph_id_list} error {error} {traceback}"
                ).format(
                    paragraph_id_list=paragraph_id_list,
                    error=str(e),
                    traceback=traceback.format_exc(),
            ...
            chatflow_logger.info(
                _("End--->Embedding paragraph: {paragraph_id_list}").format(
                    paragraph_id_list=paragraph_id_list
                )
            )
    ...
    def embedding_by_paragraph(paragraph_id, embedding_model: Embeddings):
        ...
        """
        chatflow_logger.info(
            _("Start--->Embedding paragraph: {paragraph_id}").format(
                paragraph_id=paragraph_id
            )
        )
        # 更新到开始状态
        ListenerManagement.update_status(
            QuerySet(Paragraph).filter(id=paragraph_id),
            TaskType.EMBEDDING,
            State.STARTED,
        )
        try:
            ...
            def is_the_task_interrupted():
                ...
                if (
                    _paragraph is None
                    or Status(_paragraph.status)[TaskType.EMBEDDING] == State.REVOKE
                ):
                    return True
                return False
            ...
            # 批量向量化
            VectorStore.get_embedding_vector().batch_save(
                data_list, embedding_model, is_the_task_interrupted
            )
            # 更新到开始状态
            ListenerManagement.update_status(
                QuerySet(Paragraph).filter(id=paragraph_id),
            ...
            chatflow_logger.error(
                _(
                    "Vectorized paragraph: {paragraph_id} error {error} {traceback}"
                ).format(
                    paragraph_id=paragraph_id,
                    error=str(e),
                    traceback=traceback.format_exc(),
            ...
            chatflow_logger.info(
                _("End--->Embedding paragraph: {paragraph_id}").format(
                    paragraph_id=paragraph_id
                )
            )
    ...
    def embedding_by_data_list(data_list: List, embedding_model: Embeddings):
        # 批量向量化
        VectorStore.get_embedding_vector().batch_save(
            data_list, embedding_model, lambda: False
        )

    @staticmethod
    def get_embedding_paragraph_apply(
        embedding_model, is_the_task_interrupted, post_apply=lambda: None
    ):
        def embedding_paragraph_apply(paragraph_list):
            for paragraph in paragraph_list:
                ...
                ListenerManagement.embedding_by_paragraph(
                    str(paragraph.get("id")), embedding_model
                )
            post_apply()
    ...
    def get_aggregation_document_status(document_id):
        def aggregation_document_status():
            ...
            native_update(
                {"document_custom_sql": QuerySet(Document).filter(id=document_id)},
                sql,
    ...
    def get_aggregation_document_status_by_knowledge_id(knowledge_id):
        def aggregation_document_status():
            ...
            native_update(
                {
                    "document_custom_sql": QuerySet(Document).filter(
    ...
    def post_update_document_status(document_id, task_type: TaskType):
        ...
        ListenerManagement.update_status(
            QuerySet(Document).filter(id=document_id), task_type, status[task_type]
        )
        ...
        ListenerManagement.update_status(
            ...
            .annotate(
                ...
                task_type_status=Substr(
                    "reversed_status", task_type.value, task_type.value
                ),
            )
            .filter(task_type_status=State.REVOKE.value)
    ...
    def update_status(query_set: QuerySet, taskType: TaskType, state: State):
        exec_sql = get_file_content(
            os.path.join(
                PROJECT_DIR, "apps", "knowledge", "sql", "update_paragraph_status.sql"
            )
        )
    ...
    def embedding_by_document(
        document_id, embedding_model: Embeddings, state_list=None
    ):
        """
        向量化文档
        @param state_list:
        ...
        try:
            ...
            def is_the_task_interrupted():
                ...
                if (
                    document is None
                    or Status(document.status)[TaskType.EMBEDDING] == State.REVOKE
                ):
                    return True
                return False
            ...
            if is_the_task_interrupted():
                return
            chatflow_logger.info(
                _("Start--->Embedding document: {document_id}").format(
            ...
            chatflow_logger.error(
                _(
                    "Vectorized document: {document_id} error {error} {traceback}"
                ).format(
                    document_id=document_id,
                    error=str(e),
                    traceback=traceback.format_exc(),
            ...
            ListenerManagement.post_update_document_status(
                document_id, TaskType.EMBEDDING
            )
            ListenerManagement.get_aggregation_document_status(document_id)()
            chatflow_logger.info(
                _("End--->Embedding document: {document_id}").format(
                    document_id=document_id
                )
            )
    ...
    def embedding_by_knowledge(knowledge_id, embedding_model: Embeddings):
        ...
        """
        chatflow_logger.info(
            _("Start--->Embedding knowledge: {knowledge_id}").format(
                knowledge_id=knowledge_id
            )
        )
        try:
            ...
            chatflow_logger.info(
                _("Start--->Embedding document: {document_list}").format(
                    document_list=document_list
                )
            )
            for document in document_list:
                ListenerManagement.embedding_by_document(
                    document.id, embedding_model=embedding_model
                )
        except Exception as e:
            chatflow_logger.error(
                _(
                    "Vectorized knowledge: {knowledge_id} error {error} {traceback}"
                ).format(
                    knowledge_id=knowledge_id,
                    error=str(e),
                    traceback=traceback.format_exc(),
            ...
            chatflow_logger.info(
                _("End--->Embedding knowledge: {knowledge_id}").format(
                    knowledge_id=knowledge_id
                )
            )
    ...
    def delete_embedding_by_source(source_id):
        VectorStore.get_embedding_vector().delete_by_source_id(
            source_id, SourceType.PROBLEM
        )

    @staticmethod
    def disable_embedding_by_paragraph(paragraph_id):
        VectorStore.get_embedding_vector().update_by_paragraph_id(
            paragraph_id, {"is_active": False}
        )

    @staticmethod
    def enable_embedding_by_paragraph(paragraph_id):
        VectorStore.get_embedding_vector().update_by_paragraph_id(
            paragraph_id, {"is_active": True}
        )
    ...
    def update_embedding_document_id(args: UpdateEmbeddingDocumentIdArgs):
        if args.target_embedding_model is None:
            ...
            ListenerManagement.embedding_by_paragraph_list(
                args.paragraph_id_list, embedding_model=args.target_embedding_model
            )

    @staticmethod
    def delete_embedding_by_source_ids(source_ids: List[str]):
        VectorStore.get_embedding_vector().delete_by_source_ids(
            source_ids, SourceType.PROBLEM
        )
