...
def to_result(key, args, parent_key=None):
    ...
    error_detail = list(
        filter(
            lambda d: (
                ...
                else True if isinstance(d, dict) and len(d.keys()) > 0 else False
            ),
            (
                args[0]
                if len(args) > 0
    ...
    return result.Result(
        ...
        message=(
            f"【{key if parent_key is None else parent_key + '.' + key}】为必填参数"
    ...
    if isinstance(exc_detail, dict):
        ...
        for key in keys:
            ...
            if isinstance(_value, list):
                for v in _value:
                    r = find_err_detail(
                        ReturnDict({key: v}, serializer=exc_detail.serializer)
                    )
                    if r is not None:
                        return r
            ...
            if isinstance(_value, dict) and len(_value.keys()) > 0:
                try:
                    return find_err_detail(
                        ReturnDict(_value, serializer=exc_detail.serializer.fields[key])
                    )
                except Exception as e:
                    return _value
...
def handle_exception(exc, context):
    ...
    if response is None:
        chatflow_logger.error(f"{str(exc)}:{traceback.format_exc()}")
