...
class BaseActionCommand(BaseCommand):
    ...
    def initial_util(self, *args, **options):
        ...
        services = Services.get_service_objects(
            service_names=service_names, **service_kwargs
        )

        kwargs = {
            "services": services,
            "run_daemon": options.get("daemon", False),
            "stop_daemon": self.action == Action.stop.value
            and Services.all.value in service_names,
    ...
    def handle(self, *args, **options):
        ...
        assert (
            self.action in Action.values
        ), f"The action {self.action} is not in the optional list"
