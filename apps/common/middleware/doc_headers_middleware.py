...
""".replace("/api/user/profile", CONFIG.get_admin_path() + "/api/user/profile").replace(
    "/admin/login", CONFIG.get_admin_path() + "/login"
)
...
class DocHeadersMiddleware(MiddlewareMixin):
    def process_response(self, request, response):
        if request.path.startswith(
            CONFIG.get_admin_path() + "/api-doc/"
        ) or request.path.startswith(CONFIG.get_chat_path() + "/api-doc/"):
