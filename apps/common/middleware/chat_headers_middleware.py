...
class ChatHeadersMiddleware(MiddlewareMixin):
    def process_response(self, request, response):
        ...
        if request.path.startswith(
            CONFIG.get_chat_path()
        ) and not request.path.startswith(CONFIG.get_chat_path() + "/api"):
            ...
            if application_access_token is not None:
                ...
                if white_active:
                    ...
                    response["Content-Security-Policy"] = (
                        f'frame-ancestors {" ".join(white_list)}'
                    )
                response.content = (
                    ...
                    .replace(
                        "<title>CHATFLOW</title>", f"<title>{application_name}</title>"
                    )
                    .encode("utf-8")
                )
        return response
