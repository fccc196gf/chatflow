...
class ModelCreateRequest(serializers.Serializer):
    ...
    model_params_form = serializers.ListField(
        required=False, default=list, label=_("parameter configuration")
    )
    credential = serializers.DictField(
        required=True, label=_("certification information")
    )
...
class ModelPullManage:
    ...
    def pull(model: Model, credential: Dict):
        try:
            ...
            QuerySet(Model).filter(id=model.id).update(
                meta={"down_model_chunk": [], "message": message}, status=status
            )
...
class ModelSerializer(serializers.Serializer):
    ...
    class Operate(serializers.Serializer):
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            if model.workspace_id == "None":
                raise AppApiException(
                    500, _("Shared models cannot be deleted or modified")
                )
        ...
        def pause_download(self, with_valid=True):
            ...
            QuerySet(Model).filter(id=self.data.get("id")).update(
        ...
        def edit(self, instance: Dict, user_id: str, with_valid=True):
            ...
            try:
                ...
                default_params = {
                    ...
                    for item in model.model_params_form
                }
                # 校验模型认证数据
            ...
            for update_key in update_keys:
                if update_key in instance and instance.get(update_key) is not None:
                    if update_key == "credential":
                        ...
                        model.__setattr__(
                            update_key, rsa_long_encrypt(model_credential_str)
                        )
                    else:
            ...
            if model.status == Status.DOWNLOAD:
                thread = threading.Thread(
                    target=ModelPullManage.pull, args=(model, credential)
                )
                thread.start()
    ...
    class Edit(serializers.Serializer):
        ...
        name = serializers.CharField(
            required=False, max_length=64, label=(_("model name"))
        )
        ...
        credential = serializers.DictField(
            required=False, label=(_("certification information"))
        )
        workspace_id = serializers.CharField(required=False, label=(_("workspace id")))
        ...
        def is_valid(self, model=None, raise_exception=False):
            ...
            if "name" in self.data and self.data.get("name") is not None:
                ...
                if (
                    ...
                    raise AppApiException(
                        500,
                        _("base model【{model_name}】already exists").format(
            ...
            model_credential = ModelProvideConstants[
                provider
            ].value.get_model_credential(model_type, model_name)
            source_model_credential = json.loads(rsa_long_decrypt(model.credential))
            source_encryption_model_credential = model_credential.encryption_dict(
            ...
            if credential is not None:
                for k in source_encryption_model_credential.keys():
                    if (
                        k in credential
                        and credential[k] == source_encryption_model_credential[k]
                    ):
    ...
    class Create(serializers.Serializer):
        ...
        name = serializers.CharField(
            required=True, max_length=64, label=_("model name")
        )
        provider = serializers.CharField(required=True, label=_("provider"))
        model_type = serializers.CharField(required=True, label=_("model type"))
        model_name = serializers.CharField(required=True, label=_("base model"))
        ...
        workspace_id = serializers.CharField(
            required=False, label=_("workspace id"), max_length=128
        )
        ...
        def is_valid(self, *, raise_exception=False):
            ...
            default_params = {
                item["field"]: item["default_value"]
                for item in self.data.get("model_params_form")
            }
    ...
    class Query(serializers.Serializer):
        ...
        def list(self, workspace_id, with_valid):
            ...
            query_params = self._build_query_params(
                workspace_id, workspace_manage, user_id
            )
