...
class XFZhEnSparkSpeechToText(CHATFLOWBaseModel, BaseSpeechToText):
    spark_app_id: str
    spark_api_key: str
    ...
    def speech_to_text(self, audio_file_path):
        ...
        try:
            ...
            chatflow_logger.error(f"语音识别错误: {str(err)}: {traceback.format_exc()}")
            raise
    ...
    async def send_audio(self, ws, audio_file):
        ...
        end_frame = self.merge_params_to_frame(
            end_frame,
            {
                key: value
                for key, value in self.params.items()
                if not ["model_id", "use_local", "streaming", "parameter"].__contains__(
                    key
                )
            },
        )
