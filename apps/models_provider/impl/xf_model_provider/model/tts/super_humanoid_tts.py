...
class XFSparkSuperHumanoidTextToSpeech(CHATFLOWBaseModel, BaseTextToSpeech):
        ...
        async def handle():
            try:
                ...
                if e.response.status_code == 401:
                    ...
                    raise Exception(
                        f"WebSocket connection failed: HTTP {e.response.status_code}"
                    )
    ...
    async def send(self, ws, text):
        ...
        tts_params = {
            **{
                ...
                if not ["parameter", "streaming", "model_id", "use_local"].__contains__(
                    key
                )
            },
