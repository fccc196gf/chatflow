...
class XunFeiTTSModelGeneralParams(BaseForm):
    vcn = forms.SingleSelect(
        TooltipLabel(
            _("Speaker"),
            _(
                "Speaker, optional value: Please go to the console to add a trial or purchase speaker. After adding, the speaker parameter value will be displayed."
            ),
        ),
        required=True,
        default_value="xiaoyan",
        text_field="value",
        value_field="value",
        option_list=[
...
class XunFeiTTSModelCredential(BaseForm, BaseModelCredential):
    spark_api_url = forms.TextInputField(
        "API URL", required=True, default_value="wss://tts-api.xfyun.cn/v2/tts"
    )
