        ...
        if self.usage_metadata is None or self.usage_metadata == {}:
            ...
            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(
                    super().get_num_tokens_from_messages, messages, tools
                )
                try:
                    response = future.result()
                    chatflow_logger.info("请求成功（未超时）")
                    return response
                    ...
                    if isinstance(e, ReadTimeout):
                        ...
                        return sum(
                            [
                                len(tokenizer.encode(get_buffer_string([m])))
                                for m in messages
                            ]
                        )

        return self.usage_metadata.get(
            "input_tokens", self.usage_metadata.get("prompt_tokens", 0)
        )
