...
class BedrockRerankerModelParams(BaseForm):
    top_n = forms.SliderField(
        TooltipLabel(
            _("Top N"), _("Number of top documents to return after reranking")
        ),
        required=True,
        default_value=3,
        _min=1,
...
class BedrockRerankerCredential(BaseForm, BaseModelCredential):
    ...
    region_name = forms.TextInputField(
        _("Region Name"), required=True, default_value="us-east-1"
    )
    ...
    def is_valid(
        ...
        if not any(
            ...
            raise AppApiException(
                ValidCode.valid_error.value, _("Model type is not supported")
            )
        ...
        try:
            ...
            if raise_exception:
                raise AppApiException(
                    ValidCode.valid_error.value,
                    _(
                        "Verification failed, please check whether the parameters are correct: %(error)s"
                    )
                    % {"error": str(e)},
                )
            else:
    ...
    def encryption_dict(self, model: Dict[str, object]):
        return {
            **model,
            "access_key_id": super().encryption(model.get("access_key_id", "")),
