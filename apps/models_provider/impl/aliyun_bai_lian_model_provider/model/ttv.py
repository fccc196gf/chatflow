...
class GenerationVideoModel(CHATFLOWBaseModel, BaseGenerationVideo):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                chatflow_logger.error(
                    f"⚠️ 网络错误: {e}，正在重试 {attempt + 1}/{self.max_retries}..."
                )
    ...
    def generate_video(
        ...
        **kwargs,
        ...
        chatflow_logger.info("task_id:", rsp.output.task_id)

        # --- 查询任务状态 ---
        status = self._safe_call(VideoSynthesis.fetch, task=rsp, api_key=self.api_key)
        if status.status_code == HTTPStatus.OK:
            chatflow_logger.info("当前任务状态:", status.output.task_status)
        else:
            chatflow_logger.error(
        ...
        if rsp.status_code == HTTPStatus.OK:
            chatflow_logger.info("视频生成完成！视频 URL:", rsp.output.video_url)
            if rsp.output.task_status == "SUCCEEDED":
                chatflow_logger.info("视频生成完成！视频 URL:", rsp.output.video_url)
                return rsp.output.video_url
            else:
                chatflow_logger.error("视频生成失败！")
                raise RuntimeError(f"生成失败, message: {rsp.output.message}")
        else:
            chatflow_logger.error(
                f"生成失败，status_code: {rsp.status_code}, code: {rsp.code}, message: {rsp.message}"
            )
            raise RuntimeError(
                f"生成失败，status_code: {rsp.status_code}, code: {rsp.code}, message: {rsp.message}"
            )
