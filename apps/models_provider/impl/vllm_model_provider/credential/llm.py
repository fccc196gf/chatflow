...
class VLLMModelCredential(BaseForm, BaseModelCredential):
    def is_valid(
        ...
        try:
            model_list = provider.get_base_model_list(
                model_credential.get("api_base"), model_credential.get("api_key")
            )
        except Exception as e:
            raise AppApiException(
                ValidCode.valid_error.value, gettext("API domain name is invalid")
            )
