...
class LocalReranker(CHATFLOWBaseModel, BaseDocumentCompressor):
    client: Any = None
    tokenizer: Any = None
    ...
    def __init__(self, model_name, cache_dir=None, **model_kwargs):
        ...
        self.client = AutoModelForSequenceClassification.from_pretrained(
    ...
    def compress_documents(
        ...
        with torch.no_grad():
            ...
            scores = [
                torch.sigmoid(s).float().item()
                for s in self.client(**inputs, return_dict=True)
                .logits.view(
                    -1,
                )
                .float()
            ...
            result = [
                Document(
                    ...
                    metadata={"relevance_score": scores[index]},
                )
                for index in range(len(documents))
            ]
            result.sort(
                key=lambda row: row.metadata.get("relevance_score"), reverse=True
            )
            return result
