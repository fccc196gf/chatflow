...
class RegoloTTIModelParams(BaseForm):
    size = forms.SingleSelect(
        TooltipLabel(
            _("Image size"),
            _(
                "The image generation endpoint allows you to create raw images based on text prompts. "
            ),
        ),
        required=True,
...
By default, images are produced in standard quality.
    ...
    n = forms.SliderField(
        TooltipLabel(_("Number of pictures"), _("1 as default")),
        required=True,
        default_value=1,
        _min=1,
        _max=10,
...
class RegoloTextToImageModelCredential(BaseForm, BaseModelCredential):
    api_base = forms.TextInputField(
        "API URL", required=True, default_value="https://api.regolo.ai/v1"
    )
