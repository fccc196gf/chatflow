...
class TencentTTIModelParams(BaseForm):
    Style = forms.SingleSelect(
        TooltipLabel(
            _("painting style"),
            _("If not passed, the default value is 201 (Japanese anime style)"),
        ),
        required=True,
        default_value="201",
        option_list=[
    ...
    Resolution = forms.SingleSelect(
        TooltipLabel(
            _("Generate image resolution"),
            _("If not transmitted, the default value is 768:768."),
        ),
        required=True,
...
class TencentTTIModelCredential(BaseForm, BaseModelCredential):
    ...
    def _validate_credential_fields(cls, model_credential, raise_exception=False):
        missing_keys = [
            key for key in cls.REQUIRED_FIELDS if key not in model_credential
        ]
        if missing_keys:
            if raise_exception:
                raise AppApiException(
    ...
    def encryption_dict(self, model):
        return {
            **model,
            "hunyuan_secret_key": super().encryption(
                model.get("hunyuan_secret_key", "")
            ),
        }
