...
class VolcanicEngineTTSModelGeneralParams(BaseForm):
    voice_type = forms.SingleSelect(
        TooltipLabel(
            _("timbre"),
            _("Chinese sounds can support mixed scenes of Chinese and English"),
        ),
        required=True,
    ...
    speed_ratio = forms.SliderField(
        TooltipLabel(
            _("speaking speed"),
            _("[0.2,3], the default is 1, usually one decimal place is enough"),
        ),
        required=True,
        default_value=1,
        _min=0.2,
...
class VolcanicEngineTTSModelCredential(BaseForm, BaseModelCredential):
    ...
    def encryption_dict(self, model: Dict[str, object]):
        return {
            **model,
            "volcanic_token": super().encryption(model.get("volcanic_token", "")),
        }
