...
class VolcanicASRClient:
    ...
    def _async_process(self, audio_data, submit_url):
        ...
        submit_response = requests.post(
            submit_url, data=json.dumps(request_body), headers=headers
        )
    ...
    def _handle_response(self, response, operation, silent=False):
        ...
        if "X-Api-Status-Code" in response.headers:
            if not silent:
                print(
                    f'{operation} response header X-Api-Status-Code: {response.headers["X-Api-Status-Code"]}'
                )
                print(
                    f'{operation} response header X-Api-Message: {response.headers["X-Api-Message"]}'
                )
                print(
                    f'{operation} response header X-Tt-Logid: {response.headers["X-Tt-Logid"]}'
                )
...
class VolcanicEngineBigModelSpeechToText(CHATFLOWBaseModel, BaseSpeechToText):
    volcanic_app_id: str
    ...
    def speech_to_text(self, audio_file):
        try:
            ...
            chatflow_logger.error(f"Error getting speech to text: {e}")
