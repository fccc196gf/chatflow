...
class VolcanicEngineEmbeddingModel(CHATFLOWBaseModel):
    api_key: str
    model_name: str
    api_base: str
    ...
    def __init__(
        self, api_key: str, model: str, api_base: str, params: Dict[str, object] = None
    ):
    ...
    def embed_documents(
        ...
        if self.model_name.startswith("doubao-embedding-vision-"):
            multimodal_inputs = []
            for text in texts:
                multimodal_inputs.append({"type": "text", "text": text})
            resp = self.client.multimodal_embeddings.create(
                model=self.model_name, input=multimodal_inputs, **(self.params or {})
            )
            return [resp.data.get("embedding")]
        else:
            resp = self.client.embeddings.create(
                model=self.model_name, input=texts, **(self.params or {})
            )
