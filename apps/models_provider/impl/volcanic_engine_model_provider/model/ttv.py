...
class GenerationVideoModel(CHATFLOWBaseModel, BaseGenerationVideo):
    api_key: str
    model_name: str
    params: dict
    max_retries: int = 3
    ...
    def _poll_task(
        self, client: Ark, task_id: str, max_wait: int = 60, interval: int = 5
    ):
        """轮询任务状态，直到完成或超时"""
        elapsed = 0
        while elapsed < max_wait:
            ...
            chatflow_logger.info(f"[ArkVideo] Task {task_id} status={status}")
            ...
            time.sleep(interval)
            elapsed += interval
        chatflow_logger.warning(f"[ArkVideo] Task {task_id} wait timeout")
        return None
    ...
    def generate_video(
        ...
        **kwargs,
        ...
        create_result = client.content_generation.tasks.create(
            model=self.model_name, content=content
        )
        ...
        task_id = task.id
        chatflow_logger.info(f"[ArkVideo] Created task {task_id}")
        ...
        try:
            if getattr(result, "status", None) in ("succeeded", "failed", "cancelled"):
                ...
                chatflow_logger.info(f"[ArkVideo] Deleted task {task_id}")
        except Exception as e:
            chatflow_logger.error(f"[ArkVideo] Failed to delete task {task_id}: {e}")
            raise e
        chatflow_logger.info("视频地址", result.content.video_url)
