...
class QianfanChatModelQianfan(CHATFLOWBaseModel, QianfanChatEndpoint):
    @staticmethod
    def is_cache_model():
        return False
...
class QianfanChatModelOpenai(CHATFLOWBaseModel, BaseChatOpenAI):
    @staticmethod
    def is_cache_model():
        return False
...
class QianfanChatModel(CHATFLOWBaseModel):
    @staticmethod
    def new_instance(
        model_type, model_name, model_credential: Dict[str, object], **model_kwargs
    ):
        ...
        if api_version == "v1":
            return QianfanChatModelQianfan.new_instance(
                model_type, model_name, model_credential, **model_kwargs
            )
        elif api_version == "v2":
            return QianfanChatModelOpenai.new_instance(
                model_type, model_name, model_credential, **model_kwargs
            )
