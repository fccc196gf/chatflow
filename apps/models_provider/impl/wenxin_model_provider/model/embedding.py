...
class QianfanV1Embeddings(CHATFLOWBaseModel, QianfanEmbeddingsEndpoint):
    @staticmethod
    def new_instance(
...
class QianfanV2EmbeddingModel(CHATFLOWBaseModel):
    model_name: str
...
class QianfanEmbeddings(CHATFLOWBaseModel):
    ...
    def new_instance(
        ...
        if api_version == "v1":
            return QianfanV1Embeddings.new_instance(
                model_type, model_name, model_credential, **model_kwargs
            )
        elif api_version == "v2":
            return QianfanV2EmbeddingModel.new_instance(
                model_type, model_name, model_credential, **model_kwargs
            )
