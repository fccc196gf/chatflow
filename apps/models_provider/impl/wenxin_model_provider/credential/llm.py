...
class WenxinLLMModelCredential(BaseForm, BaseModelCredential):
    def is_valid(
        ...
        if api_version == "v1":
            ...
            if not model_info.__contains__(model_name.lower()):
                raise AppApiException(
                    ValidCode.valid_error.value,
                    gettext("{model_name} The model does not support").format(
    ...
    def encryption_dict(self, model_info: Dict[str, object]):
        ...
        if api_version == "v1":
            return {
                **model_info,
                "secret_key": super().encryption(model_info.get("secret_key", "")),
            }
        else:  # v2
            return {
                **model_info,
                "api_key": super().encryption(model_info.get("api_key", "")),
            }
        ...
        if api_version == "v1":
            for key in ["api_version", "api_key", "secret_key", "model"]:
                if key not in model_info:
                    raise AppApiException(
                        500, gettext("{key}  is required").format(key=key)
                    )
    ...
    api_version = forms.Radio(
        "API Version",
        required=True,
        text_field="label",
        value_field="value",
        option_list=[{"label": "v1", "value": "v1"}, {"label": "v2", "value": "v2"}],
        default_value="v1",
        provider="",
        method="",
    )
    ...
    api_base = forms.TextInputField(
        "API URL", required=True, relation_show_field_dict={"api_version": ["v2"]}
    )

    # v1版本字段
    api_key = forms.PasswordInputField("API Key", required=True)
    secret_key = forms.PasswordInputField(
        "Secret Key", required=True, relation_show_field_dict={"api_version": ["v1"]}
    )
