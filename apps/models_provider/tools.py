...
def get_model_by_id(_id, workspace_id):
    ...
    if (
        model
        and model.workspace_id != workspace_id
        and get_authorized_model is not None
    ):
        model = get_authorized_model(
            QuerySet(Model).filter(id=_id), workspace_id
        ).first()
    if model is None:
...
def get_model_instance_by_model_workspace_id(model_id, workspace_id, **kwargs):
    ...
    return ModelManage.get_model(
        model_id, lambda _id: get_model(model, **{**s, **kwargs})
    )
