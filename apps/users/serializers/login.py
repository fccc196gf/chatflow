...
class LoginRequest(serializers.Serializer):
    username = serializers.CharField(
        required=True, max_length=64, help_text=_("Username"), label=_("Username")
    )
    ...
    encryptedData = serializers.CharField(
        required=False, label=_("encryptedData"), allow_null=True, allow_blank=True
    )
...
class LoginSerializer(serializers.Serializer):
    ...
    def get_auth_setting():
        ...
        if auth_setting_model:
            setting_obj = auth_setting_model.objects.filter(
                param_key="auth_setting"
            ).first()
            if setting_obj:
                try:
    ...
    def login(instance):
        ...
        # 检查许可证有效性
        license_validator = DatabaseModelManage.get_model("license_is_valid") or (
            lambda: False
        )
        is_license_valid = (
        ...
        user = (
            QuerySet(User)
            .filter(username=username, password=password_encrypt(password))
            .first()
        )

        if not user:
            LoginSerializer._handle_failed_login(
                username, is_license_valid, failed_attempts, lock_time
            )
    ...
    def _is_account_locked(username: str, failed_attempts: int) -> bool:
        ...
        if failed_attempts == -1:
            return False
        lock_cache = cache.get(
            system_get_key(f"system_{username}_lock"), version=system_version
        )
    ...
    def _need_captcha(username: str, max_attempts: int) -> bool:
        ...
        if max_attempts == -1:
            ...
            fail_count = (
                cache.get(system_get_key(f"system_{username}"), version=system_version)
                or 0
            )
    ...
    def _handle_failed_login(
        username: str, is_license_valid: bool, failed_attempts: int, lock_time: int
    ) -> None:
        """处理登录失败"""
        ...
        fail_count = (
            cache.get(
                system_get_key(f"system_{username}_lock_count"), version=system_version
            )
            or 0
        )
        ...
        if remain_attempts > 0:
            raise AppApiException(
                1005,
                _(
                    "Login failed %s times, account will be locked, you have %s more chances !"
                )
...
class CaptchaSerializer(serializers.Serializer):
    ...
    def generate(username: str, type: str = "system"):
        ...
        return CaptchaSerializer._generate_captcha_if_needed(
            username, type, need_captcha
        )
    ...
    def chat_generate(username: str, type: str = "chat", access_token: str = ""):
        ...
        if max_attempts == -1:
            ...
            fail_count = (
                cache.get(system_get_key(f"{type}_{username}"), version=system_version)
                or 0
            )
            need_captcha = fail_count >= max_attempts
