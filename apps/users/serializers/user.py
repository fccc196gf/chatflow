...
class UserProfileResponse(serializers.ModelSerializer):
    is_edit_password = serializers.BooleanField(
...
class CreateUserSerializer(serializers.Serializer):
    ...
    defaultPermission = serializers.CharField(
        required=False, label=_("defaultPermission")
    )
...
def is_workspace_manage(user_id: str, workspace_id: str):
    ...
    return (
        QuerySet(User)
        .filter(id=user_id, role=RoleConstants.ADMIN.value.__str__())
        .exists()
    )
...
def get_workspace_list_by_user(user_id):
    ...
    license_is_valid = DatabaseModelManage.get_model("license_is_valid") or (
        lambda: False
    )
...
class UserProfileSerializer(serializers.Serializer):
    ...
    def profile(user: User, auth: Auth):
        ...
        user_role_relation_model = DatabaseModelManage.get_model(
            "workspace_user_role_mapping"
        )
        role_name = [user.role]
        if user_role_relation_model:
            user_role_relations = (
                user_role_relation_model.objects.filter(user_id=user.id)
                .select_related("role")
                .distinct("role_id")
            )
        ...
        return {
            ...
            "is_edit_password": (
                user.password == "d880e722c47a34d8e9fce789fc62389d"
...
class UserManageSerializer(serializers.Serializer):
    class UserInstance(serializers.Serializer):
        ...
        source = serializers.CharField(
            required=False, label=_("Source"), max_length=20, default="LOCAL"
        )
        ...
        def _check_unique_username_and_email(self):
            ...
            user = User.objects.filter(
                Q(username=username) | Q(email=email) | Q(nick_name=nick_name)
            ).first()
            if user:
                if user.email == email:
    ...
    class Query(serializers.Serializer):
        username = serializers.CharField(
            required=False, label=_("Username"), max_length=64, allow_blank=True
        )
        nick_name = serializers.CharField(
            required=False, label=_("Nick Name"), max_length=64, allow_blank=True
        )
        ...
        def page(
            ...
            def _get_user_roles(user_ids, is_admin=True):
                ...
                workspace_mapping = {
                    str(workspace_model.id): workspace_model.name
                    for workspace_model in workspace_model.objects.all()
                }
                ...
                for relation in user_role_relations:
                    ...
                    user_role_workspace_mapping[user_id][
                        relation.role.role_name
                    ].append(workspace_mapping.get(workspace_id, workspace_id))

                    # 将 set 转换为 list 以符合返回格式
                user_role_mapping = {
                    uid: list(roles) for uid, roles in user_role_mapping.items()
                }

                # 转换为所需的结构
                result_user_role_setting_mapping = {
                    user_id: [
                        {"role_id": role_id, "workspace_ids": workspace_ids}
            ...
            if role_model and user_role_relation_model:
                ...
                (
                    user_role_mapping,
                    user_role_setting_mapping,
                    user_role_workspace_mapping,
                ) = _get_user_roles(user_ids, is_admin)
    ...
    class UserEditInstance(serializers.Serializer):
        ...
        def _check_unique_nick_name(self, user_id):
            ...
            if (
                nick_name
                and User.objects.filter(nick_name=nick_name)
                .exclude(id=user_id)
                .exists()
            ):
    ...
    class Operate(serializers.Serializer):
        ...
        def _check_not_admin(self):
            ...
            if (
                ...
                or str(user.id) == "f0dd8f71-e4ee-11ee-8c84-a8a1595801ab"
            ):
        ...
        def one(self, with_valid=True):
            ...
            if workspace_user_role_mapping_model:
                role_setting = {}
                workspace_user_role_mapping_list = QuerySet(
        ...
        def re_password(self, instance, with_valid=True):
            if with_valid:
                self.is_valid(raise_exception=True)
                UserManageSerializer.RePasswordInstance(data=instance).is_valid(
    ...
    class BatchDelete(serializers.Serializer):
        ...
        def batch_delete(self, with_valid=True):
            ...
            User.objects.filter(id__in=user_ids).exclude(
...
def update_user_role(instance, user, user_id=None):
    ...
    if workspace_user_role_mapping_model:
        ...
        license_is_valid = (
            license_is_valid() if license_is_valid() is not None else False
        )
        ...
        if str(user.id) == "f0dd8f71-e4ee-11ee-8c84-a8a1595801ab":
            ...
            for item in role_setting:
                ...
                if role_id == str(workspace_manage_role_id) or role_id == str(
                    RoleConstants.USER.value
                ):
...
def _get_workspace_ids(instance, default_permission):
    ...
    user_role_ids = set(
        role_setting_model.objects.filter(
            id__in=all_role_ids, type=RoleConstants.USER.name
...
def _set_resource_permissions(user_id, workspace_ids, default_permission, auth_type):
    ...
    for ws in workspace_ids:
        instances.extend(
            _create_resource_permission_instances(
...
def _get_resource_maps(workspace_ids):
    ...
    # 查询应用资源
    for ws, rid in Application.objects.filter(
    ...
    # 查询知识库资源
    for ws, kid in Knowledge.objects.filter(workspace_id__in=workspace_ids).values_list(
        "workspace_id", "id"
    ):
    ...
    # 查询工具资源
    for ws, tid in Tool.objects.filter(workspace_id__in=workspace_ids).values_list(
        "workspace_id", "id"
    ):
    ...
    # 查询模型资源
    for ws, mid in Model.objects.filter(workspace_id__in=workspace_ids).values_list(
        "workspace_id", "id"
    ):
...
def _create_resource_permission_instances(
...
class RePasswordSerializer(serializers.Serializer):
    ...
    def is_valid(self, *, raise_exception=False):
        ...
        if cache_code != self.data.get("code"):
            raise AppApiException(
                ExceptionCodeConstants.CODE_ERROR.value.code,
...
class SendEmailSerializer(serializers.Serializer):
    ...
    type = serializers.CharField(
        ...
        validators=[
            validators.RegexValidator(
                regex=re.compile("^register|reset_password$"),
    ...
    def is_valid(self, *, raise_exception=False):
        ...
        if ttl is not None and ttl > 0:
            raise AppApiException(
                500,
                _("Do not send emails again within {seconds} seconds").format(
    ...
    def send(self):
        ...
        system_setting = (
            QuerySet(SystemSetting).filter(type=SettingType.EMAIL.value).first()
        )
        ...
        try:
            ...
            send_mail(
                _(
                    "【Intelligent knowledge base question and answer system-{action}】"
                ).format(
                    action=(
                        _("User registration")
                        if state == "register"
                        else _("Change password")
                    )
                ),
                "",
...
class CheckCodeSerializer(serializers.Serializer):
    ...
    def is_valid(self, *, raise_exception=False):
        super().is_valid()
        value = cache.get(
            get_key(self.data.get("email") + ":" + self.data.get("type")),
            version=version,
        )
...
class SwitchLanguageSerializer(serializers.Serializer):
    ...
    def switch(self):
        ...
        if not support_language_list.__contains__(language):
            raise AppApiException(
                500, _("language only support:") + ",".join(support_language_list)
            )
