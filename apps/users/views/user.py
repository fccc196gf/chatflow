...
default_password = CONFIG.get("DEFAULT_PASSWORD", "CHATFLOW@123..")
...
def get_re_password_details(request):
    ...
    return {
        ...
        "body": {
            **body,
            "password": encryption(body.get("password", "")),
...
class UserProfileView(APIView):
    ...
    def get(self, request: Request):
        return result.success(
            UserProfileSerializer().profile(request.user, request.auth)
        )
...
class SwitchUserLanguageView(APIView):
    ...
    @log(
        ...
        operate="Switch Language",
        get_operation_object=lambda r, k: {"name": r.user.username},
    )
    @has_permissions(
        PermissionConstants.SWITCH_LANGUAGE,
        RoleConstants.ADMIN,
        RoleConstants.USER,
...
class TestWorkspacePermissionUserView(APIView):
    ...
    @has_permissions(
        PermissionConstants.USER_EDIT.get_workspace_permission(), RoleConstants.ADMIN
    )
...
class UserManage(APIView):
    ...
    def post(self, request: Request):
        return result.success(
            UserManageSerializer().save(request.data, str(request.user.id))
        )
    ...
    class Password(APIView):
        ...
        @has_permissions(
            ...
            PermissionConstants.CHAT_USER_CREATE,
            PermissionConstants.WORKSPACE_CHAT_USER_CREATE,
            RoleConstants.ADMIN,
            RoleConstants.WORKSPACE_MANAGE,
        )
        def get(self, request: Request):
    ...
    class Operate(APIView):
        ...
        @log(
            menu="User management",
            operate="Delete user",
            get_operation_object=lambda r, k: get_user_operation_object(
                k.get("user_id")
            ),
        )
        def delete(self, request: Request, user_id):
            return result.success(
                UserManageSerializer.Operate(data={"id": user_id}).delete(
                    with_valid=True
                )
            )
        ...
        def get(self, request: Request, user_id):
            return result.success(
                UserManageSerializer.Operate(data={"id": user_id}).one(with_valid=True)
            )
        ...
        def put(self, request: Request, user_id):
            return result.success(
                UserManageSerializer.Operate(data={"id": user_id}).edit(
                    request.data, str(request.user.id), with_valid=True
                )
            )
    ...
    class BatchDelete(APIView):
        ...
        def post(self, request: Request):
            return result.success(
                UserManageSerializer.BatchDelete({"ids": request.data}).batch_delete(
                    with_valid=True
                )
            )
    ...
    class RePassword(APIView):
        ...
        def put(self, request: Request, user_id):
            return result.success(
                UserManageSerializer.Operate(data={"id": user_id}).re_password(
...
class CheckCode(APIView):
    ...
    def post(self, request: Request):
        return result.success(
            CheckCodeSerializer(data=request.data).is_valid(raise_exception=True)
        )
...
class SendEmailToCurrentUserView(APIView):
    ...
    @log(
        menu="User management",
        operate="Send email to current user",
        get_operation_object=lambda r, k: {"name": r.user.username},
    )
    def post(self, request: Request):
        serializer_obj = SendEmailSerializer(
            data={"email": request.user.email, "type": "reset_password"}
        )
        if serializer_obj.is_valid(raise_exception=True):
            return result.success(serializer_obj.send())
