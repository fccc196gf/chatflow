...
logger = logging.getLogger("chatflow.conf")
...
class Config(dict):
            ...
            "default": {
                ...
                "OPTIONS": {
                    ...
                    "CONNECTION_POOL_KWARGS": {
                        "max_connections": int(self.get("REDIS_MAX_CONNECTIONS"))
                    },
                },
            },
        }
    ...
    def get_sandbox_python_package_paths(self):
        return self.get(
            ...
            "/opt/py3/lib/python3.11/site-packages,/opt/chatflow-app/sandbox/python-packages,/opt/chatflow/python-packages",
        )
...
class ConfigManager:
    ...
    def load_from_env(self):
        ...
        config = {
            key.replace("CHATFLOW_", ""): os.environ.get(key)
            for key in keys
            if key.startswith("CHATFLOW_")
        }
        if len(config.keys()) <= 0:
            msg = f"""
                             ...
                             Please set environment variables
                                CHATFLOW_CONFIG_TYPE: 配置文件读取方式 FILE: 使用配置文件配置  ENV: 使用ENV配置
                                CHATFLOW_DB_NAME: 数据库名称
                                CHATFLOW_DB_HOST: 数据库主机
                                CHATFLOW_DB_PORT: 数据库端口
                                CHATFLOW_DB_USER: 数据库用户名
                                CHATFLOW_DB_PASSWORD: 数据库密码
                                
                                CHATFLOW_REDIS_HOST:缓存数据库主机
                                CHATFLOW_REDIS_PORT:缓存数据库端口
                                CHATFLOW_REDIS_PASSWORD:缓存数据库密码
                                CHATFLOW_REDIS_DB:缓存数据库
                                CHATFLOW_REDIS_MAX_CONNECTIONS:缓存数据库最大连接数
                             """
            raise ImportError(msg)
    ...
    def load_user_config(cls, root_path=None, config_class=None):
        ...
        config_type = os.environ.get("CHATFLOW_CONFIG_TYPE")
