...
def pro():
    urlpatterns.append(
        re_path(
            rf"^{CONFIG.get_admin_path()[1:]}/api-doc/(?P<path>.*)$",
            static.serve,
            {
                "document_root": os.path.join(
                    settings.STATIC_ROOT, "drf_spectacular_sidecar"
                )
            },
            name="doc",
        ),
    ...
    urlpatterns.append(
        re_path(
            rf"^{CONFIG.get_chat_path()[1:]}/api-doc/(?P<path>.*)$",
            static.serve,
            {
                "document_root": os.path.join(
                    settings.STATIC_ROOT, "drf_spectacular_sidecar"
    ...
    urlpatterns.append(
        re_path(
            rf"^{CONFIG.get_admin_path()[1:]}/(?P<path>.*)$",
            static.serve,
    ...
    urlpatterns.append(
        re_path(
            rf"^{CONFIG.get_chat_path()[1:]}/(?P<path>.*)$",
            static.serve,
...
static_dict = {
    ...
    admin_ui_prefix: get_all_files(
        os.path.join(PROJECT_DIR, "apps", "static", "admin")
    ),
}
...
def page_not_found(request, exception):
    ...
    if request.path.startswith(chat_ui_prefix):
        in_ = [
            url for url in static_dict.get(chat_ui_prefix) if request.path.endswith(url)
        ]
        if len(in_) > 0:
        ...
        content = content.replace(
            "prefix: '/chat'", f"prefix: '{CONFIG.get_chat_path()}'"
        )
        ...
        in_ = [
            url
            for url in static_dict.get(admin_ui_prefix)
        ...
        content = get_index_html(index_path)
        content = content.replace(
            "prefix: '/admin'", f"prefix: '{CONFIG.get_admin_path()}'"
        ).replace("chatPrefix: '/chat'", f"chatPrefix: '{CONFIG.get_chat_path()}'")
        return HttpResponse(content, status=200)
    else:
