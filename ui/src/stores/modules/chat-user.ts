...
const normalizeMediaField = (value: any) => {
  ...
  actions: {
    ...
    getChatProfile() {
      return ChatAPI.chatProfile(this.accessToken as string).then((ok) => {
        const data = { ...(ok.data || {}) }
        data.icon = normalizeMediaField(data.icon)
        this.chat_profile = data
    ...
    applicationProfile() {
      return ChatAPI.applicationProfile().then((ok) => {
        const data = { ...(ok.data || {}) }
        data.icon = normalizeMediaField(data.icon)
        data.chat_background = normalizeMediaField(data.chat_background)
        data.avatar = normalizeMediaField(data.avatar)
        data.user_avatar = normalizeMediaField(data.user_avatar)
        data.float_icon = normalizeMediaField(data.float_icon)
