...
const radioContentStyle = computed(() => {
  if (width.value) {
    if (width.value < 350) {
      return { '--chatflow-radio-card-width': '316px' }
    } else if (width.value > 770) {
      return { '--chatflow-radio-card-width': '378px' }
    } else {
      return { '--chatflow-radio-card-width': '100%' }
    }
  }
  return {}
})
...
.radio-card {
  ...
  .item {
    ...
    align-items: center;
    width: var(--chatflow-radio-card-width, 100%);
    margin: 4px;
  }
}
</style>
