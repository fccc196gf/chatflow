...
function toChat(row: any) {
  ...
  api(row.id).then((ok) => {
    ...
    ApplicationApi.getAccessToken(row.id, loading)
      .then(async (res: any) => {
        ...
        if (res?.data?.is_active === false) {
          await ApplicationApi.putAccessToken(row.id, { is_active: true }, loading)
        }
        const newUrl = application.location + accessToken + apiParams
        window.open(newUrl)
      })
      .catch(() => {
