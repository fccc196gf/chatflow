...
function toChat(row: any) {
  ...
  const apiParams = mapToUrlParams(apiInputParams.value)
    ...
    : ''
  ApplicationResourceApi.getAccessToken(row.id, loading)
    .then(async (res: any) => {
      const accessToken = res?.data?.access_token
      if (!accessToken) {
        MsgError('应用访问链接无效，请刷新后重试')
        return
      }
      if (res?.data?.is_active === false) {
        await ApplicationResourceApi.putAccessToken(row.id, { is_active: true }, loading)
      }
      window.open(application.location + accessToken + apiParams)
    })
    .catch(() => {
      MsgError('打开对话失败，请检查应用访问设置')
    })
}
