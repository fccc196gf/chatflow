...
// 头像
export const defaultIcon = new URL('../assets/logo/favicon.ico', import.meta.url).href
...
export function getFileUrl(fileId?: string) {
  if (fileId) {
    return `${window.ChatFlow.prefix}/oss/file/${fileId}`
  }
  return ''
}

export const resetUrl = (url: string, defaultUrl?: string) => {
  if (url && url.toLowerCase().endsWith('/favicon.ico')) {
    return defaultIcon
  }
  if (url && url.startsWith('./')) {
    ...
    return `${window.ChatFlow.prefix}/${url.substring(2)}`
  }
